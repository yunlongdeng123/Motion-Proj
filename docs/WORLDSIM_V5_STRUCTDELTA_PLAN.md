...
- `WS-V5-M1-STRUCTURED-OWNERSHIP-01=running`；8-scene 30k base 已由 r035 复核为 `8/8 × 30k`，scene0471 frozen SAM r036 与 B0/B1/B3 unary r037 均已完成。r037 相对 B0 的 B1/B3 `ΔBoundary-F1=+0.107437/+0.105079`、`ΔECE=-0.110983/-0.112431`，但 `ΔFN semantic mass=+0.091532/+0.095477`；因此只确认 unary 单场方向支持，不选择 arm、不声明 validation 成功、不自动解锁 graph。当前门=`先冻结 graph 机制诊断协议`；详见 [`WS_V5_M1_FORMAL_BASE_UNARY_DIAGNOSTIC.md`](WS_V5_M1_FORMAL_BASE_UNARY_DIAGNOSTIC.md)。
