...
- `WS-V5-M1-STRUCTURED-OWNERSHIP-01=running`；result-blind 三场景 replication 已完成并失败：G3 对同 unary G0 的 Boundary F1 正向=`3/6`（门槛 `>=4/6`），mean ΔBoundary-F1=`+0.0016107723`、mean ΔFN-mass=`+0.0025676789`，逐场 topology `G3<G1`=`2/3`。裁决=`physical_graph_development_replication_rejected_3of6_boundary_support`；formal arm selection、validation、semantic split 与参数搜索继续锁定。下一步仅允许 boundary-residual forensic 判断是否满足 M1B 的条件前提。详见 [`WS_V5_M1_DEVELOPMENT_REPLICATION.md`](WS_V5_M1_DEVELOPMENT_REPLICATION.md)。
