...
- `WS-V5-M2-GEOMETRY-FIRST-REPAIR-01=running`；r004 冻结 `23=22 accepted+1 rejected` 的逐 actor request unit；G0 raw `22/22` fail，G1/G2/G3 的 `>=0.5m` 改善数分别为 `5/8/11`（门槛 `14/22`），三者均 rejected。当前只开放 Gaussianization representation/alpha-compositing forensic，router 与 validation 未解锁；见 [`WS_V5_M2_GEOMETRY_FIRST_DEVELOPMENT.md`](WS_V5_M2_GEOMETRY_FIRST_DEVELOPMENT.md)。
...
- 当前只开放冻结 development scene 上的 M2 Gaussianization/evidence-schema 工作；M1 已 rejected，M2 router、validation parameter search、完整 M3、新模型大训练、fresh test quality 与 KITTI 方法调参继续禁止。
