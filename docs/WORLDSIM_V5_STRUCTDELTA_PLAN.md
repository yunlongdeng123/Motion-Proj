...
- `WS-V5-M1-STRUCTURED-OWNERSHIP-01=running`；8-scene 30k base r035、scene0471 frozen SAM r036、unary r037 与 graph r038 均完成。r038 的 G3 相对同 unary G0 在 B1/B3 上 `ΔBoundary-F1=+0.008585/+0.006245`、`ΔFN-mass=+0.001491/+0.001061`，并把 proxy cross-boundary affinity ratio 从 G1=`0.008365` 降到 `0.004020`；因此登记 G3 mechanism preference，但单场效应很小且 Gaussian proxy 有退化，formal arm selection 仍为 false。当前门=`冻结 result-blind development replication protocol`；validation、semantic split 与参数搜索不解锁。详见 [`WS_V5_M1_FORMAL_BASE_UNARY_DIAGNOSTIC.md`](WS_V5_M1_FORMAL_BASE_UNARY_DIAGNOSTIC.md)。
