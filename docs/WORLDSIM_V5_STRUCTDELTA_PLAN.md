...
- `WS-V5-M1-STRUCTURED-OWNERSHIP-01=rejected`；result-blind replication 只有 `3/6` Boundary F1 单元正向。后续 boundary-residual forensic r001 又得到 boundary-primary=`0/6`、mean boundary classification/semantic-error share=`0.402095/0.248353`，故 M1B 条件未成立，semantic split 保持锁定。M1 不进入 validation、不选择 arm、不追加 Transformer；V5 当前转入 M2。详见 [`WS_V5_M1_DEVELOPMENT_REPLICATION.md`](WS_V5_M1_DEVELOPMENT_REPLICATION.md) 与 [`WS_V5_M1B_BOUNDARY_RESIDUAL_FORENSICS.md`](WS_V5_M1B_BOUNDARY_RESIDUAL_FORENSICS.md)。
...
- `WS-V5-M2-GEOMETRY-FIRST-REPAIR-01=running`；r004 冻结 `23=22 accepted+1 rejected` 的逐 actor request unit；G0 raw `22/22` fail，G1/G2/G3 的 `>=0.5m` 改善数分别为 `5/8/11`（门槛 `14/22`），三者均 rejected。r011 的 `2×2` Gaussianization forensic 对 r005 BASE `22/22 exact`：DENSE−BASE 改善=`20/22`、mean=`-0.424179m`，OPAQUE−BASE=`0/22`、mean=`+0.059686m`，故 density 机制受支持、opacity 不受支持；method arm 仍未选择。当前只开放一个结果盲冻结的 density representation repair，router 与 validation 未解锁；见 [`WS_V5_M2_GEOMETRY_FIRST_DEVELOPMENT.md`](WS_V5_M2_GEOMETRY_FIRST_DEVELOPMENT.md)。
...
- 当前只开放冻结 development scene 上的 M2 density representation repair/evidence-schema 工作；M1 已 rejected，M2 router、validation parameter search、完整 M3、新模型大训练、fresh test quality 与 KITTI 方法调参继续禁止。
