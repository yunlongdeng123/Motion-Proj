...
更新日期：2026-07-14。Phase 2 的最终决策是 **E：停止当前 explicit dynamics projection 核心方向**，唯一 fallback 为 **D：仅在新 target 与独立 evaluator 均可验证时继续只读诊断**。C0 已通过；P0 为 machine pass / awaiting human review；P1 target legality 已 machine fail；E0 因官方 checkpoint 不可达而 blocked。因此 F1-R、F2、F3、O1、R0 与任何生成器训练均不排程。A1 仍有效：F0/F1 不可被外推为所有 endpoint 或连续 feature relation 的普遍失败；本次停止源于 P1 的合法 counterfactual hard failure，而非单独依赖 feature cell 统计。所有预算仍限制为单张 RTX 4090 24 GB，禁止 Optuna、300/800-step、数百 GB cache 和自动 push。
...
已发生的分支：A0 pass；F0 fail；旧 projector 的 F1 fail；C0 pass；P0 machine pass / awaiting review；P1 fail；E0 blocked。没有自动“下一步”：任何重新打开都必须是新 run ID、只读且先完成新的预注册。
...
## E0 — Independent rollout evaluator validity（blocked；no fallback）
...
| Promotion threshold | identical-video rerun aggregate relative delta `<=2%`；survival-threshold sweep rank correlation `>=0.8`；synthetic acceleration/jerk ordering 正确；12-case decisive valid `>=87.5%`；输入 provenance 只含 generated RGB + first-frame grid + evaluator weights。 |
...
| Artifacts | `runs/autoresearch-e0-evaluator-*/{manifest.json,resolved.yaml,metrics.jsonl,machine_summary.json,summary.json,track_overlay,reviews.template.jsonl,COMPLETE}`。 |
| Estimated GPU budget | `0.5–1.5 GPU-hours`，不生成新 rollout 时更低。 |

结果（2026-07-14）：`autoresearch-e0-evaluator-s20260714-v1` 在 clean commit `a712587` 上完成
preflight。官方 Meta CoTracker3 repository 固定为 `82e02e8029753ad4ef13cf06be7f4fc5facdda4d`，且
local `torch.hub` entry 在 `pretrained=False` 下可构造；但官方 `scaled_offline.pth` 不在数据盘，官方
Hugging Face URL 在本机连接被拒。formal run 因此写入 `status=blocked`、`fallback_used=false`、
`uses_future_gt=false`，不回退 RAFT/KLT/任何其他 tracker。没有 tracker 输出，故 rerun、扰动、synthetic
sanity 和 human alignment 都是未执行，不能将其记为 pass 或 fail 的性能数值。该 block 禁止任何 rollout
改善结论；它不授权另换 provider。只读扫描也未发现可验证的预装 CoTracker/TAPIR/PIPS/TAPNet 等独立
tracker checkpoint 或 Python package，故没有以未核验的替代物填补该 gate。
...
fail / blocked`。P1 的 target legality hard failure 单独即可阻断 F1-R，E0 block 同时阻断 rollout 解释；
