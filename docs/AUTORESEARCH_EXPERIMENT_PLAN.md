...
更新日期：2026-07-14。当前最终决策是 **D：证据不足，先完成诊断**。F0/F1 已完成；旧 projector 下的 F1 feature-resolution gate 失败，因此 F2/F3 仍为 blocked，而不是待后台自动运行。A1 结论边界修订明确：该失败不能外推为所有连续 feature relation 都不可学习；只有 C0、P0、P1、E0 全部通过后，才允许一次只读 F1-R。所有预算均为单张 RTX 4090 24 GB，禁止 Optuna、300/800-step、数百 GB cache 和自动 push。
...
已发生的分支：A0 pass；F0 fail；旧 projector 的 F1 fail。下一步只能从 P0/P1/E0/C0 开始，不得直接跳到 F2/F3/R0。
...
| Metrics | track survival、PCK cycle consistency、identity switches、dynamic degree、velocity/acceleration/jerk、repeatability CV、与 RAFT audit 的 rank correlation、人工 occlusion spot-check。 |
| Expected failure | CoTracker3 对低纹理驾驶背景或遮挡不稳；metric variance 大；legacy source metadata 泄漏。 |
| Promotion threshold | identical-video rerun metric relative delta `<=2%`；threshold sweep rank correlation `>=0.8`；16-case manual valid `>=87.5%`；输入 provenance 证明只含 generated RGB + first-frame/query sampling。 |
| Stop condition | evaluator 读取 training tracks/future GT；repeatability 不过；acceleration/jerk 主要由 tracker jitter 决定。 |
...
## F2 — Frozen-SVD feature-head capacity（blocked by F1）
...
| Stop condition | F1-R 未通过；只 train 改善；background/collapse gate 失败；>200 updates。 |
...
## O1 — True held-out one-step capacity/locality（conditional）
...
## F3 — Future-frame-only feedback feasibility（blocked by F1/F2）
...
## R0 — Short-chain memory and credit-assignment smoke（fallback only）
