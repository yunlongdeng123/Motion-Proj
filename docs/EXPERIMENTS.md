...
- failed r1=`20260906T192156Z__lidar4d-kitti360-f4950-s0-r1`，LPIPS 权重 Python 传输停滞见 `V71-F61`；同一官方权重已预取，r2 按相同协议继续；
...
- init=each branch half optical depth；SH/opacity from its frozen optimized sidecar；geometry from M8/M26 and frozen；
- trainable=target Actor SH DC/rest/opacity only；no visual/physical geometry、trajectory、Background、other Actor gradient；
...
- main：abstract/method/experiments新增 bidirectional target-ray/surface状态、pooled四指标、64.04% Actor boundary与
  completion `14.96×` gain--tail；limitations拒绝 universal certificate/post-hoc carving。
...
- compile：TeX Live main=`8 pages/1,167,975 bytes`；supplement=`7 pages/7,221,609 bytes`。main仅既有 Table 1
