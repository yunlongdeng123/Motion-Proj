...
- 建立 `research/worldsim-v3.3-object-maintenance`，V2/M5 dirty files 保留且未混入；
- canonical P0=`20260810T171744Z__p0-source-audit-s0-r2`，terminal=`done`；r1 因缺 runner source snapshot 保留为 noncanonical `done`；
- 10 个 source 裁决为 `2 executable / 2 weights_blocked / 5 source_not_released / 1 audit_only`；
- SAM3.1 因当前 HF 未登录保持 weights-blocked，S1 使用 V3.2 SAM2.1 exact fallback；
- GS-RoadPatching 官方仓库仍只有 project-page assets；RoadPatch-Lite 必须明确标记 inspired；
- R3D2 有官方代码但无作者 exported model，不从零训练；GOR-IS 只作非商业研究 audit；
- D2/S2/S3/mixed/chunk 五个 V3.2 canonical 资产重新 hash exact，V3.2 回归 `36 passed`；
- 新增机器可执行 source auditor，定向测试 `4 passed`；P0 未训练、未模型推理、未下载大型权重；
- P0 关闭后只解锁 `WS-V33-S1-OBJECT-AWARE-GS-01`。
