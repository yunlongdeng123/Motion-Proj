...
def main():
    ...
    cutoff=int(manifest['first_timestamp_ns']); history=np.flatnonzero((times>=cutoff-1_000_000_000)&(times<cutoff))
    ...
    assert len(history)==20 and np.unique(captures).size==15 and ((requested-captures)<50_000_000).all()
