...
ap=argparse.ArgumentParser();ap.add_argument('--out',type=Path,required=True);ap.add_argument('--frames',type=int,default=8);a=ap.parse_args()
...
for i in range(a.frames):
    sd=nusc.get('sample_data',sample['data']['LIDAR_TOP']);points,sources=multisweep(sd)
    np.savez_compressed(a.out/f'input_{i:02d}.npz',points=points)
    result,packed=inference_detector(model,points)
    native=result.pred_instances_3d.to('cpu');pred,_=output_to_nusc_box(native)
    np.savez_compressed(a.out/f'prediction_{i:02d}.npz',boxes=native.bboxes_3d.tensor.numpy(),scores=native.scores_3d.numpy(),labels=native.labels_3d.numpy())
    ...
    row={'index':i,'sample_token':sample['token'],'lidar_token':sd['token'],'timestamp_us':sd['timestamp'],'points':len(points),'sweeps':sources,'eligible_gt':gt_serial,'scores':{str(t):compare(pred,gt,t) for t in [.3,.5]}}
    rows.append(row);(a.out/'summary.json').write_text(json.dumps(rows,indent=2));print('CENTERPOINT_REAL',i,{t:{m:v['matched'] for m,v in s.items()} for t,s in row['scores'].items()},'eligible',len(gt),flush=True)
    sample=nusc.get('sample',sample['next'])
reg.update(completed=True,end_unix=time.time(),detector_forwards=a.frames,failure_ledger_delta='pending_analysis')
(a.out/'registration.json').write_text(json.dumps(reg,indent=2));print('CENTERPOINT_REAL_COMPLETED',a.out,flush=True)
