...
ap=argparse.ArgumentParser();ap.add_argument('--out',type=Path,required=True);ap.add_argument('--frames',type=int,default=8);ap.add_argument('--scans-dir',type=Path);ap.add_argument('--constant-intensity',type=float);ap.add_argument('--include-real',action='store_true');a=ap.parse_args()
...
for i in range(a.frames):
    ...
    for condition in conditions:
        if a.scans_dir and condition!='real':
