...
The still-running v1 prep controller holds an earlier snapshot of the superset catalog. To prevent concurrent last-writer overwrite, replacement prep writes `/root/autodl-tmp/tmp/worldsim_v64_p4c_replacement_member_shards.json`. After both controllers finish, the two semantic member-to-shard maps are unioned once and atomically written to the persistent catalog. No hash or fingerprint is added.
