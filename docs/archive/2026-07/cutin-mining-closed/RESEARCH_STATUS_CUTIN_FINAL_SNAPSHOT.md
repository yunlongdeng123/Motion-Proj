...
> **最后更新**：2026-07-26
> **当前阶段**：`POST_V7.1 / N1_CUTIN_FINAL_REJECTED_TOO_SPARSE`
> **当前决策**：`stop_nuscenes_cutin_mining_no_n2`
> **当前路线**：[`POST_OCCGS_RESEARCH_DIRECTIONS.md`](POST_OCCGS_RESEARCH_DIRECTIONS.md)
> **当前任务**：用户开放 120 GiB 后，final strict v2 已在 clean commit `beee1de` 完成 675/675 scenes；
> 只有 1 strict PASS / 1 scene，低于冻结的 3/3 machine-readiness，终态为
> `REJECTED/stop_nuscenes_cutin_mining_too_sparse`。完整稀疏终局人工包已生成，N2 继续封闭。
> **执行授权**：final 计划已关闭；只允许用户填写/校验现有 1 primary + 3 diagnostic 审核包和维护事实源，
> 不再运行 cut-in mining、调阈值或把 ABSTAIN 提升为 PASS。
...
| 类别 | 已冻结事实 | 不可作出的推断 |
|---|---|---|
| 人工标签 | 第四轮独立裁决为 3 `TRUE_POSITIVE` / 15 `FALSE_POSITIVE`，终态 `REJECTED` | 不能把旧 parent 的 machine 候选当作新的 seed pool |
| 机器回归 | K4 evidence regression 通过、15 个 K4 human FP 的 PASS 数为 0、K4-010/011 保留 | K4 是 calibration regression，不是 prospective precision |
| prospective final | 675/675 scenes；`PASS=1`、`FAIL=200`、`ABSTAIN=1,556`；PASS 仅 1 scene | 单例不能估计总体 precision，也不能据此继续扩池 |
| 工程资源 | Resource Contract V2 通过；峰值 batch RSS `337,154,048`、cgroup current `4,556,898,304` bytes | V1 资源失败仍保留，但不再是最终拒绝原因 |
| 最终终止 | parent 为 `REJECTED/stop_nuscenes_cutin_mining_too_sparse`；`n2_authorized=false` | 不能声称 nuScenes 没有 cut-in，也不能放宽 strict v2 或自动开始下一轮 |
...
完整边界见 [`N1_CUTIN_FINAL_REPORT.md`](N1_CUTIN_FINAL_REPORT.md)。
...
- official train 685 scenes，与 26 个 calibration scenes 交集为 0；
- 8,416 transitions → 1,824 topology candidates → 328 subject body-entry passes →
  18 complete receiver-centric candidates；
- 18 candidates 覆盖 16 scenes，包含 5 parallel lane changes 与 13 receiver-branch merges；
- 6 个 30-frame receiver-matched negatives、6 个 same-actor pairs；
- positive/negative/pair/scene 四项机器 gate 全通过；
- audit population=18、items=18；58 个 immutable file hash 全部复算一致；
- blank review SHA256 为
  `15c4fc52489783383e843788c15e834125b957b232f1536f6269c3e5ba7198ae`；
- parent 只到 `AWAITING_HUMAN_REVIEW`，`human_verdict_filled=false`、`n2_authorized=false`。

完整 provenance、漏斗、两个工程失败、材料 QA 和审核命令见
...
| N1-CUTIN 工程 | lightweight map reader 与官方 arcline reference 一致；单 location cache；流式 metadata；page-cache release；batch=32；30 项测试 |
| N1-CUTIN formal | commit `f13eb0f`；config `5b1634e3…46c5`；data `79651f91…d7e9`；official train 685 scenes；18 pos / 16 scenes / 6 neg / 6 pairs；machine gate 全通过 |
| N1-CUTIN audit | 18/18 空白盲审材料；58 immutable files；set SHA `5379059a…da30`；唯一 `AWAITING_HUMAN_REVIEW`；N2=false |
...
| `N1-EVENT-CUTIN` (train, 第四版历史 parent) | receiver-centric 车身进入 + 独立接收车 | machine：8 pos / 4 neg / 4 pair / 5 scenes；human：≥6 TP、precision≥0.8、Wilson≥0.5 | **HUMAN REJECTED**：第四轮独立裁决 3 TP / 15 FP；不得把旧 18 个 machine candidates 当 seed pool |
| `N1-EVENT-CUTIN-FINAL` (train, final v2) | strict v2 + 流式 mining + 分进程 audit | clean git、K4 regression、资源合同和 675-scene formal；machine ≥3 candidates / ≥3 scenes | **REJECTED TOO SPARSE**：675/675 完成，但仅 1 PASS / 1 scene；K4/raw/resource 通过 |
...
4. 已完成：第四轮独立裁决为 3 TP / 15 FP，旧 parent 已 `REJECTED`；历史 formal 和审核包保留为 calibration/
   provenance，不再等待填写。
5. 已完成：Resource Contract V1 的失败与独立拒绝裁决保持不变；用户开放 120 GiB 后，Resource Contract V2
   在 clean commit `beee1de` 完成 675/675 scenes，最终因 1 PASS / 1 scene 低于 3/3 而 `REJECTED`。
6. 已完成：P8 事实源、最终报告和完整稀疏终局人工包；不再启动 cut-in mining。现有 4 条只可人工复核，
   不能改变 parent 终态或形成正式 seed pool。
7. N2 保持 fail closed；不得事后挑 scene、放宽阈值、复用 source rear、缩短 negative、允许 event overlap，
   或因第四版存在候选、K4 回归或 development smoke 就自动启动 N2。
