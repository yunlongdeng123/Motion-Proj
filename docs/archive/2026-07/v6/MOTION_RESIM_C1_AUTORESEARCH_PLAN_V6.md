...
> **创建 / 修订日期**：2026-07-19（2026-07-20：DAG 收口与归档链接）
...
**RF-17 重开机制（仍属 C1B-01，不改门槛/scene）**：v2 的四类 ridge 直接分类在真实视频上 turn/class 不可辨识。允许的唯一续跑是预注册
`local-ego-motion-proxy-v2-kinematic-lateral`：在同一 RAFT+affine 特征上增加显式 yaw/分段侧向特征，分别 ridge
回归位移与侧向，再仅在 fit split 上按冻结网格选择 `stationary_disp_m` / `lateral_turn_m`，用运动学规则导出
class。禁止降阈值、换 scene、偷看生成 future。v1/v2 失败证据原样保留。

该 proxy 统一称为 **local ego-motion proxy**，禁止称作 ReSim IDM、Trajectory Difference 或 ADE。

**执行结果（2026-07-19）**：v2 四类 ridge `blocked`（`RF-17`）。v3
`local-ego-motion-proxy-v2-kinematic-lateral` 同门槛通过：held-out moving BA `0.778`、turn-sign
`0.750`、位移 Spearman `0.953`；状态 `done`，解锁 `C1B-02`。证据
`/root/autodl-tmp/runs/resim_c1_v6/C1B-01/resim-c1b01-proxy-s20260719-v3`。
...
**执行结果（2026-07-19）**：`rejected`（H1 / `RF-18`）。v1 因磁盘峰值预估过高 engineering fail；v2 完成
10×2 L1 采样，future/history/quality/stationary 多数通过，但 E wins 仅 `3/8`、median improvement
`-0.107`。证据 `/root/autodl-tmp/runs/resim_c1_v6/C1B-02/resim-c1b02-screen-s20260719-v2`（`REJECTED`）。
下游 `C1B-03`/C1P/C1S 全部 `not_run`。终报见 [`C1_V6_FINAL_REPORT.md`](C1_V6_FINAL_REPORT.md)。
...
| C1B-01 | 10-context 冻结与 proxy calibration | `done` | C1B-00 | C1B-02 |
| C1B-02 | E vs F action screen | `rejected` | C1B-01 | —（H1 stop） |
| C1B-03 | action 人工盲审 | `not_run` | C1B-02 | —（H1 stop） |
| C1P-00 | 阈值、split、预算预注册 | `not_run` | C1B-03 | —（H1 stop） |
| C1P-01 | N=4→条件式 N=8 候选池 | `not_run` | C1P-00 | —（H1 stop） |
| C1P-02 | UPO 机器支持审计 | `not_run` | C1P-01 | —（H1 stop） |
| C1P-03 | preference 人工盲审 | `not_run` | C1P-02 | —（H1 stop） |
| C1S-00 | 新增 adapter 审计与一步训练 | `not_run` | C1P-03 | —（H1 stop） |
| C1S-01 | 单对 capacity/locality | `not_run` | C1S-00 | —（H1 stop） |
| C1S-02 | 多对 scene-disjoint pilot | `not_run` | C1S-01 | —（H1 stop） |
| C1S-03 | 最终 held-out 人工盲审 | `not_run` | C1S-02 | —（H1 stop） |
| C1D-00 | 汇总与 go/no-go | `done` | H1 终止 | 终报已写；计划关闭 |
...
# 12. 计划关闭说明（2026-07-19）

本计划已按 §9 停止规则在 H1 关闭：`C1B-02 rejected`（`RF-18`）。`C1B-03` 及全部 C1P/C1S 为
`not_run`；`C1D-00` 交付物为 [`C1_V6_FINAL_REPORT.md`](C1_V6_FINAL_REPORT.md)。新 agent **不得**
把本文当作可续跑任务队列；若继续，必须由用户授权的下一份预注册计划重新定义问题结构与门禁。
