...
> 整理前的阶段记录，保留原顺序与文字。正文中的“当前”、待办和电源权限只属于当时，不构成执行授权。当前状态只见 [RESEARCH_STATUS](../../../RESEARCH_STATUS.md)。
...
仓库资产清理完成：241 个大表/渲染场景/编译包已从当前分支移出，原值完整存于仓库外归档并有本地副本；核心代码、文档、报告内嵌图和失败查询索引保留，原始 runs 不删除。见 [归档清单与恢复方法](../worldsim_v74_closeout_20260913/README.md)。本次没有新实验；failure_ledger_delta 仍为 F11（追加资产位置），F08–F10 数值不变。
...
| task/run | 最终完成情况 | 证据 |
|---|---|---|
| A-REAL / probe43-assets-r2-ordered | 215/215 | final/a_assets、evaluation/a_wex |
| B-REAL / probe43-assets-r1-eliminated | 172/172 | final/b_assets、evaluation/b_rif |
| C-REAL / probe43-assets-r1 | 215/215 | final/c_assets、evaluation/c_dcs |
| C-FIT-TRAIN / pricing-teacher-r1 | 本域FULL/NO_DEMAND训练完成 | final/c_training |
| GEOMETRIC-MECHANISMS / opaque-scenes80-r2-numerics | 1120/1120 | final/geometric_mechanisms、mechanism_summary.json |
| C-EXPANDED-DICTIONARY / geometry80-union-r1 | 80/80，所有MIP gap=0 | final/expanded_dictionary |
| NKSR-REFERENCE / probe43-empty-native-r3 | 35原成功复用+8修复；一个原空面保留 | final/nksr_probe，原r1/r2错误单列 |
...
上表 run 前缀日期分别保存在 [完整run清单](../../../autoresearch/worldsim_v74/final/decision.json)，不以简称替代唯一run ID。最终报告含架构图、全部真实/机制解释与成本；[完整质量表](../../../autoresearch/worldsim_v74/final/REAL_RESULTS.md) 和 [配对日志原值（已归档）](../worldsim_v74_closeout_20260913/README.md#asset-223) 为权威数值。大资产仍驻留runs路径。
...
证据统一位于 `docs/autoresearch/worldsim_v74/p0/`。[V73 完整台账](../pre-v74/V73_EXPERIMENTS.md)。

追加精简完成：两批合计移出 307 个批量/生成资产，原始文件共 322.49 MB；两个完整包均已保存远端和本地副本。V7.4 核心图保留，旧论文与 V7.3 非架构图改为按需恢复。 [追加归档](../worldsim_v74_closeout_20260913/LEGACY_MEDIA.md)。
