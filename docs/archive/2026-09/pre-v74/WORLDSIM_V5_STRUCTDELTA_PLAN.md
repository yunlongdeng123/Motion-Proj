...
## M2 正式收口补记（2026-08-14）

- `WS-V5-M2-GEOMETRY-FIRST-REPAIR-01=rejected`。r014 G5 三相机 scaffold 在 frozen scene0471 model proxy 上通过相对门（raw/post 改善=`15/22`、`19/22`），但 absolute-safe 仅 `1/22`、`0/22`，未选择 method arm。
- r015 canonical closeout 绑定 `8 completed + 4 blocked`，结论=`m2_rejected_no_absolute_geometry_safe_candidate`；router、validation、parameter search、neural automatic unlock 全为 false。
...
- 证据入口：[`WS_V5_M2_GEOMETRY_FIRST_DEVELOPMENT.md`](WS_V5_M2_GEOMETRY_FIRST_DEVELOPMENT.md) 与 [`archive/2026-08/worldsim-v5-m2/M2_R012_R015_CROSS_VIEW_CLOSEOUT_METADATA.json`](../../2026-08/worldsim-v5-m2/M2_R012_R015_CROSS_VIEW_CLOSEOUT_METADATA.json)。
