...
截至2026-09-08 19:05 UTC，R10/R11/R14均已完成；R14结果见下节。R12仍在训练，三角与整个V7.3尚未完成。

![可训练几何解码、三维查询与统一物理表面读出](../../../../paper_v73/figures/architecture_components.png)

## R14完成：有限宽束目标未带来稳定原生物理增益（2026-09-08 19:05 UTC）
...
`WS-V73-M2-GLOBAL-ACTOR-01/20260908T100000Z__population-native-full-track-beam-range-s7304-r14`，code bf04ef32。11130次呈现、10550次实际更新、零恢复；580次no_surface_gradient中420次为无相机呈现，另160次为有相机但没有有效原生梯度，保留LiDAR读出且不虚报优化。与R11相同实际更新总数不代表每步随机状态或计算量完全相同。

全部32654562个DPT参数可训练、query冻结；native_project相对M1r3最大变化.004810881。wall29683.499341s（8.245h，含最终评价）、allocated峰值2.361161GiB、RSS34.697498GiB；744份冻结前缀/每窗口24视图保留，latest.pt为398679588字节。生成最终表面与训练过程已保存；未训练上层聚合器。

### 完整开发结果

| 方法 | hit | early | miss | free（m） | 单向distance（m） | recall@.2m |
|---|---:|---:|---:|---:|---:|---:|
| 原生共同初始化 | 0.227771 | 0.091551 | 0.573543 | 0.150304 | 0.162474 | 0.781294 |
...
全部75开发Actor/5日志保留23个无owned返回对象与8个空表面。每Actor按真实束/点加权，之后日志内Actor平均、独立日志等权；free用全部near-box原束，命中结果含missing。单向target→surface距离不等于完整表面精度/对称Chamfer。R14开发native fallback为7，对照R11为8；不可把fallback数本身解释为几何优劣。

### R14−R11：只改变原生路径的free目标

| 指标 | R14−对照 | 95%日志配对区间 | 改善日志数 |
|---|---:|---:|---:|
| hit，百分点 | +2.945 | [-0.370, +7.506] | 2/5 |
| early，百分点 | +2.350 | [-0.769, +6.135] | 2/5 |
| miss，百分点 | -2.575 | [-8.514, +2.860] | 2/5 |
| free，m | +0.001454 | [-0.022062, +0.026683] | 3/5 |
| 单向distance，m | -0.001558 | [-0.017355, +0.009775] | 2/5 |
| recall，百分点 | +2.049 | [-1.283, +5.725] | 3/5 |

同一M1r3初始化、seed7304、旧cohort、full_track标签、native1/free.5/event0与30轮，free由hard range改为beam_tube_range（width.03m/resolution32）。hard free均值反而+.001454m，区间跨0；hit虽均值提高，但仅2/5日志改善、2下降、1相同。不能照搬LiDAR分支R8−R7的free改善来推断原生分支有效；也不能把跨0写成两种目标完全等价。

### R14−R8：同beam目标的原生适配与LiDAR生成控制

| 指标 | R14−对照 | 95%日志配对区间 | 改善日志数 |
|---|---:|---:|---:|
| hit，百分点 | -10.446 | [-17.429, -5.270] | 0/5 |
| early，百分点 | +4.402 | [-0.601, +8.573] | 1/5 |
| miss，百分点 | +0.686 | [-2.764, +5.746] | 3/5 |
| free，m | +0.132099 | [+0.053660, +0.203728] | 1/5 |
| 单向distance，m | -0.062607 | [-0.156425, +0.005458] | 4/5 |
| recall，百分点 | +5.253 | [+3.180, +8.258] | 5/5 |

召回增加在5日志一致，hit下降也在5日志一致；free差值+.132099m，区间[+.053660,+.203728]m。这个通路比较包含视觉预训练、DPT适配、支持融合和PCA参数化等差异，不是单独视觉特征或attention的因果实验。R14不是当前更好的物理表面重建候选，也不因此否定全部视觉几何适配。

[R14已保存的配对区间（旧图已归档）](../../../archives/worldsim_v74_closeout_20260913/LEGACY_MEDIA.md#legacy-007)

上述为一次seed7304/10000次独立日志bootstrap的保存结果，图只读区间，没有额外推理或重采样。开发5日志仍用于研究选择，新20日志质量未读。
...
|---|---:|---:|
| 实际更新 / 371呈现 | 352 | 351 |
| native有监督呈现 / 测量点 | 346 / 173972 | 346 / 173972 |
| native测量加权Huber，m | .805201 | .469110 |
| native有监督Actor平均Huber，m | 3.470453 | 1.325106 |
| 采样target→surface，m | .318582 | .242145 |
| 采样hard free，m | .134530 | .152671 |
| 实际beam free目标，m | .135196 | .151870 |
| 有相机但无框内native支持 | 15 | 8 |
| DPT裁剪前梯度范数中位数 | 394.467438 | 206.294205 |

native与coverage拟合下降，采样free却未同步下降。采样和表面随步变化，这些是过程证据，不是固定观测的配对结论。不能从总损失下降推断物理目标已经改善，亦不能直接将冲突唯一归为loss权重。

[R14实际训练过程（旧图已归档）](../../../archives/worldsim_v74_closeout_20260913/LEGACY_MEDIA.md#legacy-008)

相对共同初始化，开发hit−2.221pp、95%[−4.761,+1.236]pp，free+.016115m、[−.003114,+.044810]m；6项指标区间全部跨0，不能宣称稳定改善，也不称显著hit退化。FIT距离−.011413m、[−.020245,−.003317]m，recall+3.213pp、[+.571,+5.978]pp；FIT full_track含评价时刻的训练标签，这些不是泛化证据，FIT hit+.660pp区间仍跨0。

移动开发子集依旧只有9 Actor/2日志，其中3无owned、1空表面；R14 hit9.343%、miss75.734%、recall87.802%。其中一个日志仅1条owned返回的稀疏边界不变，不能用两日志bootstrap支持稳健动态优势，也不事后删除该日志。

### 决策与证据

`failure_ledger_delta=update V73-F09 evidence; no new failure ID`。R11原负结果继续有效，R14没有建立稳定的原生物理改善；F02/F03/F04保持。此结果支持继续区分上游表示/支持组织、表面参数化与目标语义，不能以native-only作为自动退路，也不能说只有Query存在coverage/physics冲突。复用已核实的VGGT、LoRA3D/CAPA及候选参数化源码依据；不发起loss网格或重跑旧对照。

19:01 UTC R12/PID81766第17轮、两组梯度均为正，cgroup oom/oom_kill=0。继续原配置直到完成R12−R10、R12−R14；若Query冲突持续，按计划revision6优先Q-v2表面参数化，近边界监督/局部对应/必要的上层表示适配分别研究。资源正常，30分钟跟进ACTIVE，完成不关机。

证据：`docs/autoresearch/worldsim_v73/m2/global/population_native_r14_{summary,manifest,analysis,training}.json`、两张R14图；完整训练日志、checkpoint和表面留在原run。结果汇总与绘图复用现有脚本，执行入口`scripts/summarize_worldsim_v73_native_r14.sh`；两图已视觉核对，不另加smoke/回归。本文保留以下较早R10/R11结果及历史调度语境，当前状态以文首和三本台账为准。
...
[R10真实训练过程（旧图已归档）](../../../archives/worldsim_v74_closeout_20260913/LEGACY_MEDIA.md#legacy-004)
...
[R10相对三项对照的已保存配对区间（旧图已归档）](../../../archives/worldsim_v74_closeout_20260913/LEGACY_MEDIA.md#legacy-003)
