...
> **当前状态**：`PA0-REVIEW-00 done`、`PA0-SCENE-SPLIT-01 done`、`PA1-HORIZON-01 done`、`PA1-BRANCH-02 done`。PA1 v5 以 machine 3/4 condition、6/8 groups 和人工 8/8 `same_scene` 解锁 PA2。PA2 smoke v1–v5 已清理 deterministic CUDA、device、coverage-check 与 review-material 工程问题。首批 formal `autoresearch-pa2-pair-s20260715-v1` 以 clean `13f21ce` 完成 64 condition / 512 candidate，但冻结门禁只得到 31 个 valid global pair、30 个含 non-tie local segment 的 condition，低于 48/48，故 machine-blocked 且未生成 review。选择器无漏选，主要失败来自 track coverage/candidate feasibility；不得降阈值。按 30/64 命中率的 Beta-binomial 预测，append-only 56 条使新增至少 18 条 local-valid 的预测概率约 96.1%，因此冻结追加排序 `[64,120)` 的 56 个不重叠 condition，再与前 64 条无 GPU 合并；不重生成已有候选。扩量 aggregate 通过前仍不授权 DPO/AWR、训练或切换双卡。
...
2. 若任一 condition 的全部 sibling 不可区分，按 `small -> medium -> large` 仅推进下一档；
3. 若结构失配，再改 fork；
4. 最多 4 conditions；
5. 找到首个满足门槛的最小配置后冻结。
...
PA2 随机抽取至少 48 个 pair，分 strata 和 margin 分桶。
...
如果 48 个 pair 数量不足以形成 decisive verdict，扩大人工 review，而不是放宽阈值。
...
| PA1-BRANCH-02 | done（v5 machine + human pass） | 结构对齐候选 pilot | 机器 3/4 condition、6/8 group 通过；8/8 结构盲审为 `same_scene` | `rejected`，不退回 independent seed 主线 |
| PA2-PAIR-03 | running / append-only expansion | 64 + 56 condition pair legality | ≥48 有效 condition，48-pair 人审通过 | `rejected` |
...
v4 完成 20 candidate + CoTracker；scene-0126 future-VAE ratio 升至 `0.0431–0.0493`，仍全部低于
`0.05`，而所有 first-frame、finite、dynamic-degree、survival、correspondence、symmetry 和 quality
check 均通过；scene-1102 的 Base coverage 仍不合格。因此 v5 config 为
`configs/diagnostics/physics_dpo_branch_v5.yaml`，只执行最后一档 `large=0.04`。v5 若仍不能让
scene-0126 形成有效 candidates，禁止新增 strength 或扩大数据；必须按既定规则考察 fork `0.8` 或
PA1 `rejected` 的路径。

v5 已以 clean `0e11515` 完成相同的 4 condition × 5 candidate 和 20 条独立 CoTracker score。所有
Base rerun、official callback shared-prefix、finite、first-frame、quality、dynamic-degree、survival、
correspondence 和 antithetic symmetry 均通过；所有 condition 的 sibling future-distance 均不再属于
不可区分组。scene-1102 的 Base track coverage 仍低于硬门槛，因此它及两个 group 保持无效；其余
3/4 condition、6/8 groups 通过预注册 machine gate。8 个 group 的 provisional direction 均为
`abstain`，该 diagnostic 不是 pair label 或训练 scorer。run 写出 `MACHINE_COMPLETE`、8 个随机列
顺序 panel、`reviews.template.jsonl` 和 `REVIEW_README.md`，状态为 `awaiting_reviews`，尚无
`COMPLETE`。

人工已独立完成 8 个结构对齐盲审：8/8 为 `same_scene`，没有 `different_composition`、`invalid` 或
`uncertain`。原聚合调用首先因实现把 `machine_summary.json["machine"]["machine_pass"]` 错读为顶层字段而
拒绝；该 schema 读取缺陷由 clean `002b616` 修复并有回归测试覆盖。修复不改变候选、阈值或人工 verdict。
对同一 v5 artifact 的无 GPU aggregate 随后得到 `same_scene_rate=1.0`（阈值 `0.875`）、`bad=0`，写出
`COMPLETE`，`status=done`，并解锁 `PA2-PAIR-03`。

若任一 condition 的全部 sibling 与 Base 不可区分，下一次只能按阶梯升一档；若发生 structure mismatch，
下一次只能将 fork 调到 0.8；不得并行网格搜索，也不得跳入 re-noise、DPO/AWR、训练或双卡。

## PA2-PAIR-03

### 2026-07-15 规模修订（用户指令）

原始 32-condition / 30-pair 配置对后续证据量不足。保持所有 feasibility、margin、anti-collapse 和
blind-review 阈值不变，仅在单卡可承担范围内扩大分母：64 个 `preference_train` condition、至少 48 个
有效 condition / global pair，以及至少 48 个分层盲审 pair。不得因为放量降低 margin、把 invalid/abstain
改成 decisive，或直接进入训练。

固定：

```text
64 preference_train conditions
...
- 至少 48 个可用 global pair；
- 至少 48 个 condition 有非 tie local segment；
- 至少 48 个有效 pair 的两阶段盲审；
- 记录 abstain rate；
...
### 已登记的 PA2 formal v1 与 append-only 扩量

`autoresearch-pa2-pair-s20260715-v1` 完成 64 conditions、320 core candidates、192 audit candidates 和
512/512 P-UNC score。三 constructor 数量严格为 P0:P1:P2 = 64:128:64，schema 与 provenance 通过；
但 raw P1 只有 43/128 decisive，按每 condition 最多一条选择后为 31/64 valid global、30/64 有 non-tie
local segment。非 decisive raw P1 的主要原因是 42 个 `candidate_infeasible`、16 个
`noninferiority_conflict`、16 个 margin 不足和 11 个无 finite projection energy；选择器没有漏选任何
已有 decisive group。run 写 `COMPLETE`、`status=blocked`，未调用 CoTracker review、未训练。

不得降低 coverage、margin、non-inferiority 或 minimum-valid 阈值。扩量只追加冻结排序中的 `[64,120)`：
56 个新 condition、不同 generation/direction seed 区间、相同 scorer fingerprint。首批 64 条 artifact
全部只读复用。append run 自身只验证 schema/constructor 完整性；随后 merge 必须验证 source COMPLETE
哈希、scene 无交集、candidate/pair ID 唯一和 scorer fingerprint 相同，再按 120 条合并结果执行原 48/48
门禁。只有 merge machine-pass 才生成 48-case 两阶段人审。
...
| 7 月 17–20 日 | PA2 64-condition pair 构建与至少 48-pair 人审 |
...
→ PA1-HORIZON
→ PA1-BRANCH
→ PA2 pair legality + 至少 48-pair blind review
...
PA0 人审、唯一 scene split、PA1 horizon profile 与 PA1 branch human aggregate 已完成：
...
→ autoresearch-pa1-branch-s20260715-v5
→ PA2-PAIR-03
```
...
`autoresearch-pa1-branch-s20260715-v1` 已作为 manual-continuation exact guard 的失败证据保留；v2 已作为 schema 数值容差失败证据保留；v3/v4 均已完整完成但 machine-blocked。v5 的候选生成 provenance 为 clean `0e11515`，其 machine gate 为 3/4 condition、6/8 groups；8 条独立结构盲审均为 `same_scene`。聚合器的嵌套 `machine_pass` schema 读取缺陷由 `002b616` 修复后，v5 已写入 `COMPLETE`、`status=done`、`next_gate=PA2-PAIR-03`。PA1 只证明 common-prefix sibling 的结构对齐可行，不生成 physics winner 或训练标签。

当前唯一可执行动作是：实现并 preflight PA2 的 64-condition 三种 pair constructor，生成候选与诊断、按
冻结规则构造至多一条 pair / condition，再交付完整的 48-pair 两阶段盲审提示词。PA2 aggregate 通过前禁止：

- DPO/AWR 实现；
- 训练；
- 切双卡；
- 用更多 GPU 替代 pair 合法性门槛。
...
clean `57987b0` horizon profile 与 clean `0e11515` PA1 candidate generation
materialized scene split、schema、PA1 profile 与 v5 human aggregate
next gate：PA2-PAIR-03；先验证 pair 合法性，再允许 PA3 代数或任何训练
```
