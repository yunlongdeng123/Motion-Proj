...
| PA1-BRANCH-02 | awaiting_reviews（v5 machine pass） | 结构对齐候选 pilot | 完成 8 个盲审并通过 aggregate | `rejected`，不退回 independent seed 主线 |
...
下一步只允许人工完成这 8 个结构对齐盲审（`same_scene`、`different_composition`、`invalid` 或
`uncertain`），再以同一 run 的 `--aggregate-only` 汇总。禁止由自动 scorer 或 Codex 填写 verdict；
aggregate 通过前不得进入 PA2、DPO/AWR、训练或双卡。
...
`autoresearch-pa1-branch-s20260715-v1` 已作为 manual-continuation exact guard 的失败证据保留；v2 已作为 schema 数值容差失败证据保留；v3/v4 均已完整完成但 machine-blocked。v5 已完成 machine gate（3/4 condition、6/8 groups），但尚未完成人审，故也不得进入 pair 或训练。当前唯一可执行动作为无 GPU 的人工盲审：

```text
复制 reviews.template.jsonl 为 reviews.jsonl
完成 8 个 panels/*.mp4 的结构对齐 verdict
对同一 run 执行 --aggregate-only
```

在 PA1-BRANCH-02 人审 aggregate 通过前禁止：
