# PA1-BRANCH-02 v5：common-prefix sibling pilot
...
- formal config：`configs/diagnostics/physics_dpo_branch_v5.yaml`（继承 v4 的全部研究参数，只将预注册强度从 medium 推进到 large）。
...
strength = large = rho 0.04 × sigma_fork
...
v1 的手动 scheduler continuation 在首个 condition 未能 exact 重构 official Base trace，已保留为失败工程证据；未写入候选、score、pair 或训练结论。v2 已用上述 callback 生成全部四个 condition，随后 schema 将 permutation 的 float32 RMS 归约尾差误判为四条 sibling 不等范数；因此未进入 score、pair、panel 或训练。v3 仅令 schema 使用与 generator 相同的 `1e-7` 相对数值容差，并完整完成 20 候选的独立评分：scene-0126 的两组 sibling future-VAE ratio 均为 0.0376–0.0411，低于 0.05 下界；scene-1102 的 Base track coverage 为 0.3055，低于 0.50，因而不可用于本轮有效 condition。v4 将 `rho` 推进到 0.02，scene-0126 ratio 升至 0.0431–0.0493 但仍未跨过 0.05；没有 structure mismatch、质量或 correspondence failure。故 v5 只执行最后一档 `rho=0.04`，仍保留同一四 condition、fork、family、阈值与审查规则。
...
- 任一 condition 的全部 sibling 均不可区分：按 `small -> medium -> large` 仅推进下一档；
