...
官方 SplatAD 已与冻结 TransFuser、PDM 控制器和车辆模型串联，raw／median 两条点云路径各完成一次实际 4 秒新位姿传感器闭环。固定第 8,000 步 checkpoint 另完成 48 组同位姿模态对照；本轮共 64 次策略与执行预测。首场景完整拟合的记录快照为第 18,800／30,001 步，仍在继续。试跑不能替代完整预算的研究验证，详见 [原生闭环试跑报告](WORLDSIM_SIMULATION_NATIVE_CLOSED_LOOP_PILOT.md)。
...
上轮六日志和覆盖恢复实验没有确认普遍 phantom 导致严重事故。真实基线距路锥仅约 1.6 cm，主要差距由相机与 LiDAR 的观测范围差异解释。两个 Pi3X 残余的有限因果审计已经完成，普通右前路面网格修复均未恢复，不晋级主方法动机。新位姿 RGB＋LiDAR 循环已完成有限试跑；目前主要的同位姿退化经 RGB 通道传递，LiDAR 单独替换影响较小。完整拟合后的验证、自然几何干预及可重复的物理后果仍待完成。
