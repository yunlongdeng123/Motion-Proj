...
Relative to the frozen unit categorical baseline, M39 changes all/hazard/clear early returns by +0.229/+0.542/-0.036 points and hit recall by +5.888/+6.302/+5.537 points. The all-early and worst-stratum gates fail while hit retention passes, so only 1/3 preregistered decisions pass and the verdict is `m39_development_only_cross_sensor_rejected` (`V71-F43`). A subsequent read-only writing audit found that `_evaluate_bundle` overwrites the descriptive M8 literal baseline early/hit counts with unit categorical counts (`V71-F52`). Those mixed-operator differences do not establish geometry-transfer failure. The descriptive Chamfer pair (+0.162 mm) and the matched M39 categorical comparison remain valid; the formal result establishes a hit--early transfer trade-off without isolating its geometric cause. The immutable run is retained, and no target re-evaluation or runner change is part of the paper revision.
