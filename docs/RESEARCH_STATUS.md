...
当前提案 `WS-V75-OMNI-REVIEW-02 / 20260923-proposal-r6-human-feedback`：[当前待审说明及architecture](autoresearch/worldsim_v75/downstream_bench/review-20260923/human-feedback-r6.md)，[r5详细修订](autoresearch/worldsim_v75/downstream_bench/review-20260923/human-feedback-r5.md)。DriverQ官方源码已放远端，现有11个完整10Hz场景接入其SQLite场景/位姿/几何可见性查询；没有将官方val数据库或未运行的地图事件检测冒充train筛选。第二轮按用户截图修订9例：SPEED-ACTOR-02改框黑车 #5，源窗后移3秒并在黑车可见后加速；LATERAL-EGO-02由危险右移改为左移并加俯视绿框；LATERAL-ACTOR-03换掉右移出路的骑行者；REMOVE-02至06换目标/视角，尤其纠正施工设备 #20 错框为 #28；INSERT-06换掉静止场景。r6追加把短暂入画的摩托车移除例换成持续可见的路口巴士。地图/车身几何检查只筛候选，未替代遮挡、锥桶及规划价值人工审核。24段原始参考仍为100帧10Hz、10秒，生成与评分留空；全部`approval.status=pending`、`inference_allowed=false`，0新模型调用、0新AI视频评分。
...
下一步：等待用户review新版24例10秒HTML及反事实，按明确批准的case/配置再启动OmniDreams推理。本地`outputs/cfbench-20260923/omnidreams-case-review/index.html`，远端`/root/autodl-tmp/runs/worldsim_v75/WS-V75-OMNI-REVIEW-02/20260923-proposal-r6-human-feedback/index.html`。原始视频严格100帧10Hz、10秒；计划模型原生301帧30Hz，展示取前300帧保证10秒，不使用循环或减速播放。ReSim停止记录仍在旧run的`resim/user-stop.json`，产物未删除。自动跟进`v75-24-case`已在应用中不存在，未重建；其他方法新实验暂停，不得定时自动绕过人工确认。
