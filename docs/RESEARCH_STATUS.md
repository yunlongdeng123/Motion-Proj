...
下一步冻结一个小型 reference-only 显著性队列，按输入投影面积/深度分层而不读取模型输出选点，估计 **G1 reference editability** 的覆盖范围。只有 G1 通过的来源才调用重建并进入 G2。当前不追加这三个来源的seed、事件时刻、编辑强度或闭环反馈，不同时铺多模型或多策略实验。
