...
> **当前状态**：`done`
> **当前任务**：`RP-D0-08 done`；V5 已收口；`STORAGE-RETENTION-20260719` 已登记但尚未执行数据删除
> **最终决策**：`C1`；下一阶段以 ReSim `exp0_no_carla` 做单卡 Base/action feasibility
> **硬件**：单张 RTX 4090 24 GB；只有后续 C1-CAP 通过上游门禁后才需要停机配置双卡
...
C1 尚未下载、尚未通过 24 GB 单卡 smoke、尚未训练。V5/C0 在 2026-07-18 记录的 42 GB 剩余空间不足以在
约 34.4 GB 最小下载后保持 30 GB 安全线；2026-07-19 已在
[`ARTIFACT_RETENTION.md`](ARTIFACT_RETENTION.md) 冻结历史载荷清理清单，第一笔文档提交不删除数据，实际
回收结果将在清理后回填。存储维护不构成 C1-BOOT 授权。最终执行总结见
[`ROUTE_PIVOT_FINAL_REPORT.md`](ROUTE_PIVOT_FINAL_REPORT.md)。
...
当前不授权自动下载 ReSim/VISTA/OpenDWM/MagicDrive-V2 权重，不授权新环境安装、推理、LoRA、DPO/AWR 或
双卡切换。下一阶段必须以新计划预注册 C1-BOOT；单卡 Base/action gate 通过前不讨论训练。
...
```text
C1-BOOT：ReSim 单卡 Base/action feasibility
    ↓ pass
C1-PREF：action sibling 因果审计 + rollout sibling 安全 preference support
    ↓ machine + human pass
C1-CAP：2×4090 dense safeguarded partial-order LoRA capacity
```

开始 C1-BOOT 前必须：

1. 把 `/root/autodl-tmp` 可用空间提高到 70–80 GB；
2. 固定 ReSim `exp0_no_carla` 名称与文件 fingerprint，不借用未公开 CARLA checkpoint 的结论；
3. 禁止 future actor boxes/tracks 进入条件和自由 rollout evaluator；
4. 预注册 24 GB OOM 缩减顺序、action responsiveness、actor survival 与停止条件。

C1-BOOT 仍只需单张 4090。只有 C1-PREF 证明存在足量安全 support 后，才由用户停机配置第二张卡进入
C1-CAP；任何上游 fail 都直接停止，不靠扩卡、降 safeguard 或增加 candidate 数挽救。

## 9. 存储维护状态

`STORAGE-RETENTION-20260719` 是基础设施维护，不是新研究任务。清理前基线为 clean commit `73bd1a4`、
128G 数据盘、可用 `45,049,835,520` 字节；冻结清单的逻辑大小为 `46,525,314,508` 字节。第一笔提交只固化
逐路径删除范围和保护哈希，当前仍不得下载 ReSim、运行 C1-BOOT、推理或训练。
...
计划删除的 SVD 权重、历史 checkpoint、candidate 视频、adapter 和 feature tensor 只是二进制载荷；其清理不改变
`RF-01`、`RF-06`、`RF-09`–`RF-12`、`RF-14`、`RF-15` 的负结论、适用范围或重开条件。实际回收量、
清理后 `df/du`、保护哈希和测试结果将在第二笔文档提交中回填。
