...
当前提案 `WS-V75-OMNI-REVIEW-02 / 20260923-proposal-r4-driverq-review`：[24例待审说明及architecture](autoresearch/worldsim_v75/downstream_bench/review-20260923/driverq-review.md)。DriverQ官方源码已放远端，现有11个完整10Hz场景接入其SQLite场景/位姿/几何可见性查询；没有将官方val数据库或未运行的地图事件检测冒充train筛选。按用户反馈更新8例：SPEED-ACTOR-02改为scene-0998前视汽车（不再与第3例同一巴士），SPEED-ACTOR-03原慢速巴士改1.5×加速；LATERAL-EGO-02静止原片换为scene-0471行驶片段；LATERAL-ACTOR-01换为scene-0535后视持续可见汽车；6个横移例统一明确“+左、−右”。发现旧ego局部+Y实际为前进轴，已为新版候选和后续输入适配器定义正确横移坐标；LATERAL-EGO-01也重选为可行驶区内左移场景。其余16例逐字段保持上版，旧R3/R4与人工意见保留。24段原始参考仍为100帧10Hz、10秒，生成与评分留空；全部`approval.status=pending`、`inference_allowed=false`，0新模型调用、0新AI视频评分。
...
下一步：等待用户review新版24例10秒HTML及反事实，按明确批准的case/配置再启动OmniDreams推理。本地`outputs/cfbench-20260923/omnidreams-case-review/index.html`，远端`/root/autodl-tmp/runs/worldsim_v75/WS-V75-OMNI-REVIEW-02/20260923-proposal-r4-driverq-review/index.html`。原始视频严格100帧10Hz、10秒；计划模型原生301帧30Hz，展示取前300帧保证10秒，不使用循环或减速播放。ReSim停止记录仍在旧run的`resim/user-stop.json`，产物未删除。自动跟进`v75-24-case`已在应用中不存在，未重建；其他方法新实验暂停，不得定时自动绕过人工确认。
