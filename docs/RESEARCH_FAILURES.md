...
- `V5-F13`：fresh development cohort 冻结后，8 个 scene 在现有 DriveStudio processed root 中为 `0/8`，所需三前向相机+LIDAR keyframes 在 V4 selective raw root 中为 `0/1280`。这是 selective extraction/preprocess 工程前置，不是 M1 质量失败；不得退回 V4 scenes、替换 frozen cohort、提前读 validation，或为省事解压全部 294 GB blobs。应先建立 member→archive 索引，只提取 frozen development 所需文件并保留完整 denominator。
