...
未来减速干预随后通过了两个来源的 reference editability gate。被测模型始终是 OmniDreams；上游重建只提供状态误差样本。大误差来源中，OmniDreams 在 reference/depth-shift/depth+shape 三种输入下的配对响应误差为 `2.11/55.61/32.08px`；小误差独立对照为 `3.59/1.65/2.84px`。当前定义因而具体化为：**测量 OmniDreams 会把哪些输入状态误差传播为错误的未来占据、尺度、可见性或交互时序，而不是把所有几何差异等权相加。** 完整协议、图和边界见[未来轨迹反事实](ACTOR_TRAJECTORY_COUNTERFACTUAL.md)。
