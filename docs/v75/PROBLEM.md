...
未来减速干预随后通过了两个来源的 reference editability gate。大误差来源中，可靠 LiDAR 参照下 DVGT 派生中心误差为 `13.27m`，reference/DVGT/class-prior 的配对响应误差分别为 `2.11/55.61/32.08px`，两个重建臂都触发事前 material rule；小误差独立对照的 DVGT 中心误差为 `0.380m`，三臂响应误差为 `3.59/1.65/2.84px`，没有退化。当前定义因而具体化为：**优先研究会改变未来占据、尺度、可见性或交互时序的重建状态误差，而不是把所有几何差异等权相加。** 完整协议、图和边界见[未来轨迹反事实](ACTOR_TRAJECTORY_COUNTERFACTUAL.md)。
