...
本轮新增：来源筛查 5/8 日志、1 次 DVGT、独立确认 6 段、机制诊断 1 段，共 819 个新生成帧、105 次世界模型前向。开发发现另有 6 段、702 帧，属于先前曝光来源。所有 GPU 运行单张 RTX3090 完成，无 OOM。
...
后续只接受通过 G0 raster 和 G1 reference editability 的新固定来源，再比较 G2 重建状态、普通控制和额外信息修复。若多个来源持续在 G1 失败，应把研究主问题转为可编辑生成状态接口，而不是继续扩大重建方法榜单。
