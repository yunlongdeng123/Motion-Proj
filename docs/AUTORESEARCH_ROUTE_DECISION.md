...
审计基线：`bbb77a4`；Phase 2 诊断最终代码：`a712587`（文档提交前）。
...
VAE 的最大 LPIPS 为 0.06805（高于 0.05），并出现 source-retention 与无 depth-order overlap。E0 的
官方 CoTracker3 checkpoint 在本机不可获得，故不能建立独立 rollout 改善的证据。P1 的 target legality
hard failure 已足以否决当前 endpoint/feature/short-chain 的共同监督源；E0 block 进一步禁止任何
rollout-performance claim。
...
该 fallback 不是恢复训练。只有同时取得可校验的官方独立 evaluator 权重、并以不依赖大型视频编辑模型的
...
### 1.5 E0 独立 rollout evaluator 更新（2026-07-14；blocked）

E0 新增了可选的官方 CoTracker3 offline provider：query 固定为 evaluator 自己的 first-frame grid；输入
只允许 frozen `base_rgb` 与 evaluator weights；明确禁止读取 cache generated tracks、P0/P1 output、future
GT 或 source-future metadata。实现同时定义 evaluator-only robust affine background motion，所有速度、
加速度与 jerk 只命名为 `camera-compensated image-plane` 指标；无有效轨迹会返回 `invalid`，不会写成 0。
代码已通过完整测试，且官方 local hub entry 在无权重模式下可成功构造。

正式 run `autoresearch-e0-evaluator-s20260714-v1` 在 clean commit `a712587` 上执行时，官方仓库固定
在 `82e02e8029753ad4ef13cf06be7f4fc5facdda4d`，但
`/root/autodl-tmp/weights/cotracker3/scaled_offline.pth` 不存在，官方 URL 的下载连接亦被环境拒绝。
run 因而以 `status=blocked`、`fallback_used=false`、`uses_future_gt=false` 终止；没有把 RAFT、KLT 或
任一非独立 tracker 替代为指标。重复性、扰动、synthetic sanity 与 12-panel human alignment 均**未执行**，
不能据此声称 evaluator 有效或 rollout 得到改善。证据：
`/root/autodl-tmp/runs/autoresearch-e0-evaluator-s20260714-v1/`。
...
| B. Projected track / feature relation distillation | P0 有 290 个高 SNR 的 primary point corrections；F1 不能仅凭 sub-cell 数值永久否定连续 relation。 | B 与 A 共享 P1 的非法 target；P0 人审未完成；F1-R 的 P1/E0 前置不成立，且 Track4Gen 邻域拥挤。 | rejected |
| C. Short-chain / truncated rollout training | 仅在 one-step target 合法、局部性可控而 25-step transfer 失败时才有因果意义。 | 当前失败发生在 target 合法性之前，且 E0 未验证；short chain 不能把不可观察的 counterfactual 变合法。 | rejected |
...
| E. Stop current explicit dynamics projection core | P1 直接满足“RGB/latent counterfactual 不合法”的停止条件；E0 block 禁止 rollout 结论。 | 不等于宣称所有 endpoint 或连续 feature mechanism 在任何 target 下都失败。 | **selected** |
...
rollout 上证明高阶动力学改善而不是少运动。P1 已否决其 counterfactual supervision 源，E0 又尚未形成独立
rollout evidence，因此当前不能把这条边界写成方法贡献。
...
held-out one-step 与 independent rollout。当前分别为 `1 / awaiting review / 0 / 0 / unknown / blocked`；
...
最多重新打开两个前提：官方独立 tracker 权重可获得且 hash 可固定；以及一个不依赖大模型、可明确 source
removal/depth-order 的 counterfactual renderer 能在 P1-style gate 中通过。两者都不是当前项目已有方法的
小修；在它们之前不运行 short-chain、F1-R、F2、F3、O1 或任何生成器训练。

## 8. 明确停止做什么

- 停止用当前 temporal-only LoRA 继续 endpoint LR/preserve-weight sweep；
- 停止把更大的 preserve weight 当 locality 修复；
- 停止 F1-R、F2、F3、O1 与 short-chain；它们共享 P1 已失败的 target 或 E0 未验证的 rollout 前提；
...
1. 获得官方 CoTracker3 offline checkpoint 后，记录 URL、commit 与 SHA256，以**新** run ID 重跑 E0 的 repeatability、扰动、synthetic 与 12-panel review；不能复用 v1 或替换 provider。
...
- `configs/diagnostics/autoresearch_{c0_conditioning,p0_projector,p1_target,e0_evaluator}.yaml`
...
- `/root/autodl-tmp/runs/autoresearch-e0-evaluator-s20260714-v1`（blocked evidence）
...
测试：E0 code commit 前 full suite 为 `150 passed, 2 warnings`；官方 CoTracker3 local hub entry 在
`pretrained=False` 下构造成功。P0 panel 人审仍未完成；E0 在 checkpoint 缺失时没有生成 overlay，不能伪造
人工一致性。完整 Phase 2 汇总见 `docs/AUTORESEARCH_PHASE2_REPORT.md`。
