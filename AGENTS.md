...
[有限收口报告](docs/WORLDSIM_V8_1_BOUNDED_CLOSEOUT_REPORT.md)；[方法族badcase report](docs/WORLDSIM_V8_1_METHOD_BADCASE_REPORT.md)；[推进判定](docs/WORLDSIM_V8_1_TO_V8_2_DECISION.md)。以下为历史。
...
[方法族badcase report](docs/WORLDSIM_V8_1_METHOD_BADCASE_REPORT.md)；外部证据`/root/autodl-tmp/runs/worldsim_v81/WS-V81-CLOSE-01`。failure_ledger_refs=V81-F01/F02/F03/F04；failure_ledger_delta=待本轮收口。以下为历史。

# 当前结果：V8.1 P2主模型阶段已完成（2026-09-13）

DVGT/VGGT各220项推理、31张图与控制分析完成，V8.2 NO_GO_PENDING_RELIABLE_FAILURE。无活动推理worker、无自动恢复、未关机；2×3090足够。下一步先补可靠自然四格与残余失效证据，不重复440项已完成队列。当前入口docs/WORLDSIM_V8_1_GPU_HANDOFF.md。下文保留用户授权和各阶段历史。
