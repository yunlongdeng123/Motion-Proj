...
- 保留代码块、命令、路径、库名等原文。
- 用户偏好：不得把 `*.codexbak.*`、`*.bak` 或编辑器备份散落在 `docs/` 中原文件旁边。文档备份应直接
  放入对应路线的 `docs/archive/<月份>/<路线>/codex-backups/<日期-任务>/`；没有路线归属时放入同月份归档的
  `codex-backups/<日期-任务>/`。同名文件必须按来源目录分类或保留相对路径，禁止覆盖。
- 备份目录只用于恢复，不加入文档导航或研究事实源；除非用户明确要求，否则不提交这些备份文件。
