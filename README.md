...
- SVD common-prefix sibling 路线已停止；新的 V5 route-pivot autoresearch 已启动，当前只允许依次验证
  真实 ego–actor motion representation 与 natural independent-rollout best-of-N ceiling；
- V5 全部 gate 固定单张 RTX 4090，不恢复旧 labels/fork/rho，不切双卡，也不自动进入正式长训。
...
3. [`docs/MOTION_ROUTE_PIVOT_AUTORESEARCH_PLAN_V5.md`](docs/MOTION_ROUTE_PIVOT_AUTORESEARCH_PLAN_V5.md)：
   当前 R0→R1→A0→A1/B0→条件 A2/C→D0 执行协议；
4. [`docs/EXPERIMENTS.md`](docs/EXPERIMENTS.md)：正式实验事实源；
5. [`docs/archive/2026-07/README.md`](docs/archive/2026-07/README.md)：旧计划、报告和评审协议索引。
