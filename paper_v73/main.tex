...
\date{Interim r3: 8 September 2026, 22:20 UTC}
\begin{document}
\maketitle
\begin{abstract}
...
establish stable improvement from the same objective change. The joint model
still has fewer correct hits than its same-objective LiDAR reference in every
development log. We therefore begin Q-v2, an explicit shared-vertex surface
parameterization that preserves native-decoder adaptation and measurement
supervision. Its training is underway; no quality result is claimed. The
report records completed results, source and density limitations, and the
remaining independent-log and scene-composition evidence. No final method
...
\caption{Implemented Q-v2 candidate; research quality remains pending. The
...
Q-v2 now tests a shared-vertex parameterization under the preserved R12
data/objectives. Restricted upper-aggregation adaptation, near-surface
certified-free supervision and local correspondence consistency remain
separate later mechanisms rather than simultaneous changes to this run.
...
The R10/R12/R14 triangle and native R11 anchor are complete. Q-v2's first
shared-vertex run has started from fresh M1 initialization; its quality remains
pending. Full input-cohort training, same-decoder controls, selected-model scene
composition and independent new-log confirmation remain unfinished. These
cannot be replaced by the synthetic gradient-path check or the topology prior.
