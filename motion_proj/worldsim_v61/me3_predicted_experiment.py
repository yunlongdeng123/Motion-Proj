"""WorldSim V6.1 ME-3：GaussianWorld predicted occupancy development。"""
...
TASK_ID = "WS-V61-ME3-PREDICTED-OCC-01"


class ME3ExperimentError(RuntimeError):
    """ME-3 source、预测、适配或 verifier 合同失败。"""
        ...
        if time.monotonic() >= deadline:
            for process in processes:
                if process.poll() is None:
                    process.terminate()
            raise ME3ExperimentError("GaussianWorld parallel workers 超时")
        time.sleep(0.5)
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    p6_root = _resolve_runs_uri(config["sources"]["p6_run"])
    ...
    for root, files in (
        (p6_root, config["sources"]["p6_files"]),
    ...
    p6_gate = json.loads((p6_root / "P6_GATE.json").read_text(encoding="utf-8"))
    p6_terminal = json.loads((p6_root / "TERMINAL.json").read_text(encoding="utf-8"))
    if not p6_gate.get("passed") or not p6_terminal.get("canonical"):
        raise ME3ExperimentError("P6 GaussianWorld capability authority 未通过")

    official_repo = Path(config["model"]["official_repo"])
    if _git(official_repo, "rev-parse", "HEAD") != config["model"]["official_git_commit"]:
        raise ME3ExperimentError("GaussianWorld official commit 漂移")
    if _git(official_repo, "status", "--porcelain"):
        raise ME3ExperimentError("GaussianWorld official checkout 非 clean")
    ...
    for name, expected in config["model"]["files"].items():
        ...
        if record["bytes"] != int(expected["bytes"]) or record["sha256"] != expected["sha256"]:
            raise ME3ExperimentError(f"GaussianWorld model 输入漂移: {name}")
    ...
    run_dir = run_root / TASK_ID / f"{now}__predicted-occ-s{config['seed']}-r1"
    run_dir.mkdir(parents=True, exist_ok=False)
    processes: list[subprocess.Popen[str]] = []
    try:
        camera_ids = [int(value) for value in config["streaming"]["camera_ids"]]
        stream_frames = [int(value) for value in config["streaming"]["frames"]]
        target_frames = [int(value) for value in config["streaming"]["target_frames"]]
        worker_plans: list[dict[str, Any]] = []
        worker_logs: list[Any] = []
        worker_script = repo_root / "scripts/run_worldsim_v61_me3_gaussianworld_worker.py"
        worker_python = Path(config["environment"]["prefix"]) / "bin/python"
        for scene_index, scene in enumerate(config["cohort"]["scenes"]):
            scene_root = Path(config["raw_evidence"][scene])
            plan = {
                "schema_version": "worldsim_v61.me3_gaussianworld_worker_plan.v1",
                ...
                "final_shape": config["streaming"]["final_shape"],
                ...
                **config["model"]["output_contract"],
                "frozen_scene_files": _worker_scene_files(
                    scene_root, stream_frames, camera_ids
                ),
            }
        ...
        if any(code != 0 for code in return_codes):
            ...
            raise ME3ExperimentError("GaussianWorld worker failed:\n" + "\n".join(tails))
        ...
        workers_exact = all(
            not report["model_load_missing_keys"]
            and not report["model_load_unexpected_keys"]
            and report["stream_frames"] == stream_frames
            ...
            and report["environment_versions"]["mmcv"] == str(expected_versions["mmcv"])
            ...
            and report["environment_versions"]["spconv-cu117"]
            == str(expected_versions["spconv"])
            and not report["surroundocc_label_read"]
            and not report["training_started"]
            and not report["confirmation_content_read"]
            for report in reports
        )
        if not workers_exact:
            raise ME3ExperimentError("GaussianWorld worker source/environment contract 漂移")
        ...
        for scene in config["cohort"]["scenes"]:
            ...
            for frame in target_frames:
                source_path = (
                    ...
                    / "predictions"
                    / scene
                    / f"f{frame:03d}/GAUSSIANWORLD_CLASS.npz"
                )
                source = np.load(source_path, allow_pickle=False)
                adapted = resample_gaussianworld_classes(
                    np.asarray(source["class_label"], dtype=np.uint8), target_spec
                )
            ...
            method_rows.append(
                {
                    ...
                    "decision_inputs": [
                        "frozen_R9_P1_photo",
                        "GaussianWorld_predicted_occupancy",
        ...
        def score(row: dict[str, Any]) -> dict[str, Any]:
            ...
            predicted_decision = row["decisions"]["P1-GW-PREDICTED"]
            return {
                **row,
        ...
        predicted = _arm_summary(final_rows, "P1-GW-PREDICTED")
        ...
        checks = {
            "p6_capability_authority_passed": True,
            ...
            "stream_schedule_exact": all(
                report["stream_frames"] == stream_frames for report in reports
            ),
        ...
        gate = {
            "schema_version": "worldsim_v61.me3_gate.v1",
            "task_id": TASK_ID,
            "hypothesis_id": config["hypothesis_id"],
            "primary_arm": "P1-GW-PREDICTED",
            "checks": checks,
            "passed": all(checks.values()),
        }
        _write_json(run_dir / "ME3_GATE.json", gate)
        metrics = {
            "schema_version": "worldsim_v61.me3_metrics.v1",
            "task_id": TASK_ID,
        ...
        resource = {
            "schema_version": "worldsim_v61.me3_resource_audit.v1",
        ...
        summary = {
            "schema_version": "worldsim_v61.me3_summary.v1",
            "task_id": TASK_ID,
            "hypothesis_id": config["hypothesis_id"],
            "source_commit": source_commit,
            "status": status,
            "hypothesis_outcome": "accepted_predicted_occupancy_development"
            if gate["passed"]
            else "rejected_predicted_occupancy_development",
            ...
            if gate["passed"]
            else "literature_audit_then_one_me3_recovery_without_threshold_lowering",
        ...
        manifest = {
            "schema_version": "worldsim_v61.me3_manifest.v1",
            "task_id": TASK_ID,
            "source_commit": source_commit,
        ...
        terminal = {
            "schema_version": "worldsim_v61.me3_terminal.v1",
            "task_id": TASK_ID,
            "status": status,
            "canonical": True,
