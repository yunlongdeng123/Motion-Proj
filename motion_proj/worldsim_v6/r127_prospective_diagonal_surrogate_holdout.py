"""WorldSim V6 R127: prospective diagonal-intervention binary-surrogate holdout."""
...
TASK_ID = "WS-V6-R127-PROSPECTIVE-DIAGONAL-SURROGATE-HOLDOUT-01"


class R127ExperimentError(RuntimeError):
    """The preregistered R127 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R127ExperimentError("formal R127 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R127ExperimentError("R127 task_id drift")
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R127ExperimentError("StreetGS upstream commit drift")
    free_gib = shutil.disk_usage(run_root).free / (1024**3)
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R127ExperimentError("R127 disk resource insufficient")
    ...
    if len(candidate_rows) != 1:
        raise R127ExperimentError("R127 diagonal candidate binding drift")
    ...
    if len(frame_indices) != int(runtime["expected_frame_count"]):
        raise R127ExperimentError("R127 frame denominator drift")

    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__prospective-diagonal-holdout-s{config['seed']}-r1"
    ...
    geometry = {
        "schema_version": "worldsim_v6.r127_transform_owned_actor_geometry.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r127_actor_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r127_actor_package_manifest.v1",
            "files": {
    ...
    composition = {
        "schema_version": "worldsim_v6.r127_scene_composition.v1",
        "scene": runtime["scene"],
        "actors": [
            {
    ...
    runtime_contract = {
        "schema_version": "worldsim_v6.r127_runtime_contract.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r127_scene_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r127_scene_package_manifest.v1",
            "files": {
    ...
    if [row["frame_index"] for row in sensor_rows] != frame_indices:
        raise R127ExperimentError("R127 sensor output denominator/order drift")

    perception_dir = run_dir / "perception"
    variants = list(evaluation["variants"])
    if variants != ["logged", "edited"]:
        raise R127ExperimentError("R127 variant order drift")
    index_rows = [
        {
    ...
    transfer = {
        "schema_version": "worldsim_v6.r127_prospective_diagonal_surrogate_holdout.v1",
    ...
    checks = {
        "r101_r102_r90_r125_r126_authorities_accepted": bool(
            r101_gate["checks"]["passed"]
            and r102_gate["checks"]["passed"]
            and r90_gate["checks"]["passed"]
            and r125_gate["checks"]["passed"]
            and r126_gate["checks"]["passed"]
        ),
        "r101_accepted_diagonal_candidate_exact": candidate["proposal_id"] == runtime["proposal_id"]
        ...
        "prospective_diagonal_is_distinct_from_r102_axis_edit": sum(
            abs(float(value)) > 0.0 for value in runtime["translation_delta_m"]
        ) == 2
        and runtime["translation_delta_m"] != selected["translation_delta_m"],
    ...
    _write_json(
        run_dir / "R127_GATE.json",
        {
            "schema_version": "worldsim_v6.r127_gate.v1", "checks": checks,
            "decision": "accept_prospective_diagonal_binary_surrogate_holdout"
            if checks["passed"] else "reject_prospective_diagonal_binary_surrogate_holdout",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r127_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r127_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_prospective_diagonal_binary_surrogate_holdout"
    ...
    tracked = [
        "R127_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SELECTOR_TRANSFER.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r127_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r127_prospective_diagonal_surrogate_holdout_v1.yaml"),
    )
