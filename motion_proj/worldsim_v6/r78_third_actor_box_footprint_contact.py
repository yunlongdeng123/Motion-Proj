"""WorldSim V6 R78: verify actor5 contact with native box bottom and footprint ring."""
...
TASK_ID = "WS-V6-R78-THIRD-ACTOR-BOX-FOOTPRINT-CONTACT-01"


class R78ExperimentError(RuntimeError):
    """The preregistered R78 experiment contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R78ExperimentError("formal R78 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R78ExperimentError("R78 task_id drift")
    sources = config["sources"]
    r64_run = _resolve_runs_uri(sources["r64_run"])
    r77_run = _resolve_runs_uri(sources["r77_run"])
    ...
    frozen = {
        r64_run / "MANIFEST.json": sources["r64_manifest_sha256"],
        r64_run / "R64_GATE.json": sources["r64_gate_sha256"],
        r64_run / "SUMMARY.json": sources["r64_summary_sha256"],
        raw_union_path: sources["r64_raw_union_sha256"],
        r77_run / "MANIFEST.json": sources["r77_manifest_sha256"],
        r77_run / "R77_GATE.json": sources["r77_gate_sha256"],
        r77_run / "SUMMARY.json": sources["r77_summary_sha256"],
        r77_run / "SELECTED_PROPOSAL.json": sources["r77_selection_sha256"],
        r75_run / "R75_GATE.json": sources["r75_gate_sha256"],
        package / "PACKAGE_MANIFEST.json": sources["r75_package_manifest_sha256"],
        geometry_path: sources["r75_trajectory_geometry_sha256"],
        infos_path: sources["instances_info_sha256"],
        camera_path: sources["camera_front_start_sha256"],
    }
    for path, digest in frozen.items():
        _verify(path, digest)
    r64_gate = json.loads((r64_run / "R64_GATE.json").read_text(encoding="utf-8"))
    r77_gate = json.loads((r77_run / "R77_GATE.json").read_text(encoding="utf-8"))
    r77_selection = json.loads((r77_run / "SELECTED_PROPOSAL.json").read_text(encoding="utf-8"))
    r75_gate = json.loads((r75_run / "R75_GATE.json").read_text(encoding="utf-8"))
    package_manifest = _verify_package(package)
    free_gib = shutil.disk_usage(run_root).free / (1024**3)
    if free_gib < float(config["resources"]["minimum_disk_free_gib"]):
        raise R78ExperimentError("R78 disk resource insufficient")
    ...
    try:
        ...
        interventions = [
            {"id": "actor5_logged_baseline", "translation_delta_m": [0.0, 0.0, 0.0], "expected_contact_decision": "ACCEPT"},
            {"id": r77_selection["selected"]["proposal_id"], "translation_delta_m": r77_selection["selected"]["translation_delta_m"], "expected_contact_decision": "ACCEPT"},
        ]
        if interventions[1]["translation_delta_m"] != config["selected_intervention"]["expected_translation_delta_m"]:
            raise R78ExperimentError("R77 selected translation drift")
        rows_1 = [_evaluate(row, pose, size, points, config) for row in interventions]
        rows_2 = [_evaluate(row, pose, size, points, config) for row in interventions]
        _write_jsonl(run_dir / "BOX_FOOTPRINT_CONTACT_DECISIONS.jsonl", rows_1)
        wall_seconds = time.monotonic() - started
        checks = {
            "r64_raw_union_provenance_and_r77_r75_acceptance_preserved": bool(not r64_gate["checks"]["passed"] and r77_gate["checks"]["passed"] and r75_gate["checks"]["passed"]),
            ...
            "nearest_native_instance_binding_unambiguous": mapping_distance <= float(config["mapping"]["maximum_centroid_distance_m"]) and mapping_margin >= float(config["mapping"]["minimum_next_nearest_margin_m"]),
            "native_box_size_positive": bool(np.isfinite(size).all() and np.all(size > 0.0)),
        ...
        _write_json(run_dir / "R78_GATE.json", {"schema_version": "worldsim_v6.r78_gate.v1", "checks": checks, "decision": "accept_third_actor_box_footprint_contact" if checks["passed"] else "reject_or_repair_third_actor_box_footprint_contact"})
        _write_json(run_dir / "MAPPING_AUDIT.json", {"schema_version": "worldsim_v6.r78_mapping_audit.v1", "actor_model_index": int(cohort["actor_model_index"]), "processed_instance_id": int(instance_id), "class_name": info["class_name"], "centroid_distance_m": mapping_distance, "next_nearest_margin_m": mapping_margin, "size_lwh_m": size.tolist(), "object_local_up_world_vector": pose[:3, 2].tolist()})
        _write_json(run_dir / "RESOURCE_AUDIT.json", {"schema_version": "worldsim_v6.r78_resource_audit.v1", "gpu_used": False, "wall_seconds": wall_seconds, "disk_free_gib_at_start": free_gib, "training_started": False, "confirmation_content_read": False})
        _write_json(run_dir / "SUMMARY.json", {"schema_version": "worldsim_v6.r78_summary.v1", "task_id": TASK_ID, "hypothesis_id": config["hypothesis_id"], "status": status, "hypothesis_outcome": "accepted_development_third_actor_box_footprint_contact" if checks["passed"] else "rejected", "source_commit": source_commit, "processed_instance_id": int(instance_id), "class_name": info["class_name"], "logged_ring_candidates": rows_1[0]["ring_lidar_candidate_count"], "selected_ring_candidates": rows_1[1]["ring_lidar_candidate_count"], "logged_contact_error_m": rows_1[0]["contact_absolute_error_m"], "selected_contact_error_m": rows_1[1]["contact_absolute_error_m"], "logged_contact_decision": rows_1[0]["q_box_footprint_lidar_contact"], "selected_contact_decision": rows_1[1]["q_box_footprint_lidar_contact"], "claim_boundary": config["claim_boundary"]})
        tracked = ["R78_GATE.json", "SUMMARY.json", "BOX_FOOTPRINT_CONTACT_DECISIONS.jsonl", "MAPPING_AUDIT.json", "RESOURCE_AUDIT.json"]
        _write_json(run_dir / "MANIFEST.json", {"schema_version": "worldsim_v6.r78_manifest.v1", "files": {name: {"bytes": (run_dir / name).stat().st_size, "sha256": _sha256(run_dir / name)} for name in tracked}})
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/worldsim_v6/r78_third_actor_box_footprint_contact_v1.yaml"))
