"""WorldSim V6 R128: scene0230 orthogonal-intervention surrogate holdout."""
...
TASK_ID = "WS-V6-R128-SCENE0230-ORTHOGONAL-SURROGATE-HOLDOUT-01"


class R128ExperimentError(RuntimeError):
    """The preregistered R128 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R128ExperimentError("formal R128 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R128ExperimentError("R128 task_id drift")
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R128ExperimentError("StreetGS upstream commit drift")
    free_gib = shutil.disk_usage(run_root).free / (1024**3)
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R128ExperimentError("R128 disk resource insufficient")
    ...
    if len(candidate_rows) != 1:
        raise R128ExperimentError("R128 orthogonal candidate binding drift")
    ...
    if len(frame_indices) != int(runtime["expected_frame_count"]):
        raise R128ExperimentError("R128 frame denominator drift")

    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__scene0230-orthogonal-holdout-s{config['seed']}-r1"
    ...
    geometry = {
        "schema_version": "worldsim_v6.r128_transform_owned_actor_geometry.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r128_actor_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r128_actor_package_manifest.v1",
            "files": {
    ...
    composition = {
        "schema_version": "worldsim_v6.r128_scene_composition.v1",
        "scene": runtime["scene"],
        "actors": [
            {
    ...
    runtime_contract = {
        "schema_version": "worldsim_v6.r128_runtime_contract.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r128_scene_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r128_scene_package_manifest.v1",
            "files": {
    ...
    if [row["frame_index"] for row in sensor_rows] != frame_indices:
        raise R128ExperimentError("R128 sensor output denominator/order drift")

    perception_dir = run_dir / "perception"
    variants = list(evaluation["variants"])
    if variants != ["logged", "edited"]:
        raise R128ExperimentError("R128 variant order drift")
    index_rows = [
        {
    ...
    transfer = {
        "schema_version": "worldsim_v6.r128_scene0230_orthogonal_surrogate_holdout.v1",
    ...
    checks = {
        "r93_r94_r90_r125_r126_r127_authorities_accepted": bool(
        ...
        "r93_accepted_orthogonal_candidate_exact": candidate["proposal_id"] == runtime["proposal_id"]
        ...
        "prospective_orthogonal_equal_norm_to_r94_axis_edit": abs(
            sum(
                float(a) * float(b)
    ...
    _write_json(
        run_dir / "R128_GATE.json",
        {
            "schema_version": "worldsim_v6.r128_gate.v1", "checks": checks,
            "decision": "accept_scene0230_orthogonal_binary_surrogate_holdout"
            if checks["passed"] else "reject_scene0230_orthogonal_binary_surrogate_holdout",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r128_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r128_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_scene0230_orthogonal_binary_surrogate_holdout"
    ...
    tracked = [
        "R128_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SELECTOR_TRANSFER.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r128_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r128_scene0230_orthogonal_surrogate_holdout_v1.yaml"),
    )
