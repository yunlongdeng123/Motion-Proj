"""WorldSim V6 R71: execute a two-actor scene-edit package in one shared sensor render."""
...
TASK_ID = "WS-V6-R71-TWO-ACTOR-SCENE-SENSOR-RUNTIME-01"


class R71ExperimentError(RuntimeError):
    """The preregistered R71 experiment contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R71ExperimentError("formal R71 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R71ExperimentError("R71 task_id drift")
    ...
    frozen_files = {
        r70_run / "MANIFEST.json": sources["r70_manifest_sha256"],
        r70_run / "R70_GATE.json": sources["r70_gate_sha256"],
        r70_run / "SUMMARY.json": sources["r70_summary_sha256"],
        scene_package / "SCENE_PACKAGE_MANIFEST.json": sources["r70_scene_package_manifest_sha256"],
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R71ExperimentError("StreetGS upstream commit drift")
    ...
    run_dir = run_root / TASK_ID / f"{now}__two-actor-sensor-s{config['seed']}-r1"
    ...
    try:
        ...
        if completed.returncode != 0:
            raise R71ExperimentError(f"two-actor sensor worker failed: rc={completed.returncode}")
        rows = [
            json.loads(line)
            for line in (worker_dir / "FRAME_METRICS.jsonl").read_text(encoding="utf-8").splitlines()
        ]
        if len(rows) != 1:
            raise R71ExperimentError("R71 requires exactly one sensor frame")
        row = rows[0]
        ...
        checks = {
            ...
            "both_actors_have_visible_effect": all(
            ...
            "scene_and_nested_package_manifests_immutable": bool(
                ...
                == sources["r70_scene_package_manifest_sha256"]
        ...
        _write_json(
            run_dir / "R71_GATE.json",
            {
                "schema_version": "worldsim_v6.r71_gate.v1",
                "checks": checks,
                "decision": "accept_two_actor_scene_sensor_runtime"
                if checks["passed"]
                else "reject_or_repair_two_actor_scene_sensor_runtime",
            },
        )
        _write_json(
        ...
        _write_json(
            ...
            {
                "schema_version": "worldsim_v6.r71_resource_audit.v1",
                "gpu_used": True,
        ...
        summary = {
            "schema_version": "worldsim_v6.r71_summary.v1",
            "task_id": TASK_ID,
            "hypothesis_id": config["hypothesis_id"],
            "status": "done" if checks["passed"] else "rejected",
            "hypothesis_outcome": "accepted_development_two_actor_scene_sensor_runtime"
            if checks["passed"]
            else "rejected",
        ...
        _write_json(
            ...
            {
                "schema_version": "worldsim_v6.r71_manifest.v1",
                "files": {
