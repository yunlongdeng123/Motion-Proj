"""WorldSim V6 R13 单 actor 删除的 sensor/perception 局部性实验。"""
...
def _run_frontend_worker(command: list[str], repo_root: Path, log_path: Path) -> None:
    env = dict(os.environ)
    completed = subprocess.run(command, cwd=repo_root, env=env, capture_output=True, text=True)
    log_path.write_text(completed.stdout + completed.stderr, encoding="utf-8")
    if completed.returncode != 0:
        raise R13ActorSensorError(f"StreetGS worker 失败，见 {log_path}")
...
def run_experiment(
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R13ActorSensorError("正式 R13 single-actor run 禁止 dirty source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R13ActorSensorError("R13 single-actor task_id 漂移")
    sources = config["sources"]
    rejected_run = _resolve_runs_uri(sources["rejected_roi_run"])
    ...
    frozen = {
        rejected_run / "MANIFEST.json": sources["rejected_roi_manifest_sha256"],
        rejected_run / "R13_FACTORIZED_PERCEPTION_GATE.json": sources["rejected_roi_gate_sha256"],
    ...
    rejected_gate = json.loads(
        (rejected_run / "R13_FACTORIZED_PERCEPTION_GATE.json").read_text(encoding="utf-8")
    )
    dynamic_gate = json.loads(
        (dynamic_run / "R13_DYNAMIC_EDIT_GATE.json").read_text(encoding="utf-8")
    )
    rejected_roi_preserved = (
        not rejected_gate["checks"]["passed"]
        and rejected_gate["decision"] == "reject_factorized_roi_perception_hypothesis"
    )
    if not rejected_roi_preserved or not dynamic_gate["checks"]["passed"]:
        raise R13ActorSensorError("冻结的 ROI rejection 或 typed dynamic pass 漂移")
    ...
    if shutil.disk_usage(run_root).free / (1024**3) < float(config["resources"]["minimum_disk_free_gib"]):
        raise R13ActorSensorError("R13 single-actor 磁盘资源不足")
    now = datetime.now(timezone.utc)
    run_dir = run_root / TASK_ID / f"{now.strftime('%Y%m%dT%H%M%SZ')}__single-actor-perception-s{config['seed']}-r1"
    ...
    try:
        ...
        worker_index = {
            (int(row["frame_index"]), row["state"]): render_dir / row["path"]
            for row in render_rows
        }
        for row in render_rows:
            path = render_dir / row["path"]
            if _sha256(path) != row["sha256"]:
                raise R13ActorSensorError(f"single-actor render 漂移：{path}")
        ...
        if frontend_audit["checkpoint_sha256_before"] != frontend_audit["checkpoint_sha256_after"]:
            raise R13ActorSensorError("worker checkpoint 漂移")
        repeat_count = int(config["cohort"]["repeat_count"])
        perception_inputs = []
        for frame in frames:
            for state in ("logged", "edited"):
                path = worker_index[(frame, state)]
                for repeat_index in range(1, repeat_count + 1):
                    perception_inputs.append(
                        {
                            "case_id": f"streetgs__f{frame:03d}__{state}",
        ...
        metric_rows = []
        for frame in frames:
            logged = _rgb(worker_index[(frame, "logged")])
            edited = _rgb(worker_index[(frame, "edited")])
            frozen_rgb = _rgb(frozen_logged[frame])
            replay_rgb_mae = float(np.mean(np.abs(logged - frozen_rgb)))
            pixel_difference = np.mean(np.abs(edited - logged), axis=-1)
            ...
            if not np.any(target) or not np.any(outside):
                raise R13ActorSensorError("single-actor target/outside 分母为空")
            target_rgb_mae = float(np.mean(pixel_difference[target]))
            outside_rgb_mae = float(np.mean(pixel_difference[outside]))
            rgb_enrichment = target_rgb_mae / max(outside_rgb_mae, 1.0e-12)
            logged_case = f"streetgs__f{frame:03d}__logged"
            edited_case = f"streetgs__f{frame:03d}__edited"
            ...
            changed = logged_labels != edited_labels
            ...
            checks = {
                "effect_denominator": int(np.count_nonzero(target))
                >= int(thresholds["minimum_effect_pixels"]),
                "target_rgb_effect": target_rgb_mae >= float(thresholds["minimum_target_rgb_mae"]),
                "outside_rgb_preserved": outside_rgb_mae <= float(thresholds["maximum_outside_rgb_mae"]),
                ...
                "logged_replay_rgb": replay_rgb_mae
                <= float(thresholds["maximum_logged_replay_rgb_mae"]),
            }
            checks["passed"] = all(checks.values())
            metric_rows.append(
                {
                    "schema_version": "worldsim_v6.r13_single_actor_perception_metric.v1",
                    "frame_index": frame,
                    "selected_model_index": actor_selection["selected_model_index"],
                    "selected_gaussian_count": actor_selection["selected_gaussian_count"],
                    "effect_pixel_count": int(np.count_nonzero(target)),
                    "outside_pixel_count": int(np.count_nonzero(outside)),
                    "logged_replay_rgb_mae": replay_rgb_mae,
        ...
        _write_jsonl(run_dir / "SINGLE_ACTOR_PERCEPTION_METRICS.jsonl", metric_rows)
        ...
        checks = {
            "both_frames_pass": len(metric_rows) == 2
            and all(row["checks"]["passed"] for row in metric_rows),
            "same_actor_both_frames": len({row["selected_model_index"] for row in metric_rows}) == 1,
            "logged_replay_matches_frozen_r3": all(
                row["checks"]["logged_replay_rgb"] for row in metric_rows
            ),
            ...
            "rejected_roi_source_preserved": rejected_roi_preserved,
        ...
        checks["passed"] = all(checks.values())
        gate = {
            "schema_version": "worldsim_v6.r13_single_actor_perception_gate.v1",
            "checks": checks,
            "unsupported_metrics": unsupported,
            "decision": "accept_single_actor_perception_locality"
            if checks["passed"]
            else "reject_single_actor_perception_hypothesis",
        }
        _write_json(run_dir / "R13_SINGLE_ACTOR_PERCEPTION_GATE.json", gate)
        summary = {
            "schema_version": "worldsim_v6.r13_single_actor_perception_summary.v1",
            "task_id": TASK_ID,
            "hypothesis_id": config["hypothesis_id"],
            "status": "done" if checks["passed"] else "rejected",
            "hypothesis_outcome": "accepted_development_single_actor_perception_locality"
            if checks["passed"]
            else "rejected",
        ...
        tracked = [
            ...
            "SINGLE_ACTOR_PERCEPTION_METRICS.jsonl",
            "R13_SINGLE_ACTOR_PERCEPTION_GATE.json",
            "SUMMARY.json",
        ...
        _write_json(
            ...
            {
                "schema_version": "worldsim_v6.r13_single_actor_perception_manifest.v1",
                "source_commit": source_commit,
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/worldsim_v6/r13_single_actor_perception_v0.yaml"),
    )
    parser.add_argument(
