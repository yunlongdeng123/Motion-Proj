"""WorldSim V6 R95: actor inventory and visibility on second independent scene0048."""
...
from motion_proj.worldsim_v6.r44_verified_actor_proposal_bake import _git, _sha256, _verify, _write_json


TASK_ID = "WS-V6-R95-SECOND-INDEPENDENT-SCENE-ACTOR-VISIBILITY-01"


class R95ExperimentError(RuntimeError):
    """The preregistered R95 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R95ExperimentError("formal R95 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R95ExperimentError("R95 task_id drift")
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R95ExperimentError("StreetGS upstream commit drift")
    if shutil.disk_usage(run_root).free / (1024**3) < float(resources["minimum_disk_free_gib"]):
        raise R95ExperimentError("R95 disk resource insufficient")
    ...
    run_dir = run_root / TASK_ID / f"{now}__scene0048-actor-visibility-s{config['seed']}-r1"
    ...
    command = [
        ...
        "--upstream-root", str(upstream), "--probe-frames", ",".join(map(str, probe_frames)),
        "--minimum-primitives", str(selection["minimum_primitive_count"]),
        "--opacity-threshold", str(selection["opacity_threshold"]), "--output", str(worker_dir),
    ]
    ...
    derived_candidates = [
        actor["actor_model_index"] for actor in actors
    ...
    active_rows = [row for row in rows if row["native_frame_valid"]]
    if not active_rows:
        raise R95ExperimentError("no active visibility rows")
    selected = sorted(
        active_rows,
        key=lambda row: (-row["actor_effect_pixels"], row["actor_model_index"], row["frame_index"]),
    )[0]
    decision = {
        "schema_version": "worldsim_v6.r95_second_independent_scene_actor_selection.v1",
        "scene": sources["scene"], "scene_index": int(sources["scene_index"]),
        "probe_frames": probe_frames, "selection_rule": selection["selection_rule"],
    ...
    expected_pairs = {(frame, actor_index) for frame in probe_frames for actor_index in derived_candidates}
    ...
    checks = {
        ...
        and matched_status["scene"] == sources["scene"] and matched_summary["scene"] == sources["scene"],
        ...
        "primitive_partition_exact": inventory["assigned_primitive_count"] == inventory["primitive_count"]
        ...
        and [actor["actor_model_index"] for actor in actors] == list(range(inventory["actor_model_count"])),
        "each_lifecycle_content_addressed": all(
            _sha256(worker_dir / actor["lifecycle_path"]) == actor["lifecycle_sha256"] for actor in actors
        ),
        ...
        "visibility_pair_denominator_exact": observed_pairs == expected_pairs and len(rows) == len(expected_pairs),
        "row_primitive_counts_match_inventory": all(
            row["primitive_count"] == actors[row["actor_model_index"]]["primitive_count"] for row in rows
        ),
    ...
    _write_json(
        run_dir / "R95_GATE.json",
        {"schema_version": "worldsim_v6.r95_gate.v1", "checks": checks,
         "decision": "accept_second_independent_scene_actor_for_compiler_extension"
         if checks["passed"] else "reject_or_repair_second_independent_scene_actor_selection"},
    )
    summary = {
        "schema_version": "worldsim_v6.r95_summary.v1", "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"], "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_second_independent_scene_actor_visibility_selection"
        if checks["passed"] else "rejected", "source_commit": source_commit,
        "scene": sources["scene"], "frame_count": inventory["frame_count"],
        "actor_model_count": inventory["actor_model_count"], "rigid_primitive_count": inventory["primitive_count"],
    ...
    tracked = [
        "R95_GATE.json", "SUMMARY.json", "ACTOR_VISIBILITY_SELECTION.json", "worker.log",
    ...
    _write_json(
        ...
        {"schema_version": "worldsim_v6.r95_manifest.v1", "files": {
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/worldsim_v6/r95_second_independent_scene_actor_visibility_v1.yaml"))
