"""WorldSim V6 R124: frozen RGB-count surrogate for binary perception impact."""
...
TASK_ID = "WS-V6-R124-BINARY-PERCEPTION-SURROGATE-CORPUS-01"


class R124ExperimentError(RuntimeError):
    """The preregistered R124 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R124ExperimentError("formal R124 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R124ExperimentError("R124 task_id drift")
    ...
    for condition_name in runtime["condition_order"]:
        ...
        if int(transfer["frame_count"]) != expected_frames:
            raise R124ExperimentError(f"{condition_name} frame denominator drift")
        features = np.asarray(
            [
    ...
    certificate = {
        "schema_version": "worldsim_v6.r124_binary_perception_surrogate_corpus.v1",
    ...
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R124ExperimentError("R124 disk resource insufficient")
    ...
    _write_json(
        run_dir / "R124_GATE.json",
        {
            "schema_version": "worldsim_v6.r124_gate.v1",
            "checks": checks,
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r124_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r124_summary.v1",
        "task_id": TASK_ID,
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r124_manifest.v1",
            "files": {
                name: {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/worldsim_v6/r124_binary_perception_surrogate_corpus_v1.yaml"),
    )
    parser.add_argument(
