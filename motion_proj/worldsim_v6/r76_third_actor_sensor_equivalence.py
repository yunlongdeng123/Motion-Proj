"""WorldSim V6 R76: actor5 package versus native StreetGS five-frame sensor equivalence."""
...
TASK_ID = "WS-V6-R76-THIRD-ACTOR-SENSOR-EQUIVALENCE-01"


class R76ExperimentError(RuntimeError):
    """The preregistered R76 experiment contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R76ExperimentError("formal R76 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R76ExperimentError("R76 task_id drift")
    ...
    frozen_files = {
        r75_run / "MANIFEST.json": sources["r75_manifest_sha256"],
        r75_run / "R75_GATE.json": sources["r75_gate_sha256"],
        r75_run / "SUMMARY.json": sources["r75_summary_sha256"],
        package / "PACKAGE_MANIFEST.json": sources["r75_package_manifest_sha256"],
        package / "TRAJECTORY_GEOMETRY.json": sources["r75_trajectory_geometry_sha256"],
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R76ExperimentError("StreetGS upstream commit drift")
    ...
    if free_gib < float(config["resources"]["minimum_disk_free_gib"]):
        raise R76ExperimentError("R76 disk resource insufficient")
    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__actor5-sensor-s{config['seed']}-r1"
    ...
    try:
        ...
        if completed.returncode != 0:
            raise R76ExperimentError(f"actor5 sensor worker failed: rc={completed.returncode}")
        ...
        checks = {
            "r75_authority_accepted": bool(r75_gate["checks"]["passed"]),
            ...
            "package_manifest_immutable": audit["package_manifest_sha256_before"] == audit["package_manifest_sha256_after"] == sources["r75_package_manifest_sha256"],
        ...
        _write_json(run_dir / "R76_GATE.json", {"schema_version": "worldsim_v6.r76_gate.v1", "checks": checks, "decision": "accept_third_actor_native_sensor_equivalence" if checks["passed"] else "reject_or_repair_third_actor_sensor_equivalence"})
        _write_json(run_dir / "RESOURCE_AUDIT.json", {"schema_version": "worldsim_v6.r76_resource_audit.v1", "peak_torch_reserved_mib": peak_mib, "worker_wall_seconds": audit["wall_seconds"], "wall_seconds": wall_seconds, "disk_free_gib_at_start": free_gib, "training_started": False, "confirmation_content_read": False})
        _write_json(run_dir / "SUMMARY.json", {
            "schema_version": "worldsim_v6.r76_summary.v1", "task_id": TASK_ID, "hypothesis_id": config["hypothesis_id"], "status": "done" if checks["passed"] else "rejected",
            "hypothesis_outcome": "accepted_development_third_actor_native_sensor_equivalence" if checks["passed"] else "rejected", "source_commit": source_commit,
        ...
        tracked = ["R76_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "worker.log", "worker/FRAME_METRICS.jsonl", "worker/WORKER_AUDIT.json"]
        tracked.extend(f"worker/{row['sensor_path']}" for row in rows)
        _write_json(run_dir / "MANIFEST.json", {"schema_version": "worldsim_v6.r76_manifest.v1", "files": {name: {"bytes": (run_dir / name).stat().st_size, "sha256": _sha256(run_dir / name)} for name in tracked}})
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/worldsim_v6/r76_third_actor_sensor_equivalence_v1.yaml"))
