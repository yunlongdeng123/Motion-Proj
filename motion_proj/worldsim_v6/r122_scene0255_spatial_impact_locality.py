"""WorldSim V6 R122: RGB-difference spatial locality for saturated four-actor edits."""
...
TASK_ID = "WS-V6-R122-SCENE0255-SPATIAL-IMPACT-LOCALITY-01"


class R122ExperimentError(RuntimeError):
    """The preregistered R122 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R122ExperimentError("formal R122 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R122ExperimentError("R122 task_id drift")
    runtime = config["runtime"]
    thresholds = config["thresholds"]
    resources = config["resources"]
    radii = [int(value) for value in runtime["dilation_radii_pixels"]]
    if radii != sorted(set(radii)) or radii[0] != 0:
        raise R122ExperimentError("R122 dilation grid drift")
    ...
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R122ExperimentError("R122 disk resource insufficient")
    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__spatial-impact-locality-s{config['seed']}-r1"
    ...
    for source_name in runtime["source_order"]:
        ...
        if int(transfer["frame_count"]) != expected_frames:
            raise R122ExperimentError(f"{source_name} frame denominator drift")
        for frame in range(expected_frames):
            ...
            for relative in (relative_sensor, relative_logged, relative_edited):
                if relative not in files:
                    raise R122ExperimentError(f"{source_name} manifest missing {relative}")
            ...
            if logged_rgb.shape != edited_rgb.shape or logged_rgb.ndim != 3:
                raise R122ExperimentError(f"{source_name} RGB shape drift")
            if logged_labels.shape != edited_labels.shape or logged_labels.ndim != 2:
                raise R122ExperimentError(f"{source_name} label shape drift")
            if logged_rgb.shape[:2] != logged_labels.shape:
                raise R122ExperimentError(f"{source_name} RGB/label spatial drift")
            if expected_shape is None:
                expected_shape = tuple(int(value) for value in logged_labels.shape)
            if tuple(logged_labels.shape) != expected_shape:
                raise R122ExperimentError(f"{source_name} cross-frame shape drift")
            ...
            if label_count != expected_label_count or label_count < 1:
                raise R122ExperimentError(f"{source_name} frame{frame} label authority drift")
    ...
    certificate = {
        "schema_version": "worldsim_v6.r122_spatial_impact_locality.v1",
    ...
    _write_json(run_dir / "SPATIAL_LOCALITY_CERTIFICATE.json", certificate)
    ...
    checks = {
        ...
        "exact_label_coverage_radius_exists_within_grid": selected_radius is not None
        and selected_radius <= int(thresholds["maximum_exact_coverage_radius_pixels"]),
        "selected_radius_aggregate_and_worst_frame_recall_exact": selected_summary is not None
        and selected_summary["aggregate_label_recall"] == 1.0
        and selected_summary["worst_frame_label_recall"] == 1.0,
        "selected_sparse_roi_mean_fraction_within_gate": selected_summary is not None
        and selected_summary["mean_roi_fraction"]
        <= float(thresholds["maximum_mean_roi_fraction"]),
        "selected_sparse_roi_p95_fraction_within_gate": selected_summary is not None
        and selected_summary["p95_roi_fraction"]
        <= float(thresholds["maximum_p95_roi_fraction"]),
    ...
    _write_json(
        run_dir / "R122_GATE.json",
        {
            "schema_version": "worldsim_v6.r122_gate.v1",
            "checks": checks,
            "decision": "accept_rgb_diff_sparse_spatial_locality_candidate"
            if checks["passed"]
            else "reject_rgb_diff_sparse_spatial_locality_candidate",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r122_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r122_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_rgb_diff_sparse_spatial_locality"
        if checks["passed"]
        else "rejected",
    ...
    tracked = [
        ...
        "SUMMARY.json",
        "RESOURCE_AUDIT.json",
        "SPATIAL_LOCALITY_CERTIFICATE.json",
        "FRAME_LOCALITY.jsonl",
    ]
    _write_json(
        run_dir / "MANIFEST.json",
        {
            "schema_version": "worldsim_v6.r122_manifest.v1",
            "files": {
                name: {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/worldsim_v6/r122_scene0255_spatial_impact_locality_v1.yaml"),
    )
    parser.add_argument(
        "--run-root",
        type=Path,
