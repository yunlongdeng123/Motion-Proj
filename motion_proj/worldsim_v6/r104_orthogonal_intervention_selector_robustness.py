"""WorldSim V6 R104: selector robustness under an orthogonal actor intervention."""
...
TASK_ID = "WS-V6-R104-ORTHOGONAL-INTERVENTION-SELECTOR-ROBUSTNESS-01"


class R104ExperimentError(RuntimeError):
    """The preregistered R104 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R104ExperimentError("formal R104 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R104ExperimentError("R104 task_id drift")
    ...
    r102_run = _resolve_runs_uri(sources["r102_run"])
    ...
    frozen_files = {
        ...
        r101_run / "PROPOSAL_CATALOG.jsonl": sources["r101_proposal_catalog_sha256"],
        r101_package / "PACKAGE_MANIFEST.json": sources["r101_package_manifest_sha256"],
        r102_run / "MANIFEST.json": sources["r102_manifest_sha256"],
        r102_run / "R102_GATE.json": sources["r102_gate_sha256"],
        r102_run / "SUMMARY.json": sources["r102_summary_sha256"],
        r102_run / "SELECTOR_TRANSFER.json": sources["r102_selector_transfer_sha256"],
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R104ExperimentError("StreetGS upstream commit drift")
    free_gib = shutil.disk_usage(run_root).free / (1024**3)
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R104ExperimentError("R104 disk resource insufficient")
    r101_gate = json.loads((r101_run / "R101_GATE.json").read_text())
    r102_gate = json.loads((r102_run / "R102_GATE.json").read_text())
    r90_gate = json.loads((r90_run / "R90_GATE.json").read_text())
    selected = json.loads((r101_run / "SELECTED_PROPOSAL.json").read_text())["selected"]
    candidate_rows = [
        row for row in _load_rows(r101_run / "PROPOSAL_CATALOG.jsonl")
        if row["proposal_id"] == runtime["proposal_id"]
    ]
    if len(candidate_rows) != 1:
        raise R104ExperimentError("R104 orthogonal candidate binding drift")
    ...
    if len(frame_indices) != int(runtime["expected_frame_count"]):
        raise R104ExperimentError("R104 frame denominator drift")

    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__scene0255-orthogonal-intervention-s{config['seed']}-r1"
    ...
    geometry = {
        "schema_version": "worldsim_v6.r104_transform_owned_actor_geometry.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r104_actor_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r104_actor_package_manifest.v1",
            "files": {
    ...
    composition = {
        "schema_version": "worldsim_v6.r104_scene_composition.v1",
        "scene": runtime["scene"],
        "actors": [
            {
    ...
    runtime_contract = {
        "schema_version": "worldsim_v6.r104_runtime_contract.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r104_scene_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r104_scene_package_manifest.v1",
            "files": {
    ...
    if [row["frame_index"] for row in sensor_rows] != frame_indices:
        raise R104ExperimentError("R104 sensor output denominator/order drift")

    perception_dir = run_dir / "perception"
    variants = list(evaluation["variants"])
    if variants != ["logged", "edited"]:
        raise R104ExperimentError("R104 variant order drift")
    index_rows = [
        {
    ...
    transfer = {
        "schema_version": "worldsim_v6.r104_orthogonal_intervention_selector.v1",
    ...
    checks = {
        "r101_r102_and_r90_authorities_accepted": bool(
            r101_gate["checks"]["passed"] and r102_gate["checks"]["passed"] and r90_gate["checks"]["passed"]
        ),
        "r101_accepted_orthogonal_candidate_exact": candidate["proposal_id"] == runtime["proposal_id"]
        ...
        "orthogonal_equal_norm_to_r102_intervention_exact": abs(
            sum(float(a) * float(b) for a, b in zip(selected["translation_delta_m"], runtime["translation_delta_m"]))
        ) <= 1e-12 and abs(
    ...
    _write_json(
        run_dir / "R104_GATE.json",
        {
            "schema_version": "worldsim_v6.r104_gate.v1", "checks": checks,
            "decision": "accept_orthogonal_intervention_selector_robustness"
            if checks["passed"] else "reject_orthogonal_intervention_selector_robustness",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r104_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r104_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_orthogonal_intervention_selector_robustness"
    ...
    tracked = [
        "R104_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SELECTOR_TRANSFER.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r104_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r104_orthogonal_intervention_selector_robustness_v1.yaml"),
    )
