"""WorldSim V6 R98: zero-calibration selector transfer to scene0048."""
...
TASK_ID = "WS-V6-R98-SECOND-INDEPENDENT-SCENE-SELECTOR-TRANSFER-01"


class R98ExperimentError(RuntimeError):
    """The preregistered R98 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R98ExperimentError("formal R98 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R98ExperimentError("R98 task_id drift")
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R98ExperimentError("StreetGS upstream commit drift")
    free_gib = shutil.disk_usage(run_root).free / (1024**3)
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R98ExperimentError("R98 disk resource insufficient")
    ...
    if len(frame_indices) != int(runtime["expected_frame_count"]):
        raise R98ExperimentError("R98 frame denominator drift")

    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__scene0048-selector-transfer-s{config['seed']}-r1"
    ...
    geometry = {
        "schema_version": "worldsim_v6.r98_transform_owned_actor_geometry.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r98_actor_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r98_actor_package_manifest.v1",
            "files": {
    ...
    composition = {
        "schema_version": "worldsim_v6.r98_scene_composition.v1",
        "scene": runtime["scene"],
        "actors": [
            {
    ...
    runtime_contract = {
        "schema_version": "worldsim_v6.r98_runtime_contract.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r98_scene_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r98_scene_package_manifest.v1",
            "files": {
    ...
    if [row["frame_index"] for row in sensor_rows] != frame_indices:
        raise R98ExperimentError("R98 sensor output denominator/order drift")

    perception_dir = run_dir / "perception"
    variants = list(evaluation["variants"])
    if variants != ["logged", "edited"]:
        raise R98ExperimentError("R98 variant order drift")
    index_rows = [
        {
    ...
    transfer = {
        "schema_version": "worldsim_v6.r98_selector_transfer.v1",
    ...
    checks = {
        "r97_and_r90_authorities_accepted": bool(r97_gate["checks"]["passed"] and r90_gate["checks"]["passed"]),
        "r97_selected_edit_exact": selected["proposal_id"] == runtime["proposal_id"]
    ...
    _write_json(
        run_dir / "R98_GATE.json",
        {
            "schema_version": "worldsim_v6.r98_gate.v1", "checks": checks,
            "decision": "accept_zero_calibration_second_independent_scene_selector_transfer"
            if checks["passed"] else "reject_second_independent_scene_selector_transfer",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r98_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r98_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_zero_calibration_second_independent_scene_selector_transfer"
    ...
    tracked = [
        "R98_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SELECTOR_TRANSFER.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r98_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r98_second_independent_scene_selector_transfer_v1.yaml"),
    )
