"""WorldSim V6 R93: compile and visibly edit actor12 in independent scene0230."""
...
TASK_ID = "WS-V6-R93-INDEPENDENT-SCENE-ACTOR-EDIT-COMPILER-01"


class R93ExperimentError(RuntimeError):
    """The preregistered R93 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R93ExperimentError("formal R93 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R93ExperimentError("R93 task_id drift")
    sources = config["sources"]
    target = config["target"]
    search = config["search"]
    thresholds = config["thresholds"]
    resources = config["resources"]
    r92_run = _resolve_runs_uri(sources["r92_run"])
    r2_run = _resolve_runs_uri(sources["r2_run"])
    base_package = r2_run / sources["base_sceneir_package"]
    checkpoint = Path(sources["streetgs_checkpoint"])
    upstream = Path(sources["streetgs_upstream_root"])
    inventory_path = r92_run / "worker/ACTOR_INVENTORY.json"
    selection_path = r92_run / "ACTOR_VISIBILITY_SELECTION.json"
    lifecycle_path = r92_run / "worker" / f"ACTOR_{int(target['actor_model_index']):04d}_FRAME_VALID.npy"
    frozen_files = {
        r92_run / "MANIFEST.json": sources["r92_manifest_sha256"],
        r92_run / "R92_GATE.json": sources["r92_gate_sha256"],
        r92_run / "SUMMARY.json": sources["r92_summary_sha256"],
        selection_path: sources["r92_selection_sha256"],
        inventory_path: sources["r92_inventory_sha256"],
        lifecycle_path: target["lifecycle_sha256"],
        r2_run / "MANIFEST.json": sources["r2_manifest_sha256"],
        r2_run / "SUMMARY.json": sources["r2_summary_sha256"],
        r2_run / "REPRESENTATION_EQUIVALENCE.json": sources["r2_representation_equivalence_sha256"],
        r2_run / "FRESH_PROCESS_RELOAD.json": sources["r2_fresh_process_reload_sha256"],
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R93ExperimentError("StreetGS upstream commit drift")
    if shutil.disk_usage(run_root).free / (1024**3) < float(resources["minimum_disk_free_gib"]):
        raise R93ExperimentError("R93 disk resource insufficient")
    r92_gate = json.loads((r92_run / "R92_GATE.json").read_text())
    r2_summary = json.loads((r2_run / "SUMMARY.json").read_text())
    selection = json.loads(selection_path.read_text())
    inventory = json.loads(inventory_path.read_text())
    document = json.loads((base_package / "sceneir.json").read_text())
    source_manifest = json.loads((base_package / "MANIFEST.json").read_text())
    actor_index = int(target["actor_model_index"])
    inventory_actor = inventory["actors"][actor_index]
    ...
    if set(chunk["arrays"]) != REQUIRED_ARRAYS:
        raise R93ExperimentError("actor12 Gaussian array set drift")
    transforms_by_key = {
    ...
    for row in actor["trajectory"]:
        ...
        if key not in transforms_by_key or timestamp not in visibility:
            raise R93ExperimentError(f"actor12 trajectory reference missing: {key}")
    ...
    for name, reference in chunk["arrays"].items():
        ...
        if manifest_row is None or manifest_row["sha256"] != reference["sha256"]:
            raise R93ExperimentError(f"base manifest missing actor12 blob: {name}")
    ...
    for x, z in candidates:
        ...
        proposal_rows.append(
            {
                "proposal_id": f"scene0230_actor12_translate_x_{x:+.1f}_z_{z:+.1f}",
    ...
    if not accepted_rows:
        raise R93ExperimentError("no zero-new-overlap candidate in preregistered grid")
    selected = sorted(
        accepted_rows,
        key=lambda row: (
    ...
    run_dir = run_root / TASK_ID / f"{now}__scene0230-actor12-edit-s{config['seed']}-r1"
    ...
    geometry = {
        "schema_version": "worldsim_v6.r93_trajectory_geometry.v1",
        "asset_id": actor["id"],
        "chunk_id": chunk["id"],
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r93_actor_bundle.v1",
            "frontend_model_index": actor_index,
            "actor": actor,
            "chunk": chunk,
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r93_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r93_package_manifest.v1",
            "files": {
    ...
    _write_json(
        ...
        {"schema_version": "worldsim_v6.r93_selected_proposal.v1", "selection_rule": search["selection_rule"], "selected": selected},
    )
    ...
    def run_worker(label: str, delta: list[float]) -> tuple[dict, dict, Path]:
        ...
        if len(rows) != 1:
            raise R93ExperimentError(f"{label} sensor denominator drift")
    ...
    comparison = {
        "schema_version": "worldsim_v6.r93_edit_sensor_comparison.v1",
    ...
    checks = {
        "r92_and_r2_authorities_accepted": bool(r92_gate["checks"]["passed"])
        and r2_summary["status"] == "done" and r2_summary["fresh_process_reload_passed"]
        and r2_summary["representation_equivalence_passed"],
        "r92_selection_exact": selection["selected_actor_model_index"] == actor_index
        ...
        "baseline_native_visibility_reproduces_r92": baseline_row["actor_effect_pixels"]
        ...
        "package_checkpoint_and_sources_immutable": all(
    ...
    _write_json(
        run_dir / "R93_GATE.json",
        {
            "schema_version": "worldsim_v6.r93_gate.v1", "checks": checks,
            "decision": "accept_independent_scene_actor_edit_compiler"
            if checks["passed"] else "reject_or_repair_independent_scene_actor_edit_compiler",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r93_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r93_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_independent_scene_actor_edit_compiler"
    ...
    tracked = [
        "R93_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "PROPOSAL_CATALOG.jsonl",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r93_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r93_independent_scene_actor_edit_compiler_v1.yaml"),
    )
