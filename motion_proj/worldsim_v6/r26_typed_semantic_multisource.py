"""WorldSim V6 双架构 typed semantic consensus 正式实验。"""
...
R21_TASK_ID = "WS-V6-R21-TYPED-SEMANTIC-CONTRACT-01"
R26_TASK_ID = "WS-V6-R26-TYPED-SEMANTIC-MULTISOURCE-01"
ALLOWED_TASK_IDS = {TASK_ID, R21_TASK_ID, R26_TASK_ID}
...
def run_experiment(
    ...
    source_run = _resolve_runs_uri(sources.get("proposal_run", sources.get("r16_run")))
    proposal_gate_file = str(sources.get("proposal_gate_file", "R16_GATE.json"))
    per_case_file = str(
        sources.get("proposal_per_case_file", "verifier_worker/PER_CASE_ARMS.jsonl")
    )
    source_files = {
        source_run / "MANIFEST.json": sources.get(
            "proposal_manifest_sha256", sources.get("r16_manifest_sha256")
        ),
        source_run / proposal_gate_file: sources.get(
            "proposal_gate_sha256", sources.get("r16_gate_sha256")
        ),
        source_run / "CASES.jsonl": sources.get(
            "proposal_cases_sha256", sources.get("r16_cases_sha256")
        ),
        source_run / per_case_file: sources.get(
            "proposal_per_case_arms_sha256", sources.get("r16_per_case_arms_sha256")
        ),
    ...
    evaluation_run = None
    if sources.get("evaluation_run"):
        evaluation_run = _resolve_runs_uri(sources["evaluation_run"])
        source_files.update(
            {
                evaluation_run / "MANIFEST.json": sources["evaluation_manifest_sha256"],
                evaluation_run / "R25_GATE.json": sources["evaluation_gate_sha256"],
                evaluation_run / "ACTOR_ARM_PARETO.jsonl": sources[
                    "evaluation_actor_pareto_sha256"
                ],
            }
        )
    ...
    if task_id == R26_TASK_ID:
        proposal_gate = json.loads((source_run / proposal_gate_file).read_text(encoding="utf-8"))
        evaluation_gate = json.loads(
            (evaluation_run / "R25_GATE.json").read_text(encoding="utf-8")
        )
        if proposal_gate.get("checks", {}).get("passed") is not False:
            raise R20ExperimentError("R26 必须保留 R24 rejected 状态")
        if not evaluation_gate.get("checks", {}).get("passed"):
            raise R20ExperimentError("R26 缺少通过的 R25 actor arm authority")
        if set(evaluation_gate.get("eligible_actor_arms", [])) != {"P1", "P2"}:
            raise R20ExperimentError("R26 R25 eligible actor arms 漂移")
    ...
    try:
        ...
        source_rows = _read_jsonl(source_run / per_case_file)
        ...
        gate_name = {
            TASK_ID: "R20_GATE.json",
            R21_TASK_ID: "R21_GATE.json",
            R26_TASK_ID: "R26_GATE.json",
        }[task_id]
