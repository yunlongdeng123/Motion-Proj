"""WorldSim V6 R56：将 actor2 编译到全部 logged world poses。"""
...
TASK_ID = "WS-V6-R56-SECOND-ACTOR-RIGID-TRAJECTORY-01"


class R56ExperimentError(RuntimeError):
    """R56 正式实验合同失败。"""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R56ExperimentError("正式 R56 run 禁止 dirty source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R56ExperimentError("R56 task_id 漂移")
    sources = config["sources"]
    r55_run = _resolve_runs_uri(sources["r55_run"])
    source_package = r55_run / "package"
    frozen_files = {
        r55_run / "MANIFEST.json": sources["r55_manifest_sha256"],
        r55_run / "R55_GATE.json": sources["r55_gate_sha256"],
        r55_run / "SUMMARY.json": sources["r55_summary_sha256"],
        source_package / "PACKAGE_MANIFEST.json": sources["r55_package_manifest_sha256"],
        source_package / "ACTOR_BUNDLE.json": sources["r55_actor_bundle_sha256"],
    }
    ...
    for row in actor["trajectory"]:
        ...
        if key not in transforms or timestamp not in visibility:
            raise R56ExperimentError(f"actor2 轨迹引用缺失：{key}")
    ...
    run_dir = run_root / TASK_ID / f"{now}__actor2-trajectory-s{config['seed']}-r1"
    ...
    geometry = {
        "schema_version": "worldsim_v6.r56_trajectory_geometry.v1", "asset_id": actor["id"], "chunk_id": chunk["id"],
    ...
    validity = {
        "schema_version": "worldsim_v6.r56_validity.v1", "identity_binding": "OBSERVED_MODEL_INDEX_BOUND",
    ...
    _write_json(package / "RUNTIME_DEPENDENCY_AUDIT.json", {
        "schema_version": "worldsim_v6.r56_runtime_dependency_audit.v1", "runtime_dependencies": ["json_reader", "numpy_npy_reader"],
    ...
    _write_json(package / "PACKAGE_MANIFEST.json", {"schema_version": "worldsim_v6.r56_package_manifest.v1", "files": {name: {"bytes": (package / name).stat().st_size, "sha256": _sha256(package / name)} for name in package_files}})
    timestamps = [row["timestamp_us"] for row in trajectory_metadata]
    checks = {
        "r55_authority_accepted": bool(r55_gate["checks"]["passed"]),
    ...
    _write_json(run_dir / "R56_GATE.json", {"schema_version": "worldsim_v6.r56_gate.v1", "checks": checks, "decision": "accept_second_actor_rigid_trajectory" if checks["passed"] else "reject_or_repair_second_actor_trajectory"})
    _write_json(run_dir / "SUMMARY.json", {
        "schema_version": "worldsim_v6.r56_summary.v1", "task_id": TASK_ID, "hypothesis_id": config["hypothesis_id"], "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_second_actor_rigid_trajectory" if checks["passed"] else "rejected", "source_commit": source_commit,
    ...
    tracked = ["R56_GATE.json", "SUMMARY.json", "package/PACKAGE_MANIFEST.json", *[f"package/{name}" for name in package_files]]
    _write_json(run_dir / "MANIFEST.json", {"schema_version": "worldsim_v6.r56_manifest.v1", "files": {name: {"bytes": (run_dir / name).stat().st_size, "sha256": _sha256(run_dir / name)} for name in tracked}})
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/worldsim_v6/r56_second_actor_rigid_trajectory_v1.yaml"))
