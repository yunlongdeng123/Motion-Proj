"""WorldSim V6 R109: two-actor compositional selector on scene0255."""
...
TASK_ID = "WS-V6-R109-SCENE0255-TWO-ACTOR-COMPOSITIONAL-SELECTOR-01"


class R109ExperimentError(RuntimeError):
    """The preregistered R109 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R109ExperimentError("formal R109 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R109ExperimentError("R109 task_id drift")
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R109ExperimentError("StreetGS upstream commit drift")
    free_gib = shutil.disk_usage(run_root).free / (1024**3)
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R109ExperimentError("R109 disk resource insufficient")
    ...
    source_packages = {"actor_0034": r101_package, "actor_0024": r108_package}
    ...
    if len(frame_indices) != int(runtime["expected_frame_count"]):
        raise R109ExperimentError("R109 frame denominator drift")

    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__scene0255-two-actor-selector-s{config['seed']}-r1"
    run_dir.mkdir(parents=True, exist_ok=False)

    actor_configs = list(runtime["actors"])
    actor_ids = [str(row["actor_id"]) for row in actor_configs]
    if actor_ids != ["actor_0034", "actor_0024"] or len(set(actor_ids)) != 2:
        raise R109ExperimentError("R109 actor order/denominator drift")
    ...
    for actor in actor_configs:
        ...
        if len(base_actor) != len(edited_actor) or not base_actor:
            raise R109ExperimentError(f"R109 {actor_id} SceneIR trajectory denominator drift")
    ...
    joint_interaction = {
        "schema_version": "worldsim_v6.r109_joint_interaction_factor.v1",
        "scene": runtime["scene"],
    ...
    for actor in actor_configs:
        ...
        geometry = {
            "schema_version": "worldsim_v6.r109_transform_owned_actor_geometry.v1",
        ...
        _write_json(
            ...
            {
                "schema_version": "worldsim_v6.r109_actor_validity.v1",
        ...
        _write_json(
            ...
            {
                "schema_version": "worldsim_v6.r109_actor_package_manifest.v1",
                "files": {
                    name: {
    ...
    composition = {
        "schema_version": "worldsim_v6.r109_scene_composition.v1",
        "scene": runtime["scene"],
        "actors": [
            {
                "actor_id": str(actor["actor_id"]),
    ...
    runtime_contract = {
        "schema_version": "worldsim_v6.r109_runtime_contract.v1",
        "runtime_mode": "two_actor_transform_lifecycle_owned_scene_patch",
        "actor_order": actor_ids,
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r109_scene_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r109_scene_package_manifest.v1",
            "files": {
    ...
    if [row["frame_index"] for row in sensor_rows] != frame_indices:
        raise R109ExperimentError("R109 sensor output denominator/order drift")

    perception_dir = run_dir / "perception"
    variants = list(evaluation["variants"])
    if variants != ["logged", "edited"]:
        raise R109ExperimentError("R109 variant order drift")
    index_rows = [
        {
    ...
    transfer = {
        "schema_version": "worldsim_v6.r109_selector_transfer.v1",
    ...
    checks = {
        "r101_r108_and_r90_authorities_accepted": bool(
            r101_gate["checks"]["passed"]
            and r108_gate["checks"]["passed"]
            and r90_gate["checks"]["passed"]
        ),
        "two_independently_compiled_selected_edits_exact": all(
        ...
        "all196_two_actor_compiled_native_sensor_numerics_conformant": all(
        ...
        "two_actor_primitive_counts_and_translations_exact_all_frames": all(
            all(
        ...
        "scene_and_two_nested_package_manifests_immutable": sensor_audit[
    ...
    _write_json(
        run_dir / "R109_GATE.json",
        {
            "schema_version": "worldsim_v6.r109_gate.v1", "checks": checks,
            "decision": "accept_two_actor_compositional_selector_transfer"
            if checks["passed"] else "reject_two_actor_compositional_selector_transfer",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r109_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r109_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_two_actor_compositional_selector_transfer"
    ...
    tracked = [
        "R109_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SELECTOR_TRANSFER.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r109_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r109_scene0255_two_actor_compositional_selector_v1.yaml"),
    )
