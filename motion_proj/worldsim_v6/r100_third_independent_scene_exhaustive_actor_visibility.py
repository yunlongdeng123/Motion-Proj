"""WorldSim V6 R100: exhaustive actor visibility on third independent scene0255."""
...
TASK_ID = "WS-V6-R100-THIRD-INDEPENDENT-SCENE-EXHAUSTIVE-ACTOR-VISIBILITY-01"


class R100ExperimentError(RuntimeError):
    """The preregistered R100 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R100ExperimentError("formal R100 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R100ExperimentError("R100 task_id drift")
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R100ExperimentError("StreetGS upstream commit drift")
    if shutil.disk_usage(run_root).free / (1024**3) < float(resources["minimum_disk_free_gib"]):
        raise R100ExperimentError("R100 disk resource insufficient")
    ...
    run_dir = run_root / TASK_ID / f"{now}__scene0255-exhaustive-actor-visibility-s{config['seed']}-r1"
    ...
    if not active_rows:
        raise R100ExperimentError("no active visibility rows")
    selected = sorted(
        active_rows,
        key=lambda row: (-row["actor_effect_pixels"], row["actor_model_index"], row["frame_index"]),
    )[0]
    decision = {
        "schema_version": "worldsim_v6.r100_third_independent_scene_actor_selection.v1",
    ...
    checks = {
        ...
        "exhaustive_all196_front_frames_exact": visibility["probe_frames"] == probe_frames
        and probe_frames == list(range(int(contract["expected_frame_count"]))),
    ...
    _write_json(
        run_dir / "R100_GATE.json",
        {"schema_version": "worldsim_v6.r100_gate.v1", "checks": checks,
         "decision": "accept_third_independent_scene_actor_for_compiler_extension"
         if checks["passed"] else "reject_or_expand_third_independent_scene_camera_coverage"},
    )
    summary = {
        "schema_version": "worldsim_v6.r100_summary.v1", "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"], "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_third_independent_scene_exhaustive_actor_visibility_selection"
    ...
    tracked = [
        "R100_GATE.json", "SUMMARY.json", "ACTOR_VISIBILITY_SELECTION.json", "worker.log",
    ...
    _write_json(
        ...
        {"schema_version": "worldsim_v6.r100_manifest.v1", "files": {
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/worldsim_v6/r100_third_independent_scene_exhaustive_actor_visibility_v1.yaml"))
