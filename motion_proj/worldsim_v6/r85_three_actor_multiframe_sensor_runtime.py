"""WorldSim V6 R85: execute a three-actor scene package across lifecycle boundaries."""
...
TASK_ID = "WS-V6-R85-THREE-ACTOR-MULTIFRAME-SENSOR-RUNTIME-01"


class R85ExperimentError(RuntimeError):
    """The preregistered R85 experiment contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R85ExperimentError("formal R85 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R85ExperimentError("R85 task_id drift")
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R85ExperimentError("StreetGS upstream commit drift")
    free_gib = shutil.disk_usage(run_root).free / (1024**3)
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R85ExperimentError("R85 disk resource insufficient")
    ...
    frame_indices = [int(value) for value in contract["frame_indices"]]
    if frame_indices != sorted(set(frame_indices)) or len(frame_indices) != int(
        contract["expected_frame_count"]
    ):
        raise R85ExperimentError("R85 frame denominator drift")
    ...
    run_dir = run_root / TASK_ID / f"{now}__three-actor-multiframe-s{config['seed']}-r1"
    ...
    try:
        ...
        if completed.returncode != 0:
            raise R85ExperimentError(
                f"three-actor multiframe sensor worker failed: rc={completed.returncode}"
            )
        rows = [
            json.loads(line)
        ...
        if [int(row["frame_index"]) for row in rows] != frame_indices:
            raise R85ExperimentError("R85 output frame denominator drift")
        ...
        for frame_position, row in enumerate(rows):
            ...
            runtime_binding_pass = runtime_binding_pass and bool(
                ...
                and all(
                    ...
                    == bool(contract["expected_frame_validity"][actor_id][frame_position])
        ...
        checks = {
            ...
            "seven_frame_denominator_exact": len(rows)
            ...
            "actor0_and_actor5_lifecycle_boundaries_covered": all(
                any(validity_sequences[actor_id]) and not all(validity_sequences[actor_id])
                for actor_id in contract["required_boundary_actor_ids"]
            ),
        ...
        _write_json(
            run_dir / "R85_GATE.json",
            {
                "schema_version": "worldsim_v6.r85_gate.v1",
                "checks": checks,
                "decision": "accept_three_actor_multiframe_sensor_runtime"
                if checks["passed"]
                else "reject_or_repair_three_actor_multiframe_sensor_runtime",
            },
        )
        _write_json(
            run_dir / "MULTIFRAME_SENSOR_EFFECT.json",
            {
                "schema_version": "worldsim_v6.r85_multiframe_sensor_effect.v1",
        ...
        _write_json(
            ...
            {
                "schema_version": "worldsim_v6.r85_resource_audit.v1",
                "gpu_used": True,
        ...
        summary = {
            "schema_version": "worldsim_v6.r85_summary.v1",
            "task_id": TASK_ID,
            "hypothesis_id": config["hypothesis_id"],
            "status": "done" if checks["passed"] else "rejected",
            "hypothesis_outcome": "accepted_development_three_actor_multiframe_sensor_runtime"
            if checks["passed"]
            else "rejected",
        ...
        _write_json(
            ...
            {
                "schema_version": "worldsim_v6.r85_manifest.v1",
                "files": {
                    name: {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/worldsim_v6/r85_three_actor_multiframe_sensor_runtime_v1.yaml"),
    )
    parser.add_argument(
