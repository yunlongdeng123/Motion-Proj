"""WorldSim V6 R74: compile the R73-selected third observed SceneIR actor bundle."""
...
TASK_ID = "WS-V6-R74-THIRD-ACTOR-SCENEIR-BUNDLE-01"
REQUIRED_ARRAYS = {"means_m", "scales_m", "quaternions_wxyz", "opacities", "features_dc", "features_rest", "source_indices"}


class R74ExperimentError(RuntimeError):
    """The preregistered R74 experiment contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R74ExperimentError("formal R74 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R74ExperimentError("R74 task_id drift")
    sources = config["sources"]
    r54_run = _resolve_runs_uri(sources["r54_run"])
    r73_run = _resolve_runs_uri(sources["r73_run"])
    ...
    frozen_files = {
        ...
        r73_run / "MANIFEST.json": sources["r73_manifest_sha256"],
        r73_run / "R73_GATE.json": sources["r73_gate_sha256"],
        r73_run / "SUMMARY.json": sources["r73_summary_sha256"],
        r73_run / "THIRD_ACTOR_SELECTION.json": sources["r73_selection_sha256"],
    ...
    if shutil.disk_usage(run_root).free / (1024**3) < float(config["resources"]["minimum_disk_free_gib"]):
        raise R74ExperimentError("R74 disk resource insufficient")
    r54_gate = json.loads((r54_run / "R54_GATE.json").read_text(encoding="utf-8"))
    r73_gate = json.loads((r73_run / "R73_GATE.json").read_text(encoding="utf-8"))
    r73_selection = json.loads((r73_run / "THIRD_ACTOR_SELECTION.json").read_text(encoding="utf-8"))
    ...
    selected_index = int(r73_selection["selected_actor_model_index"])
    selected = next(actor for actor in inventory["actors"] if actor["actor_model_index"] == selected_index)
    ...
    run_dir = run_root / TASK_ID / f"{now}__actor5-bundle-s{config['seed']}-r1"
    ...
    for name, reference in sorted(chunk["arrays"].items()):
        ...
        if manifest_row is None or manifest_row["sha256"] != reference["sha256"]:
            raise R74ExperimentError(f"base manifest missing actor5 blob: {name}")
    ...
    bundle = {
        "schema_version": "worldsim_v6.r74_third_actor_bundle.v1",
    ...
    _write_json(package / "VALIDITY.json", {
        "schema_version": "worldsim_v6.r74_validity.v1", "asset_id": actor["id"], "identity_binding": "OBSERVED_MODEL_INDEX_BOUND",
    ...
    _write_json(package / "RUNTIME_DEPENDENCY_AUDIT.json", {
        "schema_version": "worldsim_v6.r74_runtime_dependency_audit.v1", "runtime_dependencies": ["json_reader", "numpy_npy_reader"],
    ...
    _write_json(package / "PACKAGE_MANIFEST.json", {"schema_version": "worldsim_v6.r74_package_manifest.v1", "files": {name: {"bytes": (package / name).stat().st_size, "sha256": _sha256(package / name)} for name in package_files}})
    checks = {
        "r54_r73_and_sceneir_binding_accepted": bool(r54_gate["checks"]["passed"] and r73_gate["checks"]["passed"] and binding_gate["checks"]["passed"]),
        "selection_rule_exact": selected_index == int(selection["expected_actor_model_index"]) == int(r73_selection["selected_actor_model_index"]),
    ...
    _write_json(run_dir / "R74_GATE.json", {"schema_version": "worldsim_v6.r74_gate.v1", "checks": checks, "decision": "accept_third_actor_sceneir_bundle" if checks["passed"] else "reject_or_repair_third_actor_bundle"})
    _write_json(run_dir / "SUMMARY.json", {
        "schema_version": "worldsim_v6.r74_summary.v1", "task_id": TASK_ID, "hypothesis_id": config["hypothesis_id"], "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_third_actor_sceneir_bundle" if checks["passed"] else "rejected", "source_commit": source_commit,
    ...
    tracked = ["R74_GATE.json", "SUMMARY.json", "package/PACKAGE_MANIFEST.json", *[f"package/{name}" for name in package_files]]
    _write_json(run_dir / "MANIFEST.json", {"schema_version": "worldsim_v6.r74_manifest.v1", "files": {name: {"bytes": (run_dir / name).stat().st_size, "sha256": _sha256(run_dir / name)} for name in tracked}})
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/worldsim_v6/r74_third_actor_sceneir_bundle_v1.yaml"))
