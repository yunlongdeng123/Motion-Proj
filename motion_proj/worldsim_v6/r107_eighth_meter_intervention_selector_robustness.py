"""WorldSim V6 R107: selector robustness at an eighth-meter intervention."""
...
TASK_ID = "WS-V6-R107-EIGHTH-METER-INTERVENTION-SELECTOR-ROBUSTNESS-01"


class R107ExperimentError(RuntimeError):
    """The preregistered R107 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R107ExperimentError("formal R107 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R107ExperimentError("R107 task_id drift")
    ...
    r106_run = _resolve_runs_uri(sources["r106_run"])
    ...
    frozen_files = {
        ...
        r106_run / "MANIFEST.json": sources["r106_manifest_sha256"],
        r106_run / "R106_GATE.json": sources["r106_gate_sha256"],
        r106_run / "SUMMARY.json": sources["r106_summary_sha256"],
        r106_run / "SELECTOR_TRANSFER.json": sources["r106_selector_transfer_sha256"],
        r106_run / "INTERACTION_FACTOR.json": sources["r106_interaction_factor_sha256"],
        r106_run / "scene_package/SCENE_COMPOSITION.json": sources["r106_scene_composition_sha256"],
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R107ExperimentError("StreetGS upstream commit drift")
    free_gib = shutil.disk_usage(run_root).free / (1024**3)
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R107ExperimentError("R107 disk resource insufficient")
    r101_gate = json.loads((r101_run / "R101_GATE.json").read_text())
    r106_gate = json.loads((r106_run / "R106_GATE.json").read_text())
    r105_gate = json.loads((r105_run / "R105_GATE.json").read_text())
    r90_gate = json.loads((r90_run / "R90_GATE.json").read_text())
    r106_composition = json.loads((r106_run / "scene_package/SCENE_COMPOSITION.json").read_text())
    ...
    interaction_factor = {
        "schema_version": "worldsim_v6.r107_eighth_meter_interaction_factor.v1",
    ...
    if len(frame_indices) != int(runtime["expected_frame_count"]):
        raise R107ExperimentError("R107 frame denominator drift")

    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__scene0255-eighth-meter-intervention-s{config['seed']}-r1"
    ...
    geometry = {
        "schema_version": "worldsim_v6.r107_transform_owned_actor_geometry.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r107_actor_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r107_actor_package_manifest.v1",
            "files": {
    ...
    composition = {
        "schema_version": "worldsim_v6.r107_scene_composition.v1",
        "scene": runtime["scene"],
        "actors": [
            {
    ...
    runtime_contract = {
        "schema_version": "worldsim_v6.r107_runtime_contract.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r107_scene_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r107_scene_package_manifest.v1",
            "files": {
    ...
    if [row["frame_index"] for row in sensor_rows] != frame_indices:
        raise R107ExperimentError("R107 sensor output denominator/order drift")

    perception_dir = run_dir / "perception"
    variants = list(evaluation["variants"])
    if variants != ["logged", "edited"]:
        raise R107ExperimentError("R107 variant order drift")
    index_rows = [
        {
    ...
    transfer = {
        "schema_version": "worldsim_v6.r107_eighth_meter_intervention_selector.v1",
    ...
    checks = {
        "r101_r106_r105_and_r90_authorities_accepted": bool(
            r101_gate["checks"]["passed"] and r106_gate["checks"]["passed"]
            and r105_gate["checks"]["passed"] and r90_gate["checks"]["passed"]
        ),
        "eighth_meter_interaction_and_self_kinematics_accept": interaction_factor["q_aabb_interaction"] == "ACCEPT"
        ...
        "same_direction_half_magnitude_of_r106_exact": len(r106_composition["actors"]) == 1
        and r106_composition["actors"][0]["actor_id"] == runtime["actor_id"]
        and np.allclose(
            np.asarray(r106_composition["actors"][0]["translation_delta_m"], dtype=np.float64),
    ...
    _write_json(
        run_dir / "R107_GATE.json",
        {
            "schema_version": "worldsim_v6.r107_gate.v1", "checks": checks,
            "decision": "accept_eighth_meter_intervention_selector_robustness"
            if checks["passed"] else "reject_eighth_meter_intervention_selector_robustness",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r107_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r107_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_eighth_meter_intervention_selector_robustness"
    ...
    tracked = [
        "R107_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SELECTOR_TRANSFER.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r107_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r107_eighth_meter_intervention_selector_robustness_v1.yaml"),
    )
