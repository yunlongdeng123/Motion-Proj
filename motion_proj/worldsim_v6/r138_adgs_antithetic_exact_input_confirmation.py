"""WorldSim V6 R138: exact-once antithetic AD-GS exact-input reuse confirmation."""
...
TASK_ID = "WS-V6-R138-ADGS-ANTITHETIC-EXACT-INPUT-CONFIRMATION-01"


class R138ExperimentError(RuntimeError):
    """The preregistered exact-once R138 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R138ExperimentError("formal R138 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R138ExperimentError("R138 task_id drift")
    ...
    if _git(adgs_root, "rev-parse", "HEAD") != sources["adgs_implementation_commit"]:
        raise R138ExperimentError("R138 AD-GS implementation commit drift")
    if not adgs_source.is_dir() or not training_adapter.is_dir():
        raise R138ExperimentError("R138 AD-GS adapter source preflight failed")
    ...
    if not r137_gate["checks"]["passed"]:
        raise R138ExperimentError("R137 development authority is not accepted")
    free_gib = shutil.disk_usage(run_root).free / (1024**3)
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R138ExperimentError("R138 disk resource insufficient")

    task_root = run_root / TASK_ID
    if task_root.exists() and any(task_root.iterdir()):
        raise R138ExperimentError("R138 exact-once confirmation attempt already exists")
    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = task_root / f"{now}__adgs-antithetic-exact-input-s{config['seed']}-r1"
    run_dir.mkdir(parents=True, exist_ok=False)
    attempt = {
        "schema_version": "worldsim_v6.r138_confirmation_attempt.v1",
        "task_id": TASK_ID,
    ...
    if len(frames) != int(runtime["expected_heldout_frames"]):
        raise R138ExperimentError("R138 heldout frame denominator drift after attempt consumption")
    adapter_tree_sha256 = _tree_sha256(adapter)
    _write_json(
        run_dir / "HELDOUT_ADAPTER_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r138_heldout_adapter_audit.v1",
    ...
    if [int(row["frame_index"]) for row in sensor_rows] != frames:
        raise R138ExperimentError("R138 heldout sensor row order drift")
    ...
    for row in sensor_rows:
        ...
        if changed_pixels != int(row["edited_vs_logged_rgb_changed_pixels"]):
            raise R138ExperimentError(f"R138 sensor feature drift at frame {frame}")
        decisions.append(
            {
                "frame_index": frame,
    ...
    checks = {
        ...
        "antithetic_edit_and_actor_state_restoration_exact": all(
    ...
    result = {
        "schema_version": "worldsim_v6.r138_adgs_antithetic_exact_input_confirmation.v1",
    ...
    _write_json(run_dir / "ANTITHETIC_CONFIRMATION.json", result)
    _write_json(
        run_dir / "R138_GATE.json",
        {
            "schema_version": "worldsim_v6.r138_gate.v1",
            "checks": checks,
            "decision": "accept_adgs_antithetic_exact_input_confirmation"
            if checks["passed"]
            else "reject_adgs_antithetic_exact_input_candidate_consumed",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r138_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r138_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_adgs_antithetic_exact_input_confirmation"
        if checks["passed"]
        else "rejected_adgs_antithetic_exact_input_candidate_consumed",
        "source_commit": source_commit,
        "attempt_sha256": attempt_sha256,
        "confirmation_consumed": True,
        "frame_count": len(frames),
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r138_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "configs/worldsim_v6/r138_adgs_antithetic_exact_input_confirmation_v1.yaml"
        ),
    )
