"""WorldSim V6 R130: prospective antithetic holdout for revised threshold13."""
...
TASK_ID = "WS-V6-R130-ANTITHETIC-THRESHOLD13-HOLDOUT-01"


class R130ExperimentError(RuntimeError):
    """The preregistered R130 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R130ExperimentError("formal R130 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R130ExperimentError("R130 task_id drift")
    sources = config["sources"]
    runtime = config["runtime"]
    evaluation = config["evaluation"]
    thresholds = config["thresholds"]
    resources = config["resources"]
    r93_run = _resolve_runs_uri(sources["r93_run"])
    r93_package = r93_run / "package"
    r94_run = _resolve_runs_uri(sources["r94_run"])
    r129_run = _resolve_runs_uri(sources["r129_run"])
    r129_package = r129_run / "package_a"
    ...
    frozen_files = {
        r93_run / "MANIFEST.json": sources["r93_manifest_sha256"],
        r93_run / "R93_GATE.json": sources["r93_gate_sha256"],
        r93_run / "SUMMARY.json": sources["r93_summary_sha256"],
        r93_run / "SELECTED_PROPOSAL.json": sources["r93_selected_proposal_sha256"],
        r93_run / "EDIT_SENSOR_COMPARISON.json": sources["r93_edit_sensor_comparison_sha256"],
        r93_run / "PROPOSAL_CATALOG.jsonl": sources["r93_proposal_catalog_sha256"],
        r93_package / "PACKAGE_MANIFEST.json": sources["r93_package_manifest_sha256"],
        r94_run / "MANIFEST.json": sources["r94_manifest_sha256"],
        r94_run / "R94_GATE.json": sources["r94_gate_sha256"],
        r94_run / "SUMMARY.json": sources["r94_summary_sha256"],
        r94_run / "SELECTOR_TRANSFER.json": sources["r94_selector_transfer_sha256"],
        ...
        r129_package / "PACKAGE_MANIFEST.json": sources["r129_package_manifest_sha256"],
        r129_package / "POLICY.json": sources["r129_policy_sha256"],
        ...
        r126_run / "package_a/PACKAGE_MANIFEST.json": sources[
            "r126_package_manifest_sha256"
        ],
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R130ExperimentError("StreetGS upstream commit drift")
    free_gib = shutil.disk_usage(run_root).free / (1024**3)
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R130ExperimentError("R130 disk resource insufficient")
    r93_gate = json.loads((r93_run / "R93_GATE.json").read_text())
    r94_gate = json.loads((r94_run / "R94_GATE.json").read_text())
    r129_gate = json.loads((r129_run / "R129_GATE.json").read_text())
    r125_gate = json.loads((r125_run / "R125_GATE.json").read_text())
    r126_gate = json.loads((r126_run / "R126_GATE.json").read_text())
    r127_gate = json.loads((r127_run / "R127_GATE.json").read_text())
    r128_gate = json.loads((r128_run / "R128_GATE.json").read_text())
    selected = json.loads((r93_run / "SELECTED_PROPOSAL.json").read_text())["selected"]
    candidate_rows = [
        row for row in _load_rows(r93_run / "PROPOSAL_CATALOG.jsonl")
        if row["proposal_id"] == runtime["proposal_id"]
    ]
    if len(candidate_rows) != 1:
        raise R130ExperimentError("R130 orthogonal candidate binding drift")
    candidate = candidate_rows[0]
    policy = json.loads((r129_package / "POLICY.json").read_text())
    source_package_manifest = json.loads((r93_package / "PACKAGE_MANIFEST.json").read_text())
    source_package_files = {}
    for relative, row in source_package_manifest["files"].items():
        path = r93_package / relative
    ...
    if len(frame_indices) != int(runtime["expected_frame_count"]):
        raise R130ExperimentError("R130 frame denominator drift")

    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__antithetic-threshold13-holdout-s{config['seed']}-r1"
    run_dir.mkdir(parents=True, exist_ok=False)
    scene_package = run_dir / "scene_package"
    actor_package = scene_package / "actors" / runtime["actor_id"]
    shutil.copytree(r93_package, actor_package)
    ...
    geometry = {
        "schema_version": "worldsim_v6.r130_transform_owned_actor_geometry.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r130_actor_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r130_actor_package_manifest.v1",
            "files": {
    ...
    composition = {
        "schema_version": "worldsim_v6.r130_scene_composition.v1",
        "scene": runtime["scene"],
        "actors": [
            {
    ...
    runtime_contract = {
        "schema_version": "worldsim_v6.r130_runtime_contract.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r130_scene_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r130_scene_package_manifest.v1",
            "files": {
    ...
    if [row["frame_index"] for row in sensor_rows] != frame_indices:
        raise R130ExperimentError("R130 sensor output denominator/order drift")

    perception_dir = run_dir / "perception"
    variants = list(evaluation["variants"])
    if variants != ["logged", "edited"]:
        raise R130ExperimentError("R130 variant order drift")
    index_rows = [
        {
    ...
    transfer = {
        "schema_version": "worldsim_v6.r130_antithetic_threshold13_holdout.v1",
        "source_policy_id": policy["policy_id"],
        "source_scene": "r129_development_union",
    ...
    checks = {
        "r93_r94_r129_r125_r126_r127_authorities_bound": bool(
            r93_gate["checks"]["passed"]
            and r94_gate["checks"]["passed"]
            and r129_gate["checks"]["passed"]
            and r125_gate["checks"]["passed"]
            and r126_gate["checks"]["passed"]
            and r127_gate["checks"]["passed"]
        ),
        "r128_failure_is_bound_as_r129_development_not_validation": not r128_gate["checks"][
            "passed"
        ]
        and policy["fit_scope"] == "r126_r127_r128_development_union"
        and policy["prospective_validation"] == "PENDING_NEW_CONDITION",
        "r93_accepted_antithetic_candidate_exact": candidate["proposal_id"] == runtime["proposal_id"]
        and candidate["translation_delta_m"] == runtime["translation_delta_m"]
        and candidate["joint_decision"] == "ACCEPT" and int(candidate["new_overlap_events"]) == 0,
        "proposal_is_exact_antithetic_of_r128_development_edit": all(
            abs(float(value) + float(reference)) <= 1e-12
            for value, reference in zip(
                runtime["translation_delta_m"], runtime["r128_translation_delta_m"]
            )
        ),
        "prospective_zminus_is_orthogonal_equal_norm_to_r94_axis_edit": abs(
            sum(
                float(a) * float(b)
                for a, b in zip(selected["translation_delta_m"], runtime["translation_delta_m"])
            )
        ) <= 1e-12
        and abs(
            sum(float(v) ** 2 for v in selected["translation_delta_m"])
            - sum(float(v) ** 2 for v in runtime["translation_delta_m"])
        ) <= 1e-12,
        "r129_policy_bound_without_target_calibration": policy["feature"]
        ...
        "prospective_zero_false_positive_and_false_negative": frozen_metrics["false_positive"] == 0
    ...
    _write_json(
        run_dir / "R130_GATE.json",
        {
            "schema_version": "worldsim_v6.r130_gate.v1", "checks": checks,
            "decision": "accept_prospective_antithetic_threshold13_holdout"
            if checks["passed"] else "reject_prospective_antithetic_threshold13_holdout",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r130_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r130_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_prospective_antithetic_threshold13_holdout"
        if checks["passed"] else "rejected",
        "source_commit": source_commit,
        "source_scene": "r129_development_union", "target_scene": runtime["scene"],
    ...
    tracked = [
        "R130_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SELECTOR_TRANSFER.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r130_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r130_antithetic_threshold13_holdout_v1.yaml"),
    )
