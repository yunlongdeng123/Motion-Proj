"""WorldSim V6 R101: export and visibly edit actor34 in independent scene0255."""
...
TASK_ID = "WS-V6-R101-THIRD-INDEPENDENT-SCENE-ACTOR-EDIT-COMPILER-01"


class R101ExperimentError(RuntimeError):
    """The preregistered R101 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R101ExperimentError("formal R101 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R101ExperimentError("R101 task_id drift")
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R101ExperimentError("StreetGS upstream commit drift")
    if shutil.disk_usage(run_root).free / (1024**3) < float(resources["minimum_disk_free_gib"]):
        raise R101ExperimentError("R101 disk resource insufficient")
    r100_gate = json.loads((r100_run / "R100_GATE.json").read_text())
    selection = json.loads(selection_path.read_text())
    inventory = json.loads(inventory_path.read_text())

    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__scene0255-actor34-edit-s{config['seed']}-r1"
    if run_dir.exists():
        raise R101ExperimentError(f"formal run directory already exists: {run_dir}")
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r101_sceneir_export.v1",
            "scene": target["scene"],
    ...
    if set(chunk["arrays"]) != REQUIRED_ARRAYS:
        raise R101ExperimentError("actor34 Gaussian array set drift")
    transforms_by_key = {
    ...
    for row in actor["trajectory"]:
        ...
        if key not in transforms_by_key or timestamp not in visibility:
            raise R101ExperimentError(f"actor34 trajectory reference missing: {key}")
    ...
    for name, reference in chunk["arrays"].items():
        ...
        if manifest_row is None or manifest_row["sha256"] != reference["sha256"]:
            raise R101ExperimentError(f"base manifest missing actor34 blob: {name}")
    ...
    for x, z in candidates:
        ...
        proposal_rows.append(
            {
                "proposal_id": f"scene0255_actor34_translate_x_{x:+.1f}_z_{z:+.1f}",
    ...
    if not accepted_rows:
        raise R101ExperimentError("no zero-new-overlap candidate in preregistered grid")
    selected = sorted(
        accepted_rows,
        key=lambda row: (
    ...
    geometry = {
        "schema_version": "worldsim_v6.r101_trajectory_geometry.v1",
        "asset_id": actor["id"],
        "chunk_id": chunk["id"],
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r101_actor_bundle.v1",
            "frontend_model_index": actor_index,
            "actor": actor,
            "chunk": chunk,
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r101_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r101_package_manifest.v1",
            "files": {
    ...
    _write_json(
        ...
        {"schema_version": "worldsim_v6.r101_selected_proposal.v1", "selection_rule": search["selection_rule"], "selected": selected},
    )
    ...
    def run_worker(label: str, delta: list[float]) -> tuple[dict, dict, Path]:
        ...
        if len(rows) != 1:
            raise R101ExperimentError(f"{label} sensor denominator drift")
    ...
    comparison = {
        "schema_version": "worldsim_v6.r101_edit_sensor_comparison.v1",
    ...
    checks = {
        ...
        "r100_selection_exact": selection["selected_actor_model_index"] == actor_index
        and selection["selected_frame_index"] == int(target["visibility_frame_index"])
        and selection["selected_actor_effect_pixels"] == int(target["native_effect_pixels"]),
    ...
    _write_json(
        run_dir / "R101_GATE.json",
        {
            "schema_version": "worldsim_v6.r101_gate.v1", "checks": checks,
            "decision": "accept_third_independent_scene_actor_edit_compiler"
            if checks["passed"] else "reject_or_repair_third_independent_scene_actor_edit_compiler",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r101_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r101_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_third_independent_scene_actor_edit_compiler"
    ...
    tracked = [
        "R101_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SCENEIR_EXPORT.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r101_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r101_third_independent_scene_actor_edit_compiler_v1.yaml"),
    )
