"""WorldSim V6 R120: compile an orthogonal actor1 edit in scene0255."""
...
TASK_ID = "WS-V6-R120-SCENE0255-ORTHOGONAL-FOURTH-ACTOR-EDIT-COMPILER-01"


class R120ExperimentError(RuntimeError):
    """The preregistered R120 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R120ExperimentError("formal R120 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R120ExperimentError("R120 task_id drift")
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R120ExperimentError("StreetGS upstream commit drift")
    if shutil.disk_usage(run_root).free / (1024**3) < float(resources["minimum_disk_free_gib"]):
        raise R120ExperimentError("R120 disk resource insufficient")
    ...
    run_dir = run_root / TASK_ID / f"{now}__scene0255-actor1-zplus0p5-edit-s{config['seed']}-r1"
    if run_dir.exists():
        raise R120ExperimentError(f"formal run directory already exists: {run_dir}")
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r120_sceneir_export.v1",
            "scene": target["scene"],
    ...
    if set(chunk["arrays"]) != REQUIRED_ARRAYS:
        raise R120ExperimentError("actor1 Gaussian array set drift")
    transforms_by_key = {
    ...
    for row in actor["trajectory"]:
        ...
        if key not in transforms_by_key or timestamp not in visibility:
            raise R120ExperimentError(f"actor1 trajectory reference missing: {key}")
    ...
    for name, reference in chunk["arrays"].items():
        ...
        if manifest_row is None or manifest_row["sha256"] != reference["sha256"]:
            raise R120ExperimentError(f"base manifest missing actor1 blob: {name}")
    ...
    if not accepted_rows:
        raise R120ExperimentError("no zero-new-overlap candidate in preregistered grid")
    expected_delta = [float(value) for value in search["selected_translation_delta_m"]]
    selected_candidates = [
        row for row in accepted_rows if row["translation_delta_m"] == expected_delta
    ]
    if len(selected_candidates) != 1:
        raise R120ExperimentError("preregistered orthogonal proposal is not uniquely accepted")
    ...
    geometry = {
        "schema_version": "worldsim_v6.r120_trajectory_geometry.v1",
        "asset_id": actor["id"],
        "chunk_id": chunk["id"],
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r120_actor_bundle.v1",
            "frontend_model_index": actor_index,
            "actor": actor,
            "chunk": chunk,
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r120_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r120_package_manifest.v1",
            "files": {
    ...
    _write_json(
        ...
        {"schema_version": "worldsim_v6.r120_selected_proposal.v1", "selection_rule": search["selection_rule"], "selected": selected},
    )
    ...
    def run_worker(label: str, delta: list[float]) -> tuple[dict, dict, Path]:
        ...
        if len(rows) != 1:
            raise R120ExperimentError(f"{label} sensor denominator drift")
    ...
    comparison = {
        "schema_version": "worldsim_v6.r120_edit_sensor_comparison.v1",
    ...
    expected_selected = selected_candidates[0]
    checks = {
    ...
    _write_json(
        run_dir / "R120_GATE.json",
        {
            "schema_version": "worldsim_v6.r120_gate.v1", "checks": checks,
            "decision": "accept_scene0255_orthogonal_fourth_actor_edit_compiler"
            if checks["passed"] else "reject_or_repair_scene0255_orthogonal_fourth_actor_edit_compiler",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r120_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r120_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_scene0255_orthogonal_fourth_actor_edit_compiler"
    ...
    tracked = [
        "R120_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SCENEIR_EXPORT.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r120_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r120_scene0255_orthogonal_fourth_actor_edit_compiler_v1.yaml"),
    )
