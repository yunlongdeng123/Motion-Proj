"""WorldSim V6 R97: export and visibly edit actor8 in independent scene0048."""
...
TASK_ID = "WS-V6-R97-SECOND-INDEPENDENT-SCENE-ACTOR-EDIT-COMPILER-01"


class R97ExperimentError(RuntimeError):
    """The preregistered R97 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R97ExperimentError("formal R97 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R97ExperimentError("R97 task_id drift")
    ...
    r96_run = _resolve_runs_uri(sources["r96_run"])
    ...
    selection_path = r96_run / "EXHAUSTIVE_FRONT_SELECTION.json"
    lifecycle_path = r95_run / "worker" / f"ACTOR_{int(target['actor_model_index']):04d}_FRAME_VALID.npy"
    frozen_files = {
        ...
        r96_run / "MANIFEST.json": sources["r96_manifest_sha256"],
        r96_run / "R96_GATE.json": sources["r96_gate_sha256"],
        r96_run / "SUMMARY.json": sources["r96_summary_sha256"],
        selection_path: sources["r96_selection_sha256"],
        r96_run / "worker/EXHAUSTIVE_VISIBILITY.json": sources["r96_visibility_sha256"],
        r96_run / "worker/WORKER_AUDIT.json": sources["r96_worker_audit_sha256"],
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R97ExperimentError("StreetGS upstream commit drift")
    if shutil.disk_usage(run_root).free / (1024**3) < float(resources["minimum_disk_free_gib"]):
        raise R97ExperimentError("R97 disk resource insufficient")
    r95_gate = json.loads((r95_run / "R95_GATE.json").read_text())
    r95_summary = json.loads((r95_run / "SUMMARY.json").read_text())
    r96_gate = json.loads((r96_run / "R96_GATE.json").read_text())
    selection = json.loads(selection_path.read_text())
    inventory = json.loads(inventory_path.read_text())

    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__scene0048-actor8-edit-s{config['seed']}-r1"
    if run_dir.exists():
        raise R97ExperimentError(f"formal run directory already exists: {run_dir}")
    ...
    document_template, chunk_arrays = streetgs_to_sceneir(
        ...
        source_uri="checkpoint://streetgs_scene0048_reference",
        reconstructor_version=sources["streetgs_upstream_commit"],
        seed=int(config["seed"]),
    )
    base_package = run_dir / "sceneir_packages/streetgs_scene0048"
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r97_sceneir_export.v1",
            "scene": target["scene"],
    ...
    if set(chunk["arrays"]) != REQUIRED_ARRAYS:
        raise R97ExperimentError("actor8 Gaussian array set drift")
    transforms_by_key = {
    ...
    for row in actor["trajectory"]:
        ...
        if key not in transforms_by_key or timestamp not in visibility:
            raise R97ExperimentError(f"actor8 trajectory reference missing: {key}")
    ...
    for name, reference in chunk["arrays"].items():
        ...
        if manifest_row is None or manifest_row["sha256"] != reference["sha256"]:
            raise R97ExperimentError(f"base manifest missing actor8 blob: {name}")
    ...
    for x, z in candidates:
        ...
        proposal_rows.append(
            {
                "proposal_id": f"scene0048_actor8_translate_x_{x:+.1f}_z_{z:+.1f}",
    ...
    if not accepted_rows:
        raise R97ExperimentError("no zero-new-overlap candidate in preregistered grid")
    selected = sorted(
        accepted_rows,
        key=lambda row: (
    ...
    geometry = {
        "schema_version": "worldsim_v6.r97_trajectory_geometry.v1",
        "asset_id": actor["id"],
        "chunk_id": chunk["id"],
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r97_actor_bundle.v1",
            "frontend_model_index": actor_index,
            "actor": actor,
            "chunk": chunk,
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r97_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r97_package_manifest.v1",
            "files": {
    ...
    _write_json(
        ...
        {"schema_version": "worldsim_v6.r97_selected_proposal.v1", "selection_rule": search["selection_rule"], "selected": selected},
    )
    ...
    def run_worker(label: str, delta: list[float]) -> tuple[dict, dict, Path]:
        ...
        if len(rows) != 1:
            raise R97ExperimentError(f"{label} sensor denominator drift")
    ...
    comparison = {
        "schema_version": "worldsim_v6.r97_edit_sensor_comparison.v1",
    ...
    r95_failed_checks = sorted(name for name, value in r95_gate["checks"].items() if not value)
    checks = {
        "r95_inventory_authority_retained_with_visibility_only_rejection":
        r95_summary["status"] == "rejected"
        and r95_failed_checks == ["passed", "selected_visible_support_nontrivial"],
        "r96_exhaustive_visibility_authority_accepted": bool(r96_gate["checks"]["passed"]),
        "r96_selection_exact": selection["selected_actor_model_index"] == actor_index
        ...
        "baseline_native_visibility_reproduces_r96": baseline_row["actor_effect_pixels"]
    ...
    _write_json(
        run_dir / "R97_GATE.json",
        {
            "schema_version": "worldsim_v6.r97_gate.v1", "checks": checks,
            "decision": "accept_second_independent_scene_actor_edit_compiler"
            if checks["passed"] else "reject_or_repair_second_independent_scene_actor_edit_compiler",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r97_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r97_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_second_independent_scene_actor_edit_compiler"
    ...
    tracked = [
        "R97_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SCENEIR_EXPORT.json",
        "PROPOSAL_CATALOG.jsonl",
        "SELECTED_PROPOSAL.json", "EDIT_SENSOR_COMPARISON.json",
        "sceneir_packages/streetgs_scene0048/MANIFEST.json",
        "sceneir_packages/streetgs_scene0048/sceneir.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r97_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r97_second_independent_scene_actor_edit_compiler_v1.yaml"),
    )
