"""WorldSim V6 R94: zero-calibration selector transfer to scene0230."""
...
TASK_ID = "WS-V6-R94-INDEPENDENT-SCENE-SELECTOR-TRANSFER-01"


class R94ExperimentError(RuntimeError):
    """The preregistered R94 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R94ExperimentError("formal R94 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R94ExperimentError("R94 task_id drift")
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R94ExperimentError("StreetGS upstream commit drift")
    free_gib = shutil.disk_usage(run_root).free / (1024**3)
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R94ExperimentError("R94 disk resource insufficient")
    ...
    if len(frame_indices) != int(runtime["expected_frame_count"]):
        raise R94ExperimentError("R94 frame denominator drift")

    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__scene0230-selector-transfer-s{config['seed']}-r1"
    ...
    geometry = {
        "schema_version": "worldsim_v6.r94_transform_owned_actor_geometry.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r94_actor_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r94_actor_package_manifest.v1",
            "files": {
    ...
    composition = {
        "schema_version": "worldsim_v6.r94_scene_composition.v1",
        "scene": runtime["scene"],
        "actors": [
            {
    ...
    runtime_contract = {
        "schema_version": "worldsim_v6.r94_runtime_contract.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r94_scene_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r94_scene_package_manifest.v1",
            "files": {
    ...
    if [row["frame_index"] for row in sensor_rows] != frame_indices:
        raise R94ExperimentError("R94 sensor output denominator/order drift")

    perception_dir = run_dir / "perception"
    variants = list(evaluation["variants"])
    if variants != ["logged", "edited"]:
        raise R94ExperimentError("R94 variant order drift")
    index_rows = [
        {
    ...
    lifecycle_mask = np.ones(len(frame_indices), dtype=bool)
    ...
    transfer = {
        "schema_version": "worldsim_v6.r94_selector_transfer.v1",
        ...
        "always_on_lifecycle_metrics": lifecycle_metrics,
    ...
    checks = {
        "r93_and_r90_authorities_accepted": bool(r93_gate["checks"]["passed"] and r90_gate["checks"]["passed"]),
        "r93_selected_edit_exact": selected["proposal_id"] == runtime["proposal_id"]
    ...
    _write_json(
        run_dir / "R94_GATE.json",
        {
            "schema_version": "worldsim_v6.r94_gate.v1", "checks": checks,
            "decision": "accept_zero_calibration_independent_scene_selector_transfer"
            if checks["passed"] else "reject_or_recalibrate_independent_scene_selector_transfer",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r94_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r94_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_zero_calibration_independent_scene_selector_transfer"
        ...
        "fixed256_f1": fixed_metrics["f1"], "always_on_lifecycle_f1": lifecycle_metrics["f1"],
        "total_changed_label_pixels": int(changed_labels.sum()),
        "log1p_sensor_label_impact_pearson": correlation,
    ...
    tracked = [
        "R94_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SELECTOR_TRANSFER.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r94_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r94_independent_scene_selector_transfer_v1.yaml"),
    )
