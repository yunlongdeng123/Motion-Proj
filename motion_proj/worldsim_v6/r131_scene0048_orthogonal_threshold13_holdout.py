"""WorldSim V6 R131: prospective scene0048 orthogonal threshold13 holdout."""
...
TASK_ID = "WS-V6-R131-SCENE0048-ORTHOGONAL-THRESHOLD13-HOLDOUT-01"


class R131ExperimentError(RuntimeError):
    """The preregistered R131 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R131ExperimentError("formal R131 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R131ExperimentError("R131 task_id drift")
    ...
    r99_run = _resolve_runs_uri(sources["r99_run"])
    ...
    frozen_files = {
        ...
        r97_run / "SUMMARY.json": sources["r97_summary_sha256"],
        r97_run / "SELECTED_PROPOSAL.json": sources["r97_selected_proposal_sha256"],
        r97_run / "EDIT_SENSOR_COMPARISON.json": sources["r97_edit_sensor_comparison_sha256"],
        r97_run / "PROPOSAL_CATALOG.jsonl": sources["r97_proposal_catalog_sha256"],
        ...
        r99_run / "MANIFEST.json": sources["r99_manifest_sha256"],
        r99_run / "R99_GATE.json": sources["r99_gate_sha256"],
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R131ExperimentError("StreetGS upstream commit drift")
    free_gib = shutil.disk_usage(run_root).free / (1024**3)
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R131ExperimentError("R131 disk resource insufficient")
    r97_gate = json.loads((r97_run / "R97_GATE.json").read_text())
    r98_gate = json.loads((r98_run / "R98_GATE.json").read_text())
    r99_gate = json.loads((r99_run / "R99_GATE.json").read_text())
    ...
    r130_gate = json.loads((r130_run / "R130_GATE.json").read_text())
    selected = json.loads((r97_run / "SELECTED_PROPOSAL.json").read_text())["selected"]
    candidate_rows = [
        row for row in _load_rows(r97_run / "PROPOSAL_CATALOG.jsonl")
        if row["proposal_id"] == runtime["proposal_id"]
    ]
    if len(candidate_rows) != 1:
        raise R131ExperimentError("R131 orthogonal candidate binding drift")
    candidate = candidate_rows[0]
    policy = json.loads((r129_package / "POLICY.json").read_text())
    source_package_manifest = json.loads((r97_package / "PACKAGE_MANIFEST.json").read_text())
    ...
    if len(frame_indices) != int(runtime["expected_frame_count"]):
        raise R131ExperimentError("R131 frame denominator drift")

    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__scene0048-orthogonal-threshold13-s{config['seed']}-r1"
    run_dir.mkdir(parents=True, exist_ok=False)
    scene_package = run_dir / "scene_package"
    actor_package = scene_package / "actors" / runtime["actor_id"]
    shutil.copytree(r97_package, actor_package)
    ...
    geometry = {
        "schema_version": "worldsim_v6.r131_transform_owned_actor_geometry.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r131_actor_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r131_actor_package_manifest.v1",
            "files": {
    ...
    composition = {
        "schema_version": "worldsim_v6.r131_scene_composition.v1",
        "scene": runtime["scene"],
        "actors": [
            {
    ...
    runtime_contract = {
        "schema_version": "worldsim_v6.r131_runtime_contract.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r131_scene_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r131_scene_package_manifest.v1",
            "files": {
    ...
    if [row["frame_index"] for row in sensor_rows] != frame_indices:
        raise R131ExperimentError("R131 sensor output denominator/order drift")

    perception_dir = run_dir / "perception"
    variants = list(evaluation["variants"])
    if variants != ["logged", "edited"]:
        raise R131ExperimentError("R131 variant order drift")
    index_rows = [
        {
    ...
    transfer = {
        "schema_version": "worldsim_v6.r131_scene0048_orthogonal_threshold13_holdout.v1",
    ...
    checks = {
        "r97_r98_r99_r129_r130_authorities_bound": bool(
            r97_gate["checks"]["passed"]
            and not r98_gate["checks"]["passed"]
            and r99_gate["checks"]["passed"]
            and r129_gate["checks"]["passed"]
            and r130_gate["checks"]["passed"]
        ),
        "accepted_r99_authority_preserves_r98_rejection": not r98_gate["checks"]["passed"]
        and r99_gate["checks"]["passed"],
        ...
        "r97_accepted_orthogonal_candidate_exact": candidate["proposal_id"] == runtime["proposal_id"]
        ...
        "prospective_zplus_is_orthogonal_equal_norm_to_r98_axis_edit": abs(
            sum(
                float(a) * float(b)
    ...
    _write_json(
        run_dir / "R131_GATE.json",
        {
            "schema_version": "worldsim_v6.r131_gate.v1", "checks": checks,
            "decision": "accept_scene0048_orthogonal_threshold13_holdout"
            if checks["passed"] else "reject_scene0048_orthogonal_threshold13_holdout",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r131_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r131_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_scene0048_orthogonal_threshold13_holdout"
    ...
    tracked = [
        "R131_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SELECTOR_TRANSFER.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r131_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r131_scene0048_orthogonal_threshold13_holdout_v1.yaml"),
    )
