"""WorldSim V6 R118: four-actor compositional selector on scene0255."""
...
TASK_ID = "WS-V6-R118-SCENE0255-FOUR-ACTOR-COMPOSITIONAL-SELECTOR-01"


class R118ExperimentError(RuntimeError):
    """The preregistered R118 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R118ExperimentError("formal R118 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R118ExperimentError("R118 task_id drift")
    ...
    r117_run = _resolve_runs_uri(sources["r117_run"])
    r117_package = r117_run / "package"
    ...
    frozen_files = {
        ...
        r117_run / "MANIFEST.json": sources["r117_manifest_sha256"],
        r117_run / "R117_GATE.json": sources["r117_gate_sha256"],
        r117_run / "SUMMARY.json": sources["r117_summary_sha256"],
        r117_run / "SELECTED_PROPOSAL.json": sources["r117_selected_proposal_sha256"],
        r117_run / "EDIT_SENSOR_COMPARISON.json": sources["r117_edit_sensor_comparison_sha256"],
        r117_package / "PACKAGE_MANIFEST.json": sources["r117_package_manifest_sha256"],
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R118ExperimentError("StreetGS upstream commit drift")
    free_gib = shutil.disk_usage(run_root).free / (1024**3)
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R118ExperimentError("R118 disk resource insufficient")
    ...
    r117_gate = json.loads((r117_run / "R117_GATE.json").read_text())
    ...
    selected_by_actor = {
        ...
        "actor_0001": json.loads((r117_run / "SELECTED_PROPOSAL.json").read_text())["selected"],
    }
    ...
    if len(frame_indices) != int(runtime["expected_frame_count"]):
        raise R118ExperimentError("R118 frame denominator drift")

    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__scene0255-four-actor-selector-s{config['seed']}-r1"
    ...
    if actor_ids != ["actor_0034", "actor_0024", "actor_0009", "actor_0001"] or len(set(actor_ids)) != 4:
        raise R118ExperimentError("R118 actor order/denominator drift")
    ...
    for actor in actor_configs:
        ...
        if len(base_actor) != len(edited_actor) or not base_actor:
            raise R118ExperimentError(f"R118 {actor_id} SceneIR trajectory denominator drift")
    ...
    joint_interaction = {
        "schema_version": "worldsim_v6.r118_joint_interaction_factor.v1",
        "scene": runtime["scene"],
    ...
    for actor in actor_configs:
        ...
        geometry = {
            "schema_version": "worldsim_v6.r118_transform_owned_actor_geometry.v1",
        ...
        _write_json(
            ...
            {
                "schema_version": "worldsim_v6.r118_actor_validity.v1",
        ...
        _write_json(
            ...
            {
                "schema_version": "worldsim_v6.r118_actor_package_manifest.v1",
                "files": {
                    name: {
    ...
    composition = {
        "schema_version": "worldsim_v6.r118_scene_composition.v1",
        "scene": runtime["scene"],
        "actors": [
            {
                "actor_id": str(actor["actor_id"]),
    ...
    runtime_contract = {
        "schema_version": "worldsim_v6.r118_runtime_contract.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r118_scene_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r118_scene_package_manifest.v1",
            "files": {
    ...
    if [row["frame_index"] for row in sensor_rows] != frame_indices:
        raise R118ExperimentError("R118 sensor output denominator/order drift")

    perception_dir = run_dir / "perception"
    variants = list(evaluation["variants"])
    if variants != ["logged", "edited"]:
        raise R118ExperimentError("R118 variant order drift")
    index_rows = [
        {
    ...
    transfer = {
        "schema_version": "worldsim_v6.r118_selector_transfer.v1",
    ...
    checks = {
        "r101_r108_r113_r117_r114_and_r90_authorities_accepted": bool(
            r101_gate["checks"]["passed"]
            and r108_gate["checks"]["passed"]
            and r113_gate["checks"]["passed"]
            and r117_gate["checks"]["passed"]
    ...
    _write_json(
        run_dir / "R118_GATE.json",
        {
            "schema_version": "worldsim_v6.r118_gate.v1", "checks": checks,
            "decision": "accept_four_actor_compositional_selector_transfer"
            if checks["passed"] else "reject_four_actor_compositional_selector_transfer",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r118_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r118_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_four_actor_compositional_selector_transfer"
    ...
    tracked = [
        "R118_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SELECTOR_TRANSFER.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r118_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r118_scene0255_four_actor_compositional_selector_v1.yaml"),
    )
