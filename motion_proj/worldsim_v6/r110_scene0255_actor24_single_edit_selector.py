"""WorldSim V6 R110: actor24-only selector cell for scene0255 factorial analysis."""
...
TASK_ID = "WS-V6-R110-SCENE0255-ACTOR24-SINGLE-EDIT-SELECTOR-01"


class R110ExperimentError(RuntimeError):
    """The preregistered R110 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R110ExperimentError("formal R110 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R110ExperimentError("R110 task_id drift")
    ...
    if len(frame_indices) != int(runtime["expected_frame_count"]):
        raise R110ExperimentError("R110 frame denominator drift")

    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__scene0255-actor24-single-selector-s{config['seed']}-r1"
    run_dir.mkdir(parents=True, exist_ok=False)
    scene_package = run_dir / "scene_package"
    actor_package = scene_package / "actors" / runtime["actor_id"]
    shutil.copytree(r108_package, actor_package)
    ...
    geometry = {
        "schema_version": "worldsim_v6.r110_transform_owned_actor_geometry.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r110_actor_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r110_actor_package_manifest.v1",
            "files": {
    ...
    composition = {
        "schema_version": "worldsim_v6.r110_scene_composition.v1",
        "scene": runtime["scene"],
        "actors": [
            {
    ...
    runtime_contract = {
        "schema_version": "worldsim_v6.r110_runtime_contract.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r110_scene_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r110_scene_package_manifest.v1",
            "files": {
    ...
    if [row["frame_index"] for row in sensor_rows] != frame_indices:
        raise R110ExperimentError("R110 sensor output denominator/order drift")

    perception_dir = run_dir / "perception"
    variants = list(evaluation["variants"])
    if variants != ["logged", "edited"]:
        raise R110ExperimentError("R110 variant order drift")
    index_rows = [
        {
    ...
    transfer = {
        "schema_version": "worldsim_v6.r110_selector_transfer.v1",
    ...
    checks = {
        "r108_and_r90_authorities_accepted": bool(r108_gate["checks"]["passed"] and r90_gate["checks"]["passed"]),
        "r108_selected_edit_exact": selected["proposal_id"] == runtime["proposal_id"]
    ...
    _write_json(
        run_dir / "R110_GATE.json",
        {
            "schema_version": "worldsim_v6.r110_gate.v1", "checks": checks,
            "decision": "accept_actor24_single_edit_selector_cell"
            if checks["passed"] else "reject_actor24_single_edit_selector_cell",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r110_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r110_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_actor24_single_edit_selector_cell"
    ...
    tracked = [
        "R110_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SELECTOR_TRANSFER.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r110_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r110_scene0255_actor24_single_edit_selector_v1.yaml"),
    )
