"""WorldSim V6 R114: three-actor compositional selector on scene0255."""
...
TASK_ID = "WS-V6-R114-SCENE0255-THREE-ACTOR-COMPOSITIONAL-SELECTOR-01"


class R114ExperimentError(RuntimeError):
    """The preregistered R114 contract was violated."""
...
def run_experiment(repo_root: Path, config_path: Path, run_root: Path) -> Path:
    ...
    if _git(repo_root, "status", "--porcelain"):
        raise R114ExperimentError("formal R114 run requires clean source")
    source_commit = _git(repo_root, "rev-parse", "HEAD")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config.get("task_id") != TASK_ID:
        raise R114ExperimentError("R114 task_id drift")
    ...
    if _git(upstream, "rev-parse", "HEAD") != sources["streetgs_upstream_commit"]:
        raise R114ExperimentError("StreetGS upstream commit drift")
    free_gib = shutil.disk_usage(run_root).free / (1024**3)
    if free_gib < float(resources["minimum_disk_free_gib"]):
        raise R114ExperimentError("R114 disk resource insufficient")
    ...
    if len(frame_indices) != int(runtime["expected_frame_count"]):
        raise R114ExperimentError("R114 frame denominator drift")

    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_dir = run_root / TASK_ID / f"{now}__scene0255-three-actor-selector-s{config['seed']}-r1"
    run_dir.mkdir(parents=True, exist_ok=False)

    actor_configs = list(runtime["actors"])
    actor_ids = [str(row["actor_id"]) for row in actor_configs]
    if actor_ids != ["actor_0034", "actor_0024", "actor_0009"] or len(set(actor_ids)) != 3:
        raise R114ExperimentError("R114 actor order/denominator drift")
    ...
    for actor in actor_configs:
        ...
        if len(base_actor) != len(edited_actor) or not base_actor:
            raise R114ExperimentError(f"R114 {actor_id} SceneIR trajectory denominator drift")
    ...
    joint_interaction = {
        "schema_version": "worldsim_v6.r114_joint_interaction_factor.v1",
        "scene": runtime["scene"],
    ...
    for actor in actor_configs:
        ...
        geometry = {
            "schema_version": "worldsim_v6.r114_transform_owned_actor_geometry.v1",
        ...
        _write_json(
            ...
            {
                "schema_version": "worldsim_v6.r114_actor_validity.v1",
        ...
        _write_json(
            ...
            {
                "schema_version": "worldsim_v6.r114_actor_package_manifest.v1",
                "files": {
                    name: {
    ...
    composition = {
        "schema_version": "worldsim_v6.r114_scene_composition.v1",
        "scene": runtime["scene"],
        "actors": [
            {
                "actor_id": str(actor["actor_id"]),
    ...
    runtime_contract = {
        "schema_version": "worldsim_v6.r114_runtime_contract.v1",
        "runtime_mode": "three_actor_transform_lifecycle_owned_scene_patch",
        "actor_order": actor_ids,
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r114_scene_validity.v1",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r114_scene_package_manifest.v1",
            "files": {
    ...
    if [row["frame_index"] for row in sensor_rows] != frame_indices:
        raise R114ExperimentError("R114 sensor output denominator/order drift")

    perception_dir = run_dir / "perception"
    variants = list(evaluation["variants"])
    if variants != ["logged", "edited"]:
        raise R114ExperimentError("R114 variant order drift")
    index_rows = [
        {
    ...
    transfer = {
        "schema_version": "worldsim_v6.r114_selector_transfer.v1",
    ...
    checks = {
        ...
        "all196_three_actor_compiled_native_sensor_numerics_conformant": all(
        ...
        "three_actor_primitive_counts_and_translations_exact_all_frames": all(
            all(
        ...
        "scene_and_three_nested_package_manifests_immutable": sensor_audit[
    ...
    _write_json(
        run_dir / "R114_GATE.json",
        {
            "schema_version": "worldsim_v6.r114_gate.v1", "checks": checks,
            "decision": "accept_three_actor_compositional_selector_transfer"
            if checks["passed"] else "reject_three_actor_compositional_selector_transfer",
        },
    )
    _write_json(
        run_dir / "RESOURCE_AUDIT.json",
        {
            "schema_version": "worldsim_v6.r114_resource_audit.v1",
    ...
    summary = {
        "schema_version": "worldsim_v6.r114_summary.v1",
        "task_id": TASK_ID,
        "hypothesis_id": config["hypothesis_id"],
        "status": "done" if checks["passed"] else "rejected",
        "hypothesis_outcome": "accepted_development_three_actor_compositional_selector_transfer"
    ...
    tracked = [
        "R114_GATE.json", "SUMMARY.json", "RESOURCE_AUDIT.json", "SELECTOR_TRANSFER.json",
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r114_manifest.v1",
            "files": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path,
        default=Path("configs/worldsim_v6/r114_scene0255_three_actor_compositional_selector_v1.yaml"),
    )
