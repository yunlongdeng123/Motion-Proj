"""WorldSim V6 R31 typed actor appearance LogSim 正式实验。"""
...
TASK_ID = "WS-V6-R31-ACTOR-APPEARANCE-LOGSIM-01"
...
def run_experiment(
    ...
    r30_gate = json.loads((r11_run / r11_gate_name).read_text(encoding="utf-8"))
    if not r30_gate.get("checks", {}).get("passed"):
        raise R12ExperimentError("R30 actor bake gate 未通过")
    if r30_gate.get("decision") != "proceed_to_actor_logsim_validation":
        raise R12ExperimentError("R30 actor LogSim authority 漂移")
    package_manifest = json.loads(
        (r11_run / "package/PACKAGE_MANIFEST.json").read_text(encoding="utf-8")
    )
    for relative, metadata in package_manifest["files"].items():
        package_path = r11_run / "package" / relative
        if _sha256(package_path) != metadata["sha256"]:
            raise R12ExperimentError(f"R30 package file 漂移：{relative}")
        frozen_files[package_path] = metadata["sha256"]
    ...
    try:
        ...
        replay_dir.mkdir()
        assets = _read_jsonl(r11_run / "package/ACTOR_ASSET_REGISTRY.jsonl")
        provenance = {
            row["asset_id"]: row
        ...
        validity = {
            row["asset_id"]: row
            for row in _read_jsonl(r11_run / "package/VALIDITY.jsonl")
        }
        trajectories = _read_jsonl(r11_run / "package/TRAJECTORY_ABSTAINS.jsonl")
        dependency = json.loads(
            (r11_run / "package/RUNTIME_DEPENDENCY_AUDIT.json").read_text(
                encoding="utf-8"
            )
        )
        expected_count = int(
            config["cohort"]["expected_actor_appearance_asset_count"]
        )
        if len(assets) != expected_count:
            raise R12ExperimentError("R30 actor appearance 资产分母漂移")
        ...
        for asset in assets:
            ...
            if asset["asset_type"] != "explicit_actor_observation_plane_appearance":
                raise R12ExperimentError(f"actor asset type 漂移：{case_id}")
            if validity[asset["asset_id"]]["trajectory_decision"] != "ABSTAIN":
                raise R12ExperimentError(f"trajectory validity 漂移：{case_id}")
        ...
        checks = {
            "all_actor_appearance_assets_replayed": len(grouped) == expected_count,
            ...
            "no_generator_runtime_dependency": not dependency[
                "online_generator_dependency"
            ]
            and all(
                not row["runtime_generator_dependency"] for row in assets
            ),
            "trajectory_scope_abstained": len(trajectories) == expected_count
            and all(row["decision"] == "ABSTAIN" for row in trajectories)
            and dependency["trajectory_asset_count"] == 0,
        ...
        _write_json(
            run_dir / "R31_GATE.json",
            {
                "schema_version": "worldsim_v6.r12_gate.v1",
                "checks": checks,
                "supported_scope": "actor_appearance_rgb_sensor_and_frozen_semantic_output_replay",
                "full_logsim_coverage": False,
                "unsupported_factors": unsupported,
                "decision": "complete_bounded_actor_appearance_logsim"
                if checks["passed"]
                else "reject_actor_appearance_logsim_hypothesis",
            },
        )
        summary = {
            ...
            "hypothesis_outcome": "accepted_development_actor_appearance_scope"
            if checks["passed"]
            else "rejected",
            "source_commit": source_commit,
            "actor_appearance_case_count": len(grouped),
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", type=Path, default=Path("configs/worldsim_v6/r31_actor_logsim_v1.yaml")
    )
    parser.add_argument(
