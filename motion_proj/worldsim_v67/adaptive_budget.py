...
BUDGET_HORIZON_CONDITIONED_FEATURE_NAMES = FEATURE_NAMES + ("selected_fraction", "horizon_seconds")
...
def budget_horizon_conditioned_case_offset_dataset(
    arrays: Mapping[str, np.ndarray], compiled_scores: np.ndarray, selected_fractions: list[float],
    horizon_seconds_by_domain: list[float],
) -> dict[str, np.ndarray]:
    dataset = budget_conditioned_case_offset_dataset(arrays, compiled_scores, selected_fractions)
    horizons = np.asarray(horizon_seconds_by_domain, dtype=np.float32)
    horizon_feature = horizons[np.asarray(dataset["domain_index"], dtype=np.int64)].reshape(-1, 1)
    dataset["features"] = np.concatenate([dataset["features"], horizon_feature], axis=1)
    return dataset
...
def coverage_constrained_selection(
    arrays: Mapping[str, np.ndarray], compiled_scores: np.ndarray, case_offsets: np.ndarray,
    ...
    for row, case in enumerate(unique_cases):
        ...
        for rank, action in enumerate(order[: int(maximum_actions_per_case)], start=1):
            slot = (float(compiled_scores[action] + case_offsets[row]), row, rank, int(action))
            all_slots.append(slot)
            if rank == 1:
                first_slots.append(slot)
    minimum_covered = max(1, int(np.ceil(float(minimum_case_coverage) * len(unique_cases))))
    mandatory = sorted(first_slots, key=lambda item: item[0])[:minimum_covered]
    ...
    return {
        ...
        "case_coverage": float(np.count_nonzero(counts) / len(counts)), "minimum_actions_per_case": int(counts.min()),
        "maximum_actions_per_case": int(counts.max()), "mean_actions_per_case": float(counts.mean()),
        "all_mean_cost": all_cost, "selected_mean_cost": selected_cost,
        "relative_cost_reduction": float((all_cost - selected_cost) / all_cost if all_cost else 0.0),
