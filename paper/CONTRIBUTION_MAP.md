...
| --- | --- | --- | --- | --- | --- |
| C1. Actor-preserving physical 3D compilation | Figure 1 metadata-locked four-quadrant teaser; Figure 2 cross-sensor literal boundary; Method 4.1; Experiments “Zero-shot Actor-local surface evidence”, “Four-action physical compilation”, “Frozen camera evidence”, and “Visibility-conditioned physical evidence”; supplement A--B/F | `av2_canonical_surface.py`, `av2_four_action_compiler.py`, `av2_p3_hard_evidence.py`, `av2_p3b_camera_evidence.py`, `visibility_certificate.py`, `visible_failure_attribution.py`, `analyze_worldsim_v7_p15_fresh_hazard_action.py`, `run_worldsim_v7_p11_provenance_authority_audit.py`, `true_first_return_attribution.py` | P3 hard evidence, P3-B camera evidence, consumed and fresh P3-C bidirectional proximity diagnostics, P3-D/P15 proxy action provenance, P20/P22 corrections, P23 fresh literal confirmation, and P11 provenance-authority audit | Matched-ray free space is respected by construction; target-only aggregate depth/ray/Chamfer and proximity F-score improve on consumed and independently fresh AV2; literal audits expose and independently confirm COMPLETE-driven first-return risk on AV2; Actor identity, trajectory, extent, and hazard are retained | No completeness proof for unseen surfaces; no probabilistic sensor-noise certificate; no per-Actor repair guarantee; nearest-output provenance is not an action ablation or literal return; metric transfer is not model/policy validity; Gaussian opacity is not collision occupancy; UNKNOWN is not FREE |
...
### P20/P21/P22/P23 hard-evidence addendum
...
P22 then applies the same literal operator to the already consumed AV2 cohort and records a metric correction, not a fresh test.
P23 prospectively freezes that operator on a disjoint ten-log AV2 cohort and independently confirms metric undercounting and
COMPLETE provenance; it does not validate a learned model or policy.
...
Main Figure~2 and Supplement Figure~1 reuse the output rendered by `plot_worldsim_v7_p21_safety_boundary.py` from frozen
P20/P21/P22/P23 summaries. Their proxy bars
use already reported legacy rates; it performs no fit, policy selection, or new data read.
...
| P3-D | `run://worldsim_v7/WS-V7-P3D-AV2-VISIBLE-FAILURE-ATTRIBUTION-01/20260902T224500Z__visible-failure-attribution-s0-r1` | KEEP/PROJECT/COMPLETE target-nearest attribution of all target rays | Completion yields 14.96 proxy new hits per proxy new-early ray and only 13.49% of output contradictions; not literal first return |
...
| P15 | `run://worldsim_v7/WS-V7-P15-FRESH-HAZARD-ACTION-AUDIT-01/20260903T044500Z__fresh-hazard-action-audit-s0-r1` | Fresh hazard/clear target-nearest and compiler-action mechanism attribution | COMPLETE dominates proxy new-early returns; KEEP dominates surface contradictions; no literal or causal ablation claim |
...
| P22 | `run://worldsim_v7/WS-V7-P22-AV2-TRUE-FIRST-RETURN-CORRECTION-01/20260903T160000Z__consumed-av2-true-first-return-s0-r1` | Literal AV2 first-return metric correction over 20 already consumed logs | Hazard exposure is 13.926% (7.29x proxy); descriptive correction, not fresh transfer or policy evidence |
| P23 | `run://worldsim_v7/WS-V7-P23-FRESH-AV2-FIRST-RETURN-CONFIRMATION-01/20260903T183000Z__fresh-av2-literal-first-return-s0-r1` | Exact-once literal first-return confirmation on 10 metadata-frozen AV2 logs | Hazard exposure is 14.817% (8.29x proxy); fresh metric/provenance transfer, not model or policy validity |
...
- P20 owns the literal source first-return correction, P21 the deletion theorem, P22 the consumed-AV2 correction, and P23 its
  disjoint exact-once confirmation. They do not replace P3/P15 frozen proxy metrics, authorize a failed policy, or establish model transfer, hit/Chamfer monotonicity,
  collision freedom, or closed-loop safety.
...
- Target-nearest proximity and literal first-return are named separately; P20/P22/P23 own literal measurements and P21 owns the theorem.
