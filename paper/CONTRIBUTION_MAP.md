...
| --- | --- | --- | --- | --- | --- |
| C1. Actor-preserving physical 3D compilation | Method 4.1; Experiments “Zero-shot Actor-local surface evidence”, “Four-action physical compilation”, and “Frozen camera evidence”; supplement A--B/F | `av2_canonical_surface.py`, `av2_four_action_compiler.py`, `av2_p3_hard_evidence.py`, `av2_p3b_camera_evidence.py` | P3 hard evidence and P3-B camera evidence | Matched-ray free space is respected by construction; target-only aggregate depth/ray/Chamfer improves; Actor identity, trajectory, extent, and hazard are retained; all frozen qualitative failures remain visible | No completeness proof for unseen surfaces; no probabilistic sensor-noise certificate; no per-Actor repair guarantee; Gaussian opacity is not collision occupancy; UNKNOWN is not FREE |
| C2. Validity--hazard factorization and zero-shot selective repair | Method 4.2; Experiments “nuScenes-to-AV2 selective factorization”, “Fresh exact-once selector boundary”; supplement C | `selective_validity_hazard.py`, `sparsity_consistent_selector.py` | P4 primary factorized selector; P8-A fresh nuScenes rejection; P6-C source fit; fresh AV2 external result pending | nuScenes-only factorization has zero structural cross-input leakage and empirically transfers to AV2 with repair-or-abstain; P4 remains the primary selector after P6-C loses fresh nuScenes ranking | No cross-domain conformal/exchangeability guarantee; no AV2 adaptation; no post-hoc threshold/radius/feature tuning; fresh AV2 cannot be claimed until its exact-once canonical summary exists |
...
| P6-C fit | `run://worldsim_v7/WS-V7-P6C-SPARSITY-CONSISTENT-SELECTOR-01/20260902T173000Z__sparsity-consistent-s70602-r1` | Source-only sparsity-consistency candidate; fresh AV2 phase writes into the same run | V7-F12 rejects ratio-only recovery; P6-C cannot replace P4 after V7-F15 |
...
- P3/P3-B own physical aggregate and qualitative evidence. P4 owns the primary selector. P7/P7-B/P7-C own descriptive
...
- The pending P6-C fresh AV2 phase may add a fresh external row and boundary, but it cannot alter P4/P8-A models, thresholds,
...
- Pending fresh AV2 language remains pending until `summary.json` exists and the one-shot evaluator exits normally.
