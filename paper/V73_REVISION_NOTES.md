...
比较检验的是 native 种子、DPT 特征和辅助 build-depth 监督的整条联合通路，不能单独归因为某一层特征。五日志是已用于模型选择的开发数据。

最终20个独立AV2日志/936对象的固定确认已经完成（全部五方法、0优化更新）。Joint r7相对LiDAR r6：hit−2.942pp、early+5.364pp、free+.325997m、target→surface距离+.013259m、recall−2.420pp，五项95%日志配对区间均不跨0且方向更差；free在20/20日志变差。miss−.962pp区间跨0。**因此本轮联合方法的跨域几何/物理增量确认失败**，不能将上面的开发收益当成最终成功。完整数值见论文外部结果表和`docs/WORLDSIM_V7_3_FINAL_RESULTS.md`；摘要、结果、证据矩阵和结论均已同步。
...
保留为被检验的研究方向：**视觉几何 + 稀疏观测 → 可查询规范刚体表面**；聚焦表面支持、真实首面以及它们的矛盾。r7有开发收益，但独立AV2确认失败，不能将该方向写成本轮已经验证成功的方法主张。按用户终点结束V7.3，不再启动视觉重接后的新训练。
...
固定场景40 heldout帧、20日志、六方法240记录全部完成；每方法3908250条原始返回。r7−r6在cohort返回hit−2.211pp/miss+1.347pp/returned MAE+.223034m，三项区间均不跨0；box边界带hit−1.578pp/free+.034947m/returned MAE+.191461m，区间均不跨0。全束背景分母更大，不能用全局小差值遮盖Actor退化。归属与边界仍为box proxy，背景不完整；不宣布解决场景边界。

最终外部及场景推理均完成，论文维持负结果研究稿定位。learned support domain/正交训练仅为待验证讨论，不再扩展本轮实验。三本台账、报告、paper与归档push成功后，暂停自动跟进并确认无任务再shutdown；实际关机回执由本任务本地保存。
