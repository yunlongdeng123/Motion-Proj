...
We further partition target-only LiDAR evidence into early termination, matched hit, late return, and unmatched ray; reverse
...
and route Actor-level authority through abstention rather than delete completion.
...
action attribution localizes the mismatch: hazardous/clear new-early target-ray rates are
...
evaluation~\cite{kalble2024accurate}, we separately re-audit the fixed source surfaces with Eq.~\ref{eq:first-return}. The
always-complete total/hazard new-early rates rise from \PtwentyLegacyTotalEarly{}/\PtwentyLegacyHazardEarly{}\% under the proxy
to \PtwentyTrueTotalEarly{}/\PtwentyTrueHazardEarly{}\%, exposing roughly six times more literal first-return risk. P17,
P17R, and the one-slot sparse veto reduce hazardous rates to \PtwentyPseventeenHazardEarly{},
\PtwentyPseventeenRHazardEarly{}, and \PtwentyPnineteenHazardEarly{}\%, respectively, but all exceed the
\PtwentyBaselineChamfer{}\,m baseline Chamfer. The sparse veto removes \PtwentyPnineteenHazardRemoved{} hazardous early events
for \PtwentyPnineteenHitsLost{} hits and \PtwentyPnineteenPenaltyMM{}\,mm mean Chamfer, or
\PtwentyPnineteenEventsPerHit{} events per hit lost. Thus the monotone early direction is certified, while zero-cost surface
utility is empirically rejected. This is a consumed-source diagnostic, not fresh transfer or road-safety evidence.
