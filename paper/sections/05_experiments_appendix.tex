...
The V7 tables describe the historical HARP-3D compiler. V7.1 milestones are identified explicitly by M-number; source/external roles and operator definitions follow Sec.~\ref{sec:archive-repro}.
...
\paragraph{Historical V7 data and protocol.}
The initial V7 AV2 cohort uses every fifth UUID in the sorted list of 150 validation logs, yielding 20 quantitative and 10 qualitative logs. Training and model selection use nuScenes; the V7 AV2 evaluations use no target fine-tuning, calibration, or threshold selection. Later consumed-cohort diagnostics and disjoint confirmations retain their individual data roles, as detailed below. This initial cohort is not the V7.1 M43 cohort: M43 excludes all 60 previously consumed logs and samples 20 logs from the remaining 90 (Sec.~\ref{sec:archive-repro}).
...
\subsection{V7.1 Geometry and Query Operators}
...
smooth-ray/surface gradient conflict, passing all five development gates. Directly matching one Gaussian center to one nearest
...
\subsection{Historical V7 Compilation and Selective Interfaces}
...
\subsection{V7.1 State Composition and Evidential Learning}
...
We also test whether average geometry losses hide safety-relevant tails. Starting from M8, M29 adds a GT-only bidirectional loss equal to the mean of the worst 10\% nearest-point distances, normalized per Actor, while retaining all set, plane, scale, frame, and smooth expected-depth losses. On the 66 exposed development Actors, target-to-surface tail distance improves by 0.924\,mm, frame-mean distance by 0.843\,mm, and Chamfer by 0.418\,mm relative to M8. However, surface-to-target tail distance worsens by 0.362\,mm and hazardous early-return rate rises from 26.37\% to 26.69\%. The geometry and ray gradients conflict in 86.6--90.6\% of batches. This paired asymmetry rejects a single symmetric tail objective as a joint solution: increasing GT coverage can still move a small set of generated surfaces into unsupported free space. We stop without sweeping the tail fraction, weight, or seed.
...
\subsection{Categorical Responsibility, Visibility, and Temporal Diagnostics}
...
Starting from frozen M8, it keeps the same canonical geometry and losses but averages smooth expected-depth and pre-hit hinge
