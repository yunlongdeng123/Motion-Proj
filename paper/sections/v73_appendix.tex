...
Trainable native geometry pathway & Full DPT and query training; r7 improves development hit/miss/distance against r6 & Fixed AV2 confirmation fails: five adverse metric intervals exclude zero. Native seeds, features and auxiliary loss change together.\\
...
\begin{figure}[t]
\centering\includegraphics[width=\columnwidth]{figures/v73/r7_failure_slice.pdf}
...
\begin{table*}[h]
\centering\small\input{tables/v73/oracle}
...
\begin{figure*}[t]
\centering\includegraphics[width=\textwidth]{figures/v73/blender_controls_a.pdf}
...
\end{figure*}
\begin{figure*}[t]
\centering\includegraphics[width=\textwidth]{figures/v73/blender_controls_b.pdf}
...
Twenty fixed AV2 logs contain 936 actor entries, including 878 build-ready and 58 missing-input entries. Their construction was completed before quality evaluation. The original research task launched the preregistered fixed r7/r6 confirmation after DEV completion; this manuscript does not substitute its own selection or alter that run. The completed five-method actor results are archived in \path{docs/autoresearch/worldsim_v73/final_confirmation/analysis.json}, with resources and original output paths in \path{models.json} and execution provenance in \path{final_manifest.json}. The fixed external comparison is reported in Sec.~\ref{sec:external}; it contains no new training. Completed full-scene composition is reported in Sec.~\ref{sec:scene}, with \path{docs/autoresearch/worldsim_v73/final_scene/summary.json}, \path{analysis.json}, and \path{manifest.json}. New support-domain training and end-to-end orthogonal updates must not be inferred from the local forensics, external inference or numerical prototype.
