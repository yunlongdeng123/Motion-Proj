"""Evaluate frozen P249 inverse budgets on P243's first fresh read."""
...
from scripts.run_worldsim_v67_p249_tandem_cycle_inverse_budget import InverseReliabilityBudgetCompiler,_inverse_teacher,_evaluate


def main():
 p=argparse.ArgumentParser();p.add_argument('--config',type=Path,required=True);p.add_argument('--runs-root',type=Path,required=True);p.add_argument('--run-id',required=True);a=p.parse_args();c=yaml.safe_load(a.config.read_text());d=a.runs_root/'worldsim_v67'/c['task_id']/a.run_id;d.mkdir(parents=True,exist_ok=False);(d/'resolved.yaml').write_text(yaml.safe_dump(c,sort_keys=False));started=time.monotonic();rows=a.runs_root/c['p243_rows']['run']/c['p243_rows']['artifact'];student_artifact=a.runs_root/c['frozen_p249']['run']/c['frozen_p249']['artifact'];deadline=time.monotonic()+float(c['inputs_wait_timeout_seconds'])
 while not rows.is_file() or not student_artifact.is_file():
  if time.monotonic()>=deadline:raise TimeoutError(f'P243 rows or P249 artifact not ready: {rows}, {student_artifact}')
  time.sleep(5)
