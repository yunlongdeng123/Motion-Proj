...
def paired(before,after):
    ...
    for group,a in before.items():
        ...
        for metric in a['means']:
            ...
            boot=sorted(statistics.mean(rng.choices(values,k=len(values))) for _ in range(10000)) if len(values)>1 else []
            metrics[metric]={'logs':len(values),'mean_delta':statistics.mean(values) if values else None,
                'bootstrap95':[boot[249],boot[9749]] if boot else None,'per_log_delta':delta,
...
parser.add_argument('--before-pair',action='append',default=[],help='BEFORE=AFTER method names across --before and --run; fixed surfaces, no new inference')
...
result={'run':str(args.run),'aggregation':'paired equal independent log differences after within-scene ray weighting; 10000 log bootstrap samples seed7306',
    'roles':summary.get('roles',sorted({r['role'] for r in summary['frames']})),
    'boundary':summary['boundary']+'; input roles and actual independent log counts retained; no uncertainty interval for a single log',
...
if args.before:
    ...
    result['cross_run_pairs']={value:paired(before[value.split('=',1)[0]],stages[value.split('=',1)[1]]) for value in args.before_pair}
