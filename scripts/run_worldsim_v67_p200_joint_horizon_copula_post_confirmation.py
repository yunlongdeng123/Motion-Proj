"""Evaluate frozen P199 joint reliability on consumed P183 trajectories."""
...
def main():
    ...
    density,frozen_density=_load_density(a.runs_root/c["frozen_p182"]["run"]/c["frozen_p182"]["artifact"]);artifact=torch.load(a.runs_root/c["frozen_p199"]["run"]/c["frozen_p199"]["artifact"],map_location="cuda");model=JointHorizonCopula(8,artifact["hidden_dimensions"],4).cuda();model.load_state_dict(artifact["model_state_dict"]);model.eval()
    with np.load(a.runs_root/c["frozen_rows"]["run"]/c["frozen_rows"]["artifact"],allow_pickle=False) as z:arrays={n:z[n] for n in z.files}
    horizons=np.asarray(c["horizons_seconds"],dtype=np.float32);scores,clearances,costs,scenes=_align(_trajectory_payload(arrays,members,ens,float(c["boundary_state_cost"]["clearance_floor_m"])),horizons);raw=np.concatenate((scores,clearances),axis=1);features=torch.from_numpy(((raw-np.asarray(artifact["feature_mean"]))/np.asarray(artifact["feature_scale"])).astype(np.float32)).cuda();budgets=np.asarray(c["reliability_budgets"],dtype=np.float32);norms=tuple(frozen_density["norms"])
    marginal=np.stack([_predict_cdf(density,scores[:,i],np.full(len(scores),horizons[i],np.float32),clearances[:,i],budgets,norms) for i in range(4)],axis=1);candidate=_joint_probabilities(model,features,torch.from_numpy(marginal.astype(np.float32)).cuda(),int(c["evaluation"]["monte_carlo_samples"]),int(c["evaluation"]["seed"]));independent=np.prod(marginal,axis=1);target=np.all(costs[:,:,None]<=budgets[None,None,:],axis=1)
