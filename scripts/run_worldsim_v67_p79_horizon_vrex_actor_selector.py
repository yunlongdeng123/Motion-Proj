"""Train horizon-balanced V-REx rank regressors during fresh-data IO."""
...
from scripts.run_worldsim_v67_p76_group_rank_actor_selector import (
    _combine, _group_percentile_target, _load_npz, _predict, _select,
)
...
def _train(query_model, actor_model, arrays, mean, scale, config):
    ...
    horizons = torch.from_numpy(np.asarray(arrays["horizon_seconds"], dtype=np.float32)).cuda()
    horizon_values = sorted(float(value) for value in np.unique(arrays["horizon_seconds"]))
    masks = [horizons == value for value in horizon_values]
    ...
    beta, penalty = float(config["huber_beta"]), float(config["risk_variance_weight"])
    query_objective = actor_objective = torch.tensor(float("nan"), device="cuda")
    query_variance = actor_variance = torch.tensor(float("nan"), device="cuda")
    for epoch in range(int(config["epochs"])):
        optimizer.zero_grad(set_to_none=True)
        query_score, actor_score = query_model(features), actor_model(actor_features)
        query_risks = torch.stack([
            torch.nn.functional.smooth_l1_loss(query_score[mask], target[mask], beta=beta)
            for mask in masks
        ])
        actor_risks = torch.stack([
            torch.nn.functional.smooth_l1_loss(actor_score[mask], target[mask], beta=beta)
            for mask in masks
        ])
        query_variance, actor_variance = query_risks.var(unbiased=False), actor_risks.var(unbiased=False)
        query_objective = query_risks.mean() + penalty * query_variance
        actor_objective = actor_risks.mean() + penalty * actor_variance
        (query_objective + actor_objective).backward()
        optimizer.step()
        if epoch % 250 == 0 or epoch + 1 == int(config["epochs"]):
            print(
                f"horizon-vrex epoch={epoch + 1} query={float(query_objective):.6f} "
                f"actor={float(actor_objective):.6f} qvar={float(query_variance):.6g} "
                f"avar={float(actor_variance):.6g}", flush=True,
            )
    result = tuple(float(value.detach().cpu()) for value in (
        query_objective, actor_objective, query_variance, actor_variance
    ))
    del features, actor_features, target, horizons
    torch.cuda.empty_cache()
    return result, horizon_values
...
def main() -> None:
    ...
    losses, horizon_values = _train(query_model, actor_model, source, mean, scale, config["model"])
    torch.save({
    ...
    }, run_dir / "HORIZON_VREX_ACTOR_SELECTOR.pt")
    ...
    while not (evaluation_path.is_file() and all(path.is_file() for path in baseline_paths.values())):
        ...
        print("horizon-vrex training done; waiting for P75 fresh validation read", flush=True)
        time.sleep(10.0)
    ...
    best_blind_cost = min(selected_costs[name] for name in ("p76", "p77", "p78"))
    ...
    summary = {
        ...
        "training": {
            "row_count": int(len(source["target_cost"])), "horizon_values": horizon_values,
            "final_query_objective": losses[0], "final_actor_only_objective": losses[1],
            "final_query_horizon_risk_variance": losses[2],
            "final_actor_horizon_risk_variance": losses[3],
