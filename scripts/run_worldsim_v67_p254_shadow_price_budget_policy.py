"""Compile a monotone shadow-price policy over the frozen P246 utility surface."""
...
class ShadowPriceBudgetPolicy(nn.Module):
 def __init__(self,width,knot_count):
  super().__init__();self.knot_count=int(knot_count);self.encoder=nn.Sequential(nn.Linear(36,width),nn.SiLU(),nn.Linear(width,width),nn.SiLU());self.intercept=nn.Linear(width,1);self.rate_knots=nn.Linear(width,self.knot_count)
 def forward(self,feature,normalized_log_price):
  context=self.encoder(feature);rates=functional.softplus(self.rate_knots(context));width=2./(self.knot_count-1);areas=.5*(rates[:,:-1]+rates[:,1:])*width;cumulative=torch.cat((torch.zeros_like(rates[:,:1]),torch.cumsum(areas,1)),1);position=((normalized_log_price+1)/width).clamp(0,self.knot_count-1);index=torch.floor(position).long().clamp(max=self.knot_count-2);fraction=position-index;r0=torch.gather(rates,1,index[:,None]).squeeze(1);r1=torch.gather(rates,1,(index+1)[:,None]).squeeze(1);base=torch.gather(cumulative,1,index[:,None]).squeeze(1);integral=base+width*(r0*fraction+.5*(r1-r0)*fraction.square());return torch.tanh(self.intercept(context).squeeze(1)-integral)


def _teacher_grid_reward(model,feature,grid_z,chunk_size):
 outputs=[]
 with torch.no_grad():
  for start in range(0,len(feature),chunk_size):
   x=torch.from_numpy(feature[start:start+chunk_size]).cuda();columns=[model(x,torch.full((len(x),),float(value),device='cuda')).mean(1) for value in grid_z];outputs.append(torch.stack(columns,1).cpu().numpy())
 return np.concatenate(outputs)


def _optimal_grid_budget(model,feature,prices,grid_z,chunk_size):
 reward=_teacher_grid_reward(model,feature,grid_z,chunk_size);utility=reward[:,:,None]-prices[None,None,:]*(grid_z[None,:,None]+1)/2;index=np.argmax(utility,axis=1);return grid_z[index]


def _evaluate(student,teacher,feature,target,prices,price_z):
 ...
 with torch.no_grad():
  for column,value in enumerate(price_z):
   prediction=student(x,torch.full((len(x),),float(value),device='cuda'));candidate.append(prediction.cpu().numpy());candidate_reward.append(teacher(x,prediction).mean(1).cpu().numpy());truth=torch.from_numpy(target[:,column]).cuda();target_reward.append(teacher(x,truth).mean(1).cpu().numpy())
 torch.cuda.synchronize();forward=time.monotonic()-before;candidate=np.stack(candidate,1);candidate_reward=np.stack(candidate_reward,1);target_reward=np.stack(target_reward,1);candidate_utility=candidate_reward-prices[None,:]*(candidate+1)/2;target_utility=target_reward-prices[None,:]*(target+1)/2;return {'trajectory_count':int(len(feature)),'price_count':int(len(prices)),'normalized_log_budget_MAE':float(np.mean(np.abs(candidate-target))),'mean_frozen_teacher_utility_regret':float(np.mean(target_utility-candidate_utility)),'price_monotonicity_violations':int(np.sum(np.diff(candidate,axis=1)>1e-7)),'student_forward_seconds':forward}
