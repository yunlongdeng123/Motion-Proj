"""Select whole reliability curves by frozen P203 calibrated confidence."""
...
def _evaluate(calibrator,prob,truth,coverage,index=None):
 ...
 calibrated=calibrator(torch.from_numpy(prob).cuda()).detach().cpu().numpy();k=int(round(len(prob)*coverage));candidate=np.argsort(np.mean(calibrated*(1-calibrated),1),kind='stable')[:k];control=np.argsort(np.mean(prob*(1-prob),1),kind='stable')[:k]
 cb=float(np.mean((calibrated[candidate]-truth[candidate])**2));bb=float(np.mean((prob[control]-truth[control])**2));cc=float(np.mean(np.abs(calibrated[candidate].mean(0)-truth[candidate].mean(0))));bc=float(np.mean(np.abs(prob[control].mean(0)-truth[control].mean(0))));return {'trajectory_count':int(len(prob)),'authority_coverage':coverage,'calibrated_confidence_selected_integrated_brier':cb,'raw_confidence_selected_integrated_brier':bb,'selected_Brier_reduction_vs_raw_confidence':(bb-cb)/bb,'calibrated_confidence_selected_calibration_error':cc,'raw_confidence_selected_calibration_error':bc,'selected_calibration_error_reduction_vs_raw_confidence':(bc-cc)/max(bc,1e-12),'selection_overlap_fraction':float(len(np.intersect1d(candidate,control))/k)}
