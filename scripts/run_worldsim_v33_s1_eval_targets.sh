...
printf '{"status":"running","task_id":"WS-V33-S1-OBJECT-AWARE-GS-01","stage":"heldout_targets"}\n' > "$RUN_DIR/status.json"
failed() {
  code=$?
  printf '{"status":"failed","exit_code":%d,"stage":"heldout_targets"}\n' "$code" > "$RUN_DIR/status.json"
  exit "$code"
}
trap failed ERR
exec > >(tee -a "$RUN_DIR/logs/heldout_targets.log") 2>&1
cd "$PROJECT"
...
printf '{"status":"done","task_id":"WS-V33-S1-OBJECT-AWARE-GS-01","stage":"heldout_targets"}\n' > "$RUN_DIR/status.json"
trap - ERR
echo "S1 heldout targets done: $RUN_DIR"
