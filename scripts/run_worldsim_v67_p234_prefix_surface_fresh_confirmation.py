"""Confirm frozen P233 on the prospectively shared P228 fresh rows."""
...
def main():
 ...
 density,fd=_load_density(a.runs_root/c['frozen_p182']['run']/c['frozen_p182']['artifact']);p199=torch.load(a.runs_root/c['frozen_p199']['run']/c['frozen_p199']['artifact'],map_location='cuda');copula=JointHorizonCopula(8,p199['hidden_dimensions'],4).cuda();copula.load_state_dict(p199['model_state_dict']);copula.eval();p203=torch.load(a.runs_root/c['frozen_p203']['run']/c['frozen_p203']['artifact'],map_location='cuda');calibrator=MonotoneBetaCalibration().cuda();calibrator.load_state_dict(p203['model_state_dict']);calibrator.eval();h=np.asarray(c['horizons_seconds'],np.float32);budgets=np.asarray(c['reliability_budgets'],np.float32);feature,teacher,truth,scenes,teacher_seconds=_dataset(rows,members,ens,density,fd,p199,copula,calibrator,h,budgets,float(c['boundary_state_cost']['clearance_floor_m']),int(c['teacher']['monte_carlo_samples']),int(c['seed']),float(c['teacher']['ignored_future_marginal_probability']));artifact=torch.load(a.runs_root/c['frozen_p233']['run']/c['frozen_p233']['artifact'],map_location='cuda');model=MonotonePrefixSurface(artifact['hidden_dimensions']).cuda();model.load_state_dict(artifact['model_state_dict']);model.eval();result=_evaluate(model,feature,teacher,truth);decision=c['decision'];checks={'surface_teacher_fidelity':result['surface_teacher_probability_MAE']<=float(decision['maximum_surface_teacher_probability_MAE']),'final_curve_teacher_fidelity':result['final_curve_teacher_probability_MAE']<=float(decision['maximum_final_curve_teacher_probability_MAE']),'surface_quality_noninferior':result['relative_surface_Brier_degradation_vs_teacher']<=float(decision['maximum_relative_surface_Brier_degradation']) and result['absolute_surface_calibration_error_increase_vs_teacher']<=float(decision['maximum_absolute_surface_calibration_error_increase'])};verdict=c['verdict_on_pass'] if all(checks.values()) else c['verdict_on_failure'];summary={'schema_version':c['output_schema_version'],'task_id':c['task_id'],'hypothesis_id':c['hypothesis_id'],'status':'done','verdict':verdict,'role':c['role'],'scene_indices':sorted(int(value) for value in np.unique(scenes)),'fresh_secondary':result,'teacher_MC_seconds':teacher_seconds,'decision_checks':checks,'resources':{'gpu':torch.cuda.get_device_name(0),'peak_gpu_memory_gib':torch.cuda.max_memory_allocated()/2**30,'peak_rss_gib':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/2**20,'wall_seconds':time.monotonic()-started},'claim_boundary':c['claim_boundary']};(d/'summary.json').write_text(json.dumps(summary,indent=2)+'\n');(d/'status.json').write_text(json.dumps({'status':'done','completed_at_utc':datetime.now(timezone.utc).isoformat()},indent=2)+'\n');print(json.dumps({'run_dir':str(d),**summary},indent=2))
