"""Train the P182 density with uniform source-scene sampling."""
...
def _scene_balanced_randint(high: int, size: tuple[int, ...], *args, **kwargs) -> torch.Tensor:
    if (
        high == _TRAJECTORY_COUNT and tuple(size) == (_BATCH_SIZE,)
        and _SCENE_INDEX_TABLE is not None and _SCENE_LENGTHS is not None
    ):
        device = kwargs.get("device", "cuda")
        scene = _ORIGINAL_RANDINT(len(_SCENE_LENGTHS), size, device=device)
        local = torch.floor(
            torch.rand(size, device=device) * _SCENE_LENGTHS[scene].to(torch.float32)
        ).to(torch.long)
        return _SCENE_INDEX_TABLE[scene, local]
...
def _rewrite_summary(config_path: Path, runs_root: Path, run_id: str) -> None:
    ...
    control = json.loads((
        runs_root / config["frozen_p182_summary"]["run"] / "summary.json"
    ...
    checks = {
        "brier_noninferior_to_p182_every_cohort": all(
            row["scene_balanced_integrated_brier"] <= row["p182_integrated_brier"]
    ...
    summary["verdict"] = config["verdict_on_pass"] if all(checks.values()) else config["verdict_on_failure"]
    summary["role"] = config["role"]
    summary["training"]["scene_sampling"] = "uniform_scene_then_uniform_trajectory"
...
def main() -> None:
    ...
    torch.randint = _scene_balanced_randint
    try:
        p182.main()
    finally:
