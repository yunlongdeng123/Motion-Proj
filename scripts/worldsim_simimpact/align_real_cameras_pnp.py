...
for name in ['scene-0013','scene-0038','scene-0041']:
    out=R/'alignment_pnp_r1'/name;out.mkdir(parents=True,exist_ok=True)
    ...
    def render(T,K):
        ...
        return rgb,pkg['depth'][0].cpu().numpy()
    for vi,v in enumerate(views):
        row={'scene':name,'view_index':vi,'camera':v['camera'],'sample_index':v['sample_index'],'input':v['image'],'extra_reference':'Published GS RGB and depth used by PnP; not physical GT','iterations':[]}
...
(R/'alignment_pnp_r1.json').write_text(json.dumps(all_rows,indent=2))
