...
for ax,data,title,cmap,vmin,vmax,fmt in zip(axs,[E,S,A],['Early returns (%)','Executed endpoint change (m)','Change in recorded-path ADE (m)'],['YlOrRd','Blues','RdBu_r'],[0,0,-.15],[60,1.2,.15],['.1f','.2f','+.3f']):
 ...
 for (i,j),x in np.ndenumerate(data):ax.text(j,i,format(x,fmt),ha='center',va='center',fontsize=10,color='white' if (cmap=='Blues' and x>.78*vmax) else 'black')
...
ax.axhline(10,c=gray,ls='--',lw=1);ax.set(xticks=x,xticklabels=[str(r['opacity']) for r in c],xlabel='Single-Gaussian opacity',ylabel='Reported depth (m)',ylim=(0,14.5));ax.legend(loc='upper left',fontsize=10,ncol=1)
