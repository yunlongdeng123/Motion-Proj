...
W=Path(__file__).resolve().parent;D=W/'evidence_native_full';O=W.parents[1]/'outputs/Simulation_Impact_Research'
...
for col,name,title in [(0,'real_front_initial.jpg','(a) Real input at the initial pose'),(1,'render_front_initial.jpg','(b) SplatAD, 30k-step asset')]:
...
fig.text(.02,.066,'scene-0004 · complete 30,001-step fit · 64 policy/controller forecasts · logged sensor times matched in replay',fontsize=10,color='#526477')
...
for ext in ['png','pdf','svg']:fig.savefig(O/f'F14_Full_Budget_Native_Loop.{ext}',dpi=180,bbox_inches='tight')
