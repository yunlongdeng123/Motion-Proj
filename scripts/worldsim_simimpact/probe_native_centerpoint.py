...
ap=argparse.ArgumentParser();ap.add_argument('--out',type=Path,required=True);ap.add_argument('--frames',type=int,default=8);ap.add_argument('--scans-dir',type=Path);ap.add_argument('--constant-intensity',type=float);ap.add_argument('--include-real',action='store_true');ap.add_argument('--input-manifest',type=Path);a=ap.parse_args()
manifest=json.loads(a.input_manifest.read_text()) if a.input_manifest else None
if manifest:
    assert not a.scans_dir and not a.include_real and a.constant_intensity is None
    a.frames=len(manifest['frames'])
...
if a.scans_dir and a.include_real:conditions=['real',*conditions]
reg['conditions']=conditions
reg['constant_intensity_control']=a.constant_intensity
if a.scans_dir:
...
if manifest:
    reg.update(task_id=manifest['registration']['task_id'],scene=manifest['registration']['scenes'],
        role=manifest['registration']['scope'],
        inputs=str(a.input_manifest),sensor_registration=manifest['registration'],conditions=manifest['registration']['conditions'])
...
reg['dataset_split']='per_frame' if manifest else ('train' if 'scene-0004' in create_splits_scenes()['train'] else 'val')
(a.out/'registration.json').write_text(json.dumps(reg,indent=2))
...
for i in range(a.frames):
    if manifest:
        spec=manifest['frames'][i];sample=nusc.get('sample',spec['sample_token']);conditions=list(spec['points'])
    ...
    for condition in conditions:
        if manifest:
            points=np.load(spec['points'][condition])['points'];sources=spec['points'][condition]
        elif a.scans_dir and condition!='real':
        ...
        if a.constant_intensity is not None:points[:,3]=a.constant_intensity
        ...
        if manifest:row.update(scene=spec['scene'],dataset_split='train' if spec['scene'] in create_splits_scenes()['train'] else 'val')
...
reg.update(completed=True,end_unix=time.time(),detector_forwards=len(rows),failure_ledger_delta='pending_analysis')
