"""运行V6.6 P6 Actor-preserving HARP bake。"""
...
def run(config_path: Path, runs_root: Path, run_id: str) -> dict[str, object]:
    ...
    retained = sum(row["existence_state"] == "SUPPORTED_ACTOR" for row in state_rows)
    ...
    metrics = {
        ...
        "actor_state_count": state_count,
        "actor_primitive_count": int(manifest["counts"]["actor_primitive_count"]),
        "actor_state_retention": retained / state_count,
        "actor_metadata_completeness": metadata_complete / len(actor_rows),
        "actor_removed_count": actor_removed_count,
        "hidden_target_field_count": hidden_target_field_count,
        "package_file_count": len(PACKAGE_FILES),
        "package_bytes": sum(
            path.stat().st_size for path in (run_dir / "package").iterdir() if path.is_file()
        ),
    }
    ...
    gates = {
        ...
        "maximum_actor_removed_count": actor_removed_count
        <= int(gates_config["maximum_actor_removed_count"]),
        "maximum_hidden_target_field_count": hidden_target_field_count
    ...
    verdict = (
        ...
        if all(gates.values())
        else "rejected_consumed_legacy_harp_bake_capability"
    )
    summary = {
        "schema_version": "worldsim_v66.p6_harp_bake_summary.v1",
