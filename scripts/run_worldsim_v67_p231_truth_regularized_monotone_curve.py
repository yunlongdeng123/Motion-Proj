"""Train one half-teacher, half-outcome proper-loss monotone curve student."""
...
def main():
 ...
 density,fd=_load_density(a.runs_root/c['frozen_p182']['run']/c['frozen_p182']['artifact']);p199=torch.load(a.runs_root/c['frozen_p199']['run']/c['frozen_p199']['artifact'],map_location='cuda');copula=JointHorizonCopula(8,p199['hidden_dimensions'],4).cuda();copula.load_state_dict(p199['model_state_dict']);copula.eval();p203=torch.load(a.runs_root/c['frozen_p203']['run']/c['frozen_p203']['artifact'],map_location='cuda');calibrator=MonotoneBetaCalibration().cuda();calibrator.load_state_dict(p203['model_state_dict']);calibrator.eval();h=np.asarray(c['horizons_seconds'],np.float32);budgets=np.asarray(c['reliability_budgets'],np.float32);common=(members,ens,density,fd,p199,copula,calibrator,h,budgets,float(c['boundary_state_cost']['clearance_floor_m']),int(c['teacher']['monte_carlo_samples']),int(c['seed']));sf,st,sy,scenes,ss=_dataset(a.runs_root/c['source_rows']['run']/c['source_rows']['artifact'],*common);f183,t183,y183,_,s183=_dataset(a.runs_root/c['p183_rows']['run']/c['p183_rows']['artifact'],*common);f201,t201,y201,_,s201=_dataset(a.runs_root/c['p201_rows']['run']/c['p201_rows']['artifact'],*common);dev=scenes%int(c['split']['development_scene_modulus'])==int(c['split']['development_scene_remainder']);train_idx=torch.from_numpy(np.flatnonzero(~dev)).cuda();x=torch.from_numpy(sf).cuda();teacher=torch.from_numpy(st).cuda();truth=torch.from_numpy(sy).cuda();m=c['student'];student=MonotoneCurveStudent(m['hidden_dimensions']).cuda();opt=torch.optim.AdamW(student.parameters(),lr=float(m['learning_rate']),weight_decay=float(m['weight_decay']));tw=float(m['teacher_weight']);yw=float(m['truth_weight']);last=0.
 for step in range(int(m['steps'])):
  idx=train_idx[torch.randint(len(train_idx),(int(m['batch_size']),),device='cuda')];prediction=student(x[idx]);loss=tw*functional.mse_loss(prediction,teacher[idx])+yw*functional.mse_loss(prediction,truth[idx]);opt.zero_grad(set_to_none=True);loss.backward();opt.step();last=float(loss.detach())
  if step%500==0:print(f'P231 truth-regularized curve step={step+1} loss={last:.7f}',flush=True)
 source=_evaluate(student,sf,st,sy,dev);r183=_evaluate(student,f183,t183,y183);r201=_evaluate(student,f201,t201,y201);decision=c['decision'];checks={'P201_teacher_probability_fidelity':r201['student_teacher_probability_MAE']<=float(decision['maximum_P201_teacher_probability_MAE']),'P201_integrated_Brier_strictly_better_than_teacher':r201['student_integrated_brier']<r201['teacher_integrated_brier'],'P201_calibration_noninferior':r201['absolute_calibration_error_increase_vs_teacher']<=float(decision['maximum_absolute_P201_calibration_error_increase'])};verdict=c['verdict_on_pass'] if all(checks.values()) else c['verdict_on_failure'];torch.save({'model_state_dict':student.state_dict(),'hidden_dimensions':m['hidden_dimensions'],'input_dimension':36,'output_budget_count':7},d/c['model_artifact']);summary={'schema_version':c['output_schema_version'],'task_id':c['task_id'],'hypothesis_id':c['hypothesis_id'],'status':'done','verdict':verdict,'role':c['role'],'training':{'trajectory_count':int((~dev).sum()),'final_mixed_proper_loss':last,'teacher_weight':tw,'truth_weight':yw},'source_development':source,'P183_consumed_development':r183,'P201_post_hoc_development':r201,'teacher_MC_seconds':{'source':ss,'P183':s183,'P201':s201},'decision_checks':checks,'resources':{'gpu':torch.cuda.get_device_name(0),'peak_gpu_memory_gib':torch.cuda.max_memory_allocated()/2**30,'peak_rss_gib':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/2**20,'wall_seconds':time.monotonic()-started},'claim_boundary':c['claim_boundary']};(d/'summary.json').write_text(json.dumps(summary,indent=2)+'\n');(d/'status.json').write_text(json.dumps({'status':'done','completed_at_utc':datetime.now(timezone.utc).isoformat()},indent=2)+'\n');print(json.dumps({'run_dir':str(d),**summary},indent=2))
