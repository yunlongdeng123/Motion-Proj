...
tail='''<section><h2>接下来怎样推进</h2><p>用户最新反馈：“目标选中没问题，原位 factual 问题不大，就是删除了以后背景补全就是糊的了”。已据此把下一轮优先级改为背景补全：固定当前目标、GLB与放置，先核对ProPainter直接RGB结果、mask对车身/轮胎/阴影的覆盖与合成接缝，再做一次有依据的同模型控制。停止长度扫参；生成参考的围栏问题降为次要。</p><p>人工verdict保持空白。几何检查使用GT与LiDAR作为诊断输入，不声称RGB端到端自动。<a href="report.md">技术记录与边界</a> · <a href="summary.json">完整统计</a> · <a href="validation.json">产物检查</a></p></section></main><script>
