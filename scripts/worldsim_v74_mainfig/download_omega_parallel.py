...
url='https://huggingface.co/1kaiser/vggt-omega-jax/resolve/main/vggt_omega_1b_512.pt';dest=D/'vggt_omega_1b_512.pt';part=dest.with_suffix('.pt.part');chunks=D/'omega512_chunks';chunks.mkdir(exist_ok=True)
...
record={'source':url,'user_authorized_mirror':True,'mirror_identity':'User-provided mirror; strict official architecture loading checked separately','file':str(dest),'bytes':total,'version':'original512, not416reproduction','elapsed_s':time.time()-started,'range_streams':8}
(D/'omega_download.json').write_text(json.dumps(record,indent=2));print(json.dumps(record),flush=True)
