...
def main():
 ...
 for f in sorted((R/'inputs').glob('*.json')):
  ...
  for variant,n in [('six',6),('twelve',12)]:
   out=R/'predictions'/a.method/m['scene']/variant;out.mkdir(parents=True,exist_ok=True)
   if (out/'result.json').exists():continue
   views=m['views'][:n];paths=[v['image'] for v in views]
   try:
    if a.method=='vggt':images=load_and_preprocess_images(paths,mode='crop').cuda()
     ...
     for i,v in enumerate(views):
      d=inp/f'frame_{i//6}' if a.method=='dvgt1' else inp;d.mkdir(exist_ok=True)
      link=d/f'{i:02d}_{v["camera"]}.jpg'
      if not link.exists():link.symlink_to(v['image'])
     images=load_and_preprocess_images(str(inp),mode='crop').cuda() if a.method=='dvgt1' else load_images_as_tensor(str(inp),verbose=False)[None].cuda()
    ...
    keys={'vggt':['depth','depth_conf','world_points','world_points_conf','pose_enc'],'omega512':['depth','depth_conf','pose_enc'],'dvgt1':['points','points_conf','absolute_ego_pose_enc'],'pi3x':['points','local_points','camera_poses','conf']}[a.method]
