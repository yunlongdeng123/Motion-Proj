"""Prospectively evaluate frozen P102 on the still-unread P96 cohort."""
...
from scripts.run_worldsim_v67_p96_occupancy_flip_confirmation import _group_any
from scripts.run_worldsim_v67_p102_hierarchical_temporal_interaction import (
    HierarchicalTemporalInteractionRisk,
)


def _augment(raw: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    arrays = dict(raw)
    features = np.asarray(arrays["features"], dtype=np.float32)
    horizon = features[:, 14].clip(min=1e-3)
    relative_position = features[:, 6:8]
    relative_velocity = features[:, 8:10].copy()
    relative_velocity[:, 0] += features[:, 10] * (1.0 - features[:, 19])
    relative_velocity[:, 1] -= features[:, 20] / horizon
    fractions = np.linspace(0.0, 1.0, 9, dtype=np.float32)
    relative_path = relative_position[:, None, :] + (
        relative_velocity[:, None, :] * horizon[:, None, None] * fractions[None, :, None]
    )
    distance = np.linalg.norm(relative_path, axis=2)
    radius = np.asarray(arrays["occupancy_interaction_radius_m"], dtype=np.float32)
    signed = distance - radius[:, None]
    arrays["features"] = np.concatenate((features, signed, np.abs(signed)), axis=1).astype(np.float32)
    return arrays
...
def main() -> None:
    ...
    while not row_path.is_file():
        if time.monotonic() >= deadline:
            raise TimeoutError(f"P96 confirmation rows not ready: {row_path}")
        time.sleep(10.0)
    raw = dict(np.load(row_path, allow_pickle=False))
    augmented = _augment(raw)
    model_config = config["model"]
    evaluation = _build_sets(
        augmented, float(config["evaluation"]["visited_region_radius_m"]),
        float(config["evaluation"]["unreliable_actor_state_error_m"]),
        int(model_config["maximum_visited_actors"]),
    )
    checkpoint = torch.load(
        args.runs_root / config["frozen_p102"]["run"] / config["frozen_p102"]["artifact"],
        map_location="cuda",
    )
    ...
    query_model = HierarchicalTemporalInteractionRisk(
        42, checkpoint["element_dimensions"], checkpoint["decoder_dimensions"],
    ).cuda()
    actor_model = HierarchicalTemporalInteractionRisk(
    ...
    frozen_row_score = predict_reliability(
        frozen_model.eval(), raw["features"][:, :len(FEATURE_NAMES)],
