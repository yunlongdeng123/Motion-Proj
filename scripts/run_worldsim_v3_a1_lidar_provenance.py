#!/usr/bin/env python
"""WorldSim V3 A1 最小 LiDAR provenance 正式控制器。"""
...
from scripts.run_worldsim_v3_a1_endpoints import resource_guard
...
def main() -> None:
    ...
    parser.add_argument(
        "--audit-config",
        type=Path,
        default=PROJECT / "configs/worldsim_v3/a1_lidar_provenance_v1.yaml",
    )
    parser.add_argument("--timeout-seconds", type=float, default=3600)
    parser.add_argument(
        "--max-heldout-images",
        type=int,
        help="仅供工程 smoke；正式审计必须省略。",
    )
    ...
    if json.loads(source_terminal_path.read_text(encoding="utf-8")).get(
        "status"
    ) != "done":
    ...
    shutil.copy2(args.audit_config, args.run_dir / "resolved_config.yaml")
    atomic_json(
    ...
    sources = (
        PROJECT / "scripts/run_worldsim_v3_a1_lidar_provenance.py",
        PROJECT / "scripts/eval_worldsim_v3_a1_lidar_provenance.py",
        ...
        PROJECT / "motion_proj/worldsim_v3/lidar_provenance.py",
    ...
    output_dir = args.run_dir / "artifacts" / "lidar_provenance"
    command = [
        str(DRIVESTUDIO_ENV / "bin/python"),
        str(PROJECT / "scripts/eval_worldsim_v3_a1_lidar_provenance.py"),
        "--source-summary",
    ...
    if args.max_heldout_images is not None:
        command.extend(["--max-heldout-images", str(args.max_heldout_images)])
    ...
    fingerprint = {
        ...
        "source_initialization_provenance_sha256": source[
            "initialization_provenance"
        ]["sha256"],
        "audit_config_sha256": sha256_file(args.run_dir / "resolved_config.yaml"),
    }
    ...
    manifest = {
        ...
        "seed": int(contract["runtime_initialization"]["seed"]),
        "split": "heldout test split",
        "formal_full_split": args.max_heldout_images is None,
    ...
    def validate() -> tuple[bool, dict[str, object]]:
        ...
        ok = (
            ...
            and payload.get("provenance_comparison", {}).get(
                "recorded_lidar_actor_inputs_exact"
            )
            is True
            and payload.get("provenance_comparison", {}).get(
                "rigid_initial_gaussian_count_exact"
            )
            is True
            and payload.get("raw_inputs", {}).get("frame_block_count") == 196
            and payload.get("runtime_sampled_inputs", {}).get("actor_count") == 24
            and depth.get("status") in {"done", "ABSTAIN"}
            and depth.get("formal_full_split")
            == (args.max_heldout_images is None)
        ...
        return ok, {
            ...
            "recorded_lidar_actor_inputs_exact": payload.get(
                "provenance_comparison", {}
            ).get("recorded_lidar_actor_inputs_exact"),
            "reproduction_exact_sha_match": payload.get(
                "reproduction_exact_sha_match"
            ),
            "raw_frame_block_count": payload.get("raw_inputs", {}).get(
                "frame_block_count"
            ),
            "actor_count": payload.get("runtime_sampled_inputs", {}).get(
                "actor_count"
            ),
    ...
    audit_summary_path = output_dir / "summary.json"
    audit_summary = json.loads(audit_summary_path.read_text(encoding="utf-8"))
    shutil.copy2(audit_summary_path, args.run_dir / "metrics.json")
    summary = {
        "status": "done",
        "task_id": contract["task_id"],
        "component": "A1 minimal LiDAR initialization provenance",
        ...
        "audit_config_sha256": fingerprint["audit_config_sha256"],
        "audit": audit_summary,
        "resources": {
            key: stage[key]
            for key in (
        ...
        if _ACTIVE_RUN_DIR is not None and _ACTIVE_RUN_DIR.is_dir() and not _TERMINAL_FINAL:
            atomic_json(
                _ACTIVE_RUN_DIR / "terminal.json",
                {
                    ...
                    "updated_at": now(),
                    "failure": {
                        "code": "A1_LIDAR_PROVENANCE_FAILED",
