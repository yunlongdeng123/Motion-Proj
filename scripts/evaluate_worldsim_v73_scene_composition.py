...
def main():
    ...
    save('manifest.json',{'code_commit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),
        ...
        'method':'per-owner canonical CPU BVH + inverse rigid rays + global nearest positive distance, same fixed build background',
        ...
        'evaluation':'existing development logs only; extra-time rays read for evaluation, no optimizer',
    ...
    try:
        ...
        for scene in scenes:
            ...
            bg=np.load(folder/'background.npz'); background=SurfaceBVH(bg['vertices_world_m'],bg['faces'])
            frames=[]
            for frame in scene['frames']:
                raw=np.load(folder/frame['file']); origins=raw['origin_world_m']; directions=raw['directions_world']
                frames.append((frame,{key:raw[key] for key in raw.files},background.cast(origins,directions)))
        ...
        save('summary.json',{'status':'done','frames':rows,'supports':supports,'statistics':aggregate(rows),
            'wall_s':time.monotonic()-started,'peak_rss_gib':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/2**20,
            'boundary':'scene metric on existing logs; box attribution/boundary proxy; finite build background incomplete; no counterfactual GT'})
