#!/usr/bin/env python3
"""Recover SurfNCC with frozen proxy primal-dual rate constraints."""
...
TASK_ID = "WS-V63-P5R-CONSTRAINED-SURFNCC-TRAIN-01"
CONSTRAINTS = (
    "safe_occ_retention",
...
def _constraint_terms(
    ...
    authority = outputs["authority"].float()
    ...
    veto = eligible & (projected == OCCUPIED_INDEX) & (
...
def run(config_path: Path, repo_root: Path, run_dir: Path) -> dict[str, Any]:
    ...
    if subprocess.check_output(["git", "status", "--porcelain"], cwd=repo_root, text=True).strip():
        raise RuntimeError("P5R formal requires clean source")
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    if config["task_id"] != TASK_ID:
        raise ValueError("P5R task identity drift")
    ...
    if shutil.disk_usage(disk_probe).free / 1024**3 < float(
        ...
        raise RuntimeError("insufficient disk before P5R")
    if not torch.cuda.is_available():
        raise RuntimeError("P5R requires CUDA")
        ...
        for epoch in range(max_epochs):
            ...
            if progress_improved:
                ...
                torch.save(
                    ...
                    run_dir / "SURFNCC_RECOVERY_BEST_PROGRESS.pt",
                )
            ...
            if candidate_improved:
                ...
                torch.save(
                    _checkpoint_payload(model, optimizer, epoch, selection, multipliers),
                    run_dir / "SURFNCC_RECOVERY_BEST_CANDIDATE.pt",
                )
            ...
            _write_json(run_dir / "P5R_EPOCHS.json", epoch_rows)
    ...
    candidate_promotable = (
        ...
        and (run_dir / "SURFNCC_RECOVERY_BEST_CANDIDATE.pt").is_file()
    )
    passed = (
        finite_training
        and hard_violations == 0
        and best_progress_epoch is not None
        and (run_dir / "SURFNCC_RECOVERY_BEST_PROGRESS.pt").is_file()
        and peak_gib <= float(config["resources"]["gpu_peak_gib_max"])
    )
    summary = {
        "schema_version": "worldsim_v63.p5r_constrained_train_summary.v1",
        "task_id": TASK_ID,
    ...
    _write_json(run_dir / "P5R_TRAIN_SUMMARY.json", summary)
    _write_json(
        run_dir / "P5R_TRAIN_MANIFEST.json",
        {
            "task_id": TASK_ID,
