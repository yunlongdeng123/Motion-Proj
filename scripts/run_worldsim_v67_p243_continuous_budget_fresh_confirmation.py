"""Confirm the frozen P242 continuous-budget surface on unused scenes."""
...
from scripts.run_worldsim_v67_p241_integrated_monotone_budget_surface import IntegratedMonotonePrefixSurface,_evaluate
...
def main():
 ...
 while pending:
  ready=[name for name,path in pending.items() if (path/'instances'/'instances_info.json').is_file() and (path/'lidar_pose').is_dir()]
  for name in ready:pending.pop(name)
  if pending:
   if time.monotonic()>=deadline:raise TimeoutError(f'P243 scenes not ready: {sorted(pending)}')
   time.sleep(5)
 parts=[]
 ...
 arrays={name:np.concatenate([part[name] for part in parts]) for name in parts[0]};partial=d/'P243_CONTINUOUS_BUDGET_ROWS.partial.npz';np.savez_compressed(partial,**arrays);rows=d/c['model_artifact'];partial.replace(rows);torch.cuda.reset_peak_memory_stats();ens=torch.load(a.runs_root/c['frozen_p126']['run']/c['frozen_p126']['artifact'],map_location='cuda');members=[]
