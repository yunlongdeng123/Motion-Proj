"""Train a variable-set four-horizon simplex alpha-fair dual compiler."""
...
from scripts.run_worldsim_v67_p266_simplex_horizon_group_dual import _allocate,_target_prices,_utility


class VariableSetSimplexHorizonDual(nn.Module):
 def __init__(self,element_width,context_width,knot_count):
  super().__init__();self.knot_count=int(knot_count);self.element=nn.Sequential(nn.Linear(36,element_width),nn.SiLU(),nn.Linear(element_width,element_width),nn.SiLU());self.context=nn.Sequential(nn.Linear(3*element_width+6,context_width),nn.SiLU(),nn.Linear(context_width,context_width),nn.SiLU());self.intercept=nn.Linear(context_width,1);self.rate_knots=nn.Linear(context_width,self.knot_count)
 def forward(self,groups,mask,normalized_fraction,alpha,horizon_weights):
  encoded=self.element(groups);weight=mask[:,:,None];count=weight.sum(1).clamp(min=1);mean=(encoded*weight).sum(1)/count;variance=((encoded-mean[:,None]).square()*weight).sum(1)/count;maximum=encoded.masked_fill(~mask[:,:,None],-torch.inf).amax(1);size=torch.log2(count.squeeze(1))/7.;context=self.context(torch.cat((mean,torch.sqrt(variance+1e-6),maximum,size[:,None],alpha[:,None],horizon_weights),1));rates=F.softplus(self.rate_knots(context));width=2./(self.knot_count-1);areas=.5*(rates[:,:-1]+rates[:,1:])*width;cumulative=torch.cat((torch.zeros_like(rates[:,:1]),torch.cumsum(areas,1)),1);position=((normalized_fraction+1)/width).clamp(0,self.knot_count-1);index=torch.floor(position).long().clamp(max=self.knot_count-2);fraction=position-index;r0=torch.gather(rates,1,index[:,None]).squeeze(1);r1=torch.gather(rates,1,(index+1)[:,None]).squeeze(1);base=torch.gather(cumulative,1,index[:,None]).squeeze(1);integral=base+width*(r0*fraction+.5*(r1-r0)*fraction.square());return torch.tanh(self.intercept(context).squeeze(1)-integral)


def _predict(student,padded,mask,alphas,weight_vectors,fractions,batch):
 result=[]
 with torch.no_grad():
  for alpha in alphas:
   by_weight=[]
   for weights in weight_vectors:
    ...
    for fraction in fractions:
     ...
     for start in range(0,len(padded),batch):
      stop=start+batch;g=torch.from_numpy(padded[start:stop]).cuda();m=torch.from_numpy(mask[start:stop]).cuda();wv=torch.from_numpy(np.broadcast_to(weights,(len(g),4)).copy()).cuda();values.append(student(g,m,torch.full((len(g),),float(2*fraction-1),device='cuda'),torch.full((len(g),),float(alpha),device='cuda'),wv).cpu().numpy())
     by_fraction.append(np.concatenate(values))
    by_weight.append(np.stack(by_fraction,1))
   result.append(np.stack(by_weight,1))
 return np.stack(result,1)


def _evaluate_size(student,policy,teacher,groups,target,low,high,alphas,weight_vectors,fractions,mean,scale,epsilon,chunk,batch):
 padded,mask,_=_pad([groups]);torch.cuda.synchronize();started=time.monotonic();candidate=_predict(student,padded,mask,alphas,weight_vectors,fractions,batch);torch.cuda.synchronize();forward=time.monotonic()-started;candidate_budget=_allocate(policy,groups,candidate.astype(np.float32),alphas,weight_vectors,chunk);target_budget=_allocate(policy,groups,target.astype(np.float32),alphas,weight_vectors,chunk);candidate_cost=np.mean((candidate_budget+1)/2,4);target_cost=np.mean((target_budget+1)/2,4);attained=(candidate_cost-low[:,:,:,None])/(high-low)[:,:,:,None].clip(min=1e-6);candidate_value=_utility(teacher,groups,candidate_budget.astype(np.float32),alphas,weight_vectors,epsilon,chunk);target_value=_utility(teacher,groups,target_budget.astype(np.float32),alphas,weight_vectors,epsilon,chunk);price=np.exp(mean+scale*target);regret=(target_value-price*target_cost)-(candidate_value-price*candidate_cost);return {'group_count':int(len(groups)),'group_size':int(groups.shape[1]),'normalized_log_price_MAE':float(np.mean(np.abs(candidate-target))),'attained_budget_fraction_MAE':float(np.mean(np.abs(attained-fractions[None,None,None,:]))),'mean_frozen_simplex_alpha_fair_Lagrangian_utility_regret':float(np.mean(regret)),'fraction_price_monotonicity_violations':int(np.sum(np.diff(candidate,axis=3)>1e-7)),'student_forward_seconds':forward}


def _aggregate(rows):
 weights=np.asarray([r['group_count'] for r in rows],np.float64);total=float(weights.sum());keys=['normalized_log_price_MAE','attained_budget_fraction_MAE','mean_frozen_simplex_alpha_fair_Lagrangian_utility_regret'];result={k:float(sum(w*r[k] for w,r in zip(weights,rows))/total) for k in keys};result.update({'group_count':int(total),'heldout_group_sizes':[r['group_size'] for r in rows],'fraction_price_monotonicity_violations':int(sum(r['fraction_price_monotonicity_violations'] for r in rows)),'per_group_size':rows});return result
...
def main():
 ...
 for size in train_sizes:
  groups,scenes=_groups(sf,ss,size);target,_,_=_target_prices(policy,groups,alphas,weights,fractions,steps,chunk);group_sets.append(groups);scene_sets.append(scenes);target_sets.append(target)
 padded,mask,_=_pad(group_sets);scene_labels=np.concatenate(scene_sets);targets=np.concatenate(target_sets);dev=scene_labels%int(c['split']['development_scene_modulus'])==int(c['split']['development_scene_remainder']);m=c['student'];student=VariableSetSimplexHorizonDual(int(m['element_width']),int(m['context_width']),int(m['rate_knot_count'])).cuda();opt=torch.optim.AdamW(student.parameters(),lr=float(m['learning_rate']),weight_decay=float(m['weight_decay']));pg=torch.from_numpy(padded).cuda();pm=torch.from_numpy(mask).cuda();target=torch.from_numpy(targets).cuda();fz=torch.from_numpy(2*fractions-1).cuda();at=torch.from_numpy(alphas).cuda();wt=torch.from_numpy(weights).cuda();train_idx=torch.from_numpy(np.flatnonzero(~dev)).cuda();last=0.
 for step in range(int(m['steps'])):
  idx=train_idx[torch.randint(len(train_idx),(int(m['batch_size']),),device='cuda')];ai=torch.randint(len(alphas),(len(idx),),device='cuda');wi=torch.randint(len(weights),(len(idx),),device='cuda');fi=torch.randint(len(fractions),(len(idx),),device='cuda');prediction=student(pg[idx],pm[idx],fz[fi],at[ai],wt[wi]);loss=F.l1_loss(prediction,target[idx,ai,wi,fi]);opt.zero_grad(set_to_none=True);loss.backward();opt.step();last=float(loss.detach())
  if step%500==0:print(f'P267 variable simplex dual step={step+1} price_mae={last:.7f}',flush=True)
