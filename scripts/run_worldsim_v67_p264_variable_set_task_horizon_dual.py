"""Train a variable-set task-horizon alpha-fair dual compiler."""
...
from scripts.run_worldsim_v67_p263_task_horizon_group_dual import _allocate,_target_prices,_utility


class VariableSetTaskHorizonDual(nn.Module):
 def __init__(self,element_width,context_width,knot_count):
  super().__init__();self.knot_count=int(knot_count);self.element=nn.Sequential(nn.Linear(36,element_width),nn.SiLU(),nn.Linear(element_width,element_width),nn.SiLU());self.context=nn.Sequential(nn.Linear(3*element_width+3,context_width),nn.SiLU(),nn.Linear(context_width,context_width),nn.SiLU());self.intercept=nn.Linear(context_width,1);self.rate_knots=nn.Linear(context_width,self.knot_count)
 def forward(self,groups,mask,normalized_fraction,alpha,preference):
  encoded=self.element(groups);weight=mask[:,:,None];count=weight.sum(1).clamp(min=1);mean=(encoded*weight).sum(1)/count;variance=((encoded-mean[:,None]).square()*weight).sum(1)/count;maximum=encoded.masked_fill(~mask[:,:,None],-torch.inf).amax(1);size=torch.log2(count.squeeze(1))/7.;context=self.context(torch.cat((mean,torch.sqrt(variance+1e-6),maximum,size[:,None],alpha[:,None],preference[:,None]),1));rates=F.softplus(self.rate_knots(context));width=2./(self.knot_count-1);areas=.5*(rates[:,:-1]+rates[:,1:])*width;cumulative=torch.cat((torch.zeros_like(rates[:,:1]),torch.cumsum(areas,1)),1);position=((normalized_fraction+1)/width).clamp(0,self.knot_count-1);index=torch.floor(position).long().clamp(max=self.knot_count-2);fraction=position-index;r0=torch.gather(rates,1,index[:,None]).squeeze(1);r1=torch.gather(rates,1,(index+1)[:,None]).squeeze(1);base=torch.gather(cumulative,1,index[:,None]).squeeze(1);integral=base+width*(r0*fraction+.5*(r1-r0)*fraction.square());return torch.tanh(self.intercept(context).squeeze(1)-integral)


def _predict(student,padded,mask,alphas,preferences,fractions,batch):
 result=[]
 with torch.no_grad():
  for alpha in alphas:
   by_preference=[]
   for preference in preferences:
    ...
    for fraction in fractions:
     ...
     for start in range(0,len(padded),batch):
      stop=start+batch;g=torch.from_numpy(padded[start:stop]).cuda();m=torch.from_numpy(mask[start:stop]).cuda();values.append(student(g,m,torch.full((len(g),),float(2*fraction-1),device='cuda'),torch.full((len(g),),float(alpha),device='cuda'),torch.full((len(g),),float(preference),device='cuda')).cpu().numpy())
     by_fraction.append(np.concatenate(values))
    by_preference.append(np.stack(by_fraction,1))
   result.append(np.stack(by_preference,1))
 return np.stack(result,1)


def _evaluate_size(student,policy,teacher,groups,target,low,high,alphas,preferences,fractions,mean,scale,epsilon,positions,logit_scale,chunk,batch):
 padded,mask,_=_pad([groups]);torch.cuda.synchronize();started=time.monotonic();candidate=_predict(student,padded,mask,alphas,preferences,fractions,batch);torch.cuda.synchronize();forward=time.monotonic()-started;candidate_budget=_allocate(policy,groups,candidate.astype(np.float32),alphas,preferences,chunk);target_budget=_allocate(policy,groups,target.astype(np.float32),alphas,preferences,chunk);candidate_cost=np.mean((candidate_budget+1)/2,4);target_cost=np.mean((target_budget+1)/2,4);attained=(candidate_cost-low[:,:,:,None])/(high-low)[:,:,:,None].clip(min=1e-6);candidate_value=_utility(teacher,groups,candidate_budget.astype(np.float32),alphas,preferences,epsilon,positions,logit_scale,chunk);target_value=_utility(teacher,groups,target_budget.astype(np.float32),alphas,preferences,epsilon,positions,logit_scale,chunk);price=np.exp(mean+scale*target);regret=(target_value-price*target_cost)-(candidate_value-price*candidate_cost);return {'group_count':int(len(groups)),'group_size':int(groups.shape[1]),'normalized_log_price_MAE':float(np.mean(np.abs(candidate-target))),'attained_budget_fraction_MAE':float(np.mean(np.abs(attained-fractions[None,None,None,:]))),'mean_frozen_task_alpha_fair_Lagrangian_utility_regret':float(np.mean(regret)),'fraction_price_monotonicity_violations':int(np.sum(np.diff(candidate,axis=3)>1e-7)),'student_forward_seconds':forward}


def _aggregate(rows):
 weights=np.asarray([r['group_count'] for r in rows],np.float64);total=float(weights.sum());keys=['normalized_log_price_MAE','attained_budget_fraction_MAE','mean_frozen_task_alpha_fair_Lagrangian_utility_regret'];result={k:float(sum(w*r[k] for w,r in zip(weights,rows))/total) for k in keys};result.update({'group_count':int(total),'heldout_group_sizes':[r['group_size'] for r in rows],'fraction_price_monotonicity_violations':int(sum(r['fraction_price_monotonicity_violations'] for r in rows)),'per_group_size':rows});return result
...
def main():
 ...
 for size in train_sizes:
  groups,scenes=_groups(sf,ss,size);target,_,_=_target_prices(policy,groups,alphas,preferences,fractions,steps,chunk);group_sets.append(groups);scene_sets.append(scenes);target_sets.append(target)
 padded,mask,_=_pad(group_sets);scene_labels=np.concatenate(scene_sets);targets=np.concatenate(target_sets);dev=scene_labels%int(c['split']['development_scene_modulus'])==int(c['split']['development_scene_remainder']);m=c['student'];student=VariableSetTaskHorizonDual(int(m['element_width']),int(m['context_width']),int(m['rate_knot_count'])).cuda();opt=torch.optim.AdamW(student.parameters(),lr=float(m['learning_rate']),weight_decay=float(m['weight_decay']));pg=torch.from_numpy(padded).cuda();pm=torch.from_numpy(mask).cuda();target=torch.from_numpy(targets).cuda();fz=torch.from_numpy(2*fractions-1).cuda();at=torch.from_numpy(alphas).cuda();pt=torch.from_numpy(preferences).cuda();train_idx=torch.from_numpy(np.flatnonzero(~dev)).cuda();last=0.
 for step in range(int(m['steps'])):
  idx=train_idx[torch.randint(len(train_idx),(int(m['batch_size']),),device='cuda')];ai=torch.randint(len(alphas),(len(idx),),device='cuda');pi=torch.randint(len(preferences),(len(idx),),device='cuda');fi=torch.randint(len(fractions),(len(idx),),device='cuda');prediction=student(pg[idx],pm[idx],fz[fi],at[ai],pt[pi]);loss=F.l1_loss(prediction,target[idx,ai,pi,fi]);opt.zero_grad(set_to_none=True);loss.backward();opt.step();last=float(loss.detach())
  if step%500==0:print(f'P264 variable task dual step={step+1} price_mae={last:.7f}',flush=True)
 mean=float(artifact['shadow_price_log_mean']);scale=float(artifact['shadow_price_log_scale']);epsilon=float(artifact['alpha_utility_epsilon']);positions=np.asarray(artifact['horizon_positions'],np.float32);logit_scale=float(artifact['horizon_preference_logit_scale']);batch=int(c['student_inference_batch_size']);collections=[]
