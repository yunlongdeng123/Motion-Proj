...
class ContextAnchoredRiskWarp(nn.Module):
    """Positive two-sided quantile warp slopes around an exact central anchor."""

    def __init__(self, input_width: int, hidden_dimensions, maximum_slope: float) -> None:
        super().__init__()
        layers = []
        width = int(input_width)
        for hidden in hidden_dimensions:
    ...
    def forward(self, features):
        return self.maximum_slope * torch.sigmoid(self.network(features))


class DifferentiableAuthorityTemperature(nn.Module):
    """Context-conditioned bounded temperature for nested authority relaxation."""

    def __init__(self, input_width: int, hidden_dimensions, minimum_temperature: float, maximum_temperature: float) -> None:
        super().__init__()
        layers = []
        width = int(input_width)
        for hidden in hidden_dimensions:
    ...
    def forward(self, features):
        ...
        return self.minimum_temperature + (self.maximum_temperature - self.minimum_temperature) * unit


class DecisionFocusedWarpMultiplier(nn.Module):
    """Positive low/high risk-warp multipliers for decision-focused authority training."""

    def __init__(self, input_width: int, hidden_dimensions, minimum_multiplier: float, maximum_multiplier: float) -> None:
        super().__init__()
        layers = []
        width = int(input_width)
        for hidden in hidden_dimensions:
    ...
    def forward(self, features):
        unit = torch.sigmoid(self.network(features))
        return self.minimum_multiplier + (self.maximum_multiplier - self.minimum_multiplier) * unit


class DirectVisitedReliabilityHead(nn.Module):
    def __init__(self, input_width, hidden_dimensions, set_size_count):
        ...
        layers.append(nn.Linear(width, int(set_size_count)))
        self.network = nn.Sequential(*layers)

    def forward(self, features, normalized_horizon, log_cost_ceiling):
        raw_logits = self.network(torch.cat((features, normalized_horizon[:, None], log_cost_ceiling[:, None]), 1))
...
def main() -> None:
    ...
    horizon_anchor_preserving_context_risk_warp_training = bool(config.get("horizon_anchor_preserving_context_risk_warp_training", False))
    horizon_differentiable_risk_ceiling_authority_training = bool(config.get("horizon_differentiable_risk_ceiling_authority_training", False))
    horizon_decision_focused_differentiable_authority_training = bool(config.get("horizon_decision_focused_differentiable_authority_training", False))
    horizon_direct_reliability_authority_training = bool(config.get("horizon_direct_reliability_authority_training", False))
    ...
    if horizon_quantile_training or horizon_quantile_confirmation or horizon_selective_authority_training or horizon_temporal_calibration_training or horizon_risk_matched_quantile_training or horizon_risk_conditioned_surface_training or horizon_implicit_quantile_surface_training or horizon_spline_quantile_surface_training or horizon_size_conditioned_authority_training or horizon_risk_size_authority_training or horizon_lattice_risk_size_authority_training or horizon_locally_adaptive_lattice_calibration_training or horizon_risk_locally_adaptive_lattice_calibration_training or horizon_anchor_preserving_continuous_risk_training or horizon_anchor_preserving_risk_curvature_training or horizon_anchor_preserving_context_risk_warp_training or horizon_differentiable_risk_ceiling_authority_training or horizon_decision_focused_differentiable_authority_training or horizon_direct_reliability_authority_training:
        ...
        if horizon_quantile_confirmation:
            ...
            return
        if horizon_direct_reliability_authority_training:
            frozen_lattice = torch.load(
                args.runs_root / config["frozen_lattice_authority"]["run"] / config["frozen_lattice_authority"]["artifact"], map_location="cuda"
            )
            ...
            split = config["reliability_split"]
            ...
            for step in range(int(config["calibration"]["steps"])):
                row = calibration_index[torch.randint(len(calibration_index), (int(model_config["batch_size"]),), device="cuda")]
                local_horizon_index = training_horizon_tensor[
                    torch.randint(len(training_horizon_tensor), (len(row),), device="cuda")
                ]
                local_ceiling_index = torch.randint(len(ceiling_tensor), (len(row),), device="cuda")
                ...
                if horizon_group_calibration:
                    ...
                    calibrated_logits = calibrated_logits + calibration_horizon_slope[:, local_ceiling_index, :] * (
                        horizon_tensor[local_horizon_index][None, :, None]
                    )
                elif memberwise_calibration:
                    calibrated_logits = raw_logits / log_temperature[:, None, None].exp() + calibration_bias[:, None, None]
                ...
                target = (y[row, :, local_horizon_index] > local_ceiling[:, None]).float()
                if memberwise_calibration:
                    target = target[None].expand(len(ensemble_fit_remainders), -1, -1)
            ...
            def reliability_frontier(features, targets, conditions, row_mask=None):
                indices = np.arange(len(features)) if row_mask is None else np.flatnonzero(row_mask)
                index_tensor = torch.from_numpy(indices).cuda()
                local_x = features[index_tensor]
                    ...
                    for ceiling_index, ceiling in enumerate(ceilings):
                        ...
                        if memberwise_calibration:
                            ...
                            for member_index, member in enumerate(model.members):
                                logits = member(local_x, local_h, local_ceiling)
                                if group_calibration:
                                    ...
                                    logits = logits / log_temperature[member_index].exp() + calibration_bias[member_index]
                                member_probability = torch.sigmoid(logits).cpu().numpy()
                                if isotonic_calibration:
                                    local_logits = logits.cpu().numpy()
                                    for set_size_index in range(len(set_sizes)):
                                        local_map = isotonic_maps[member_index][ceiling_index][set_size_index]
                                        member_probability[:, set_size_index] = apply_binary_isotonic_map(
                ...
                local_conditions = conditions[indices]
                frontier, previous_sizes = {}, None
                ceiling_violations = 0
                quantile_sizes = []
                for q_value in evaluation_quantiles:
                    by_ceiling, coverages, unsafe_rates = {}, [], []
                    current_sizes = []
                    for ceiling_index, ceiling in enumerate(ceilings):
                        feasible = probabilities[:, ceiling_index] <= 1.0 - float(q_value)
                        selected_position = feasible.sum(1) - 1
                        admitted = selected_position >= 0
                        selected_size = np.where(admitted, set_sizes[np.maximum(selected_position, 0)], 0)
                        selected_target = np.zeros(len(indices), np.float32)
                        row_index = np.arange(len(indices))
                        selected_target[admitted] = local_target[row_index[admitted], selected_position[admitted]]
                        unsafe_rate = float(np.mean(selected_target[admitted] > ceiling)) if admitted.any() else 0.0
                        by_condition = {}
                        for progress, command in np.unique(local_conditions, axis=0):
                            condition_mask = np.isclose(local_conditions[:, 0], progress) & np.isclose(local_conditions[:, 1], command)
                            local_admitted = condition_mask & admitted
                            by_condition[f"progress={float(progress)},command={float(command)}"] = {
                                "any_authority_coverage": float(local_admitted.sum() / max(int(condition_mask.sum()), 1)),
                                "unsafe_selected_set_rate": float(np.mean(selected_target[local_admitted] > ceiling)) if local_admitted.any() else 0.0,
                            }
                        by_ceiling[f"{float(ceiling):.6f}"] = {
                            "log_cost_ceiling": float(ceiling), "admitted_count": int(admitted.sum()),
                            "any_authority_coverage": float(np.mean(admitted)), "unsafe_selected_set_rate": unsafe_rate,
                            "mean_selected_set_size": float(np.mean(selected_size[admitted])) if admitted.any() else 0.0,
                            "by_task_condition": by_condition,
                        }
                        coverages.append(float(np.mean(admitted)))
                        unsafe_rates.append(unsafe_rate)
                        current_sizes.append(selected_size)
                    for left, right in zip(current_sizes[:-1], current_sizes[1:]):
                        ceiling_violations += int(np.sum(right < left))
                    quantile_sizes.append(np.stack(current_sizes, 1))
                    frontier[f"{float(q_value):.2f}"] = {
                        "mean_any_authority_coverage": float(np.mean(coverages)),
                        "maximum_unsafe_selected_set_rate": float(np.max(unsafe_rates)),
                        "by_ceiling": by_ceiling,
                    }
                quantile_violations = sum(
                    int(np.sum(right > left)) for left, right in zip(quantile_sizes[:-1], quantile_sizes[1:])
                )
                return frontier, ceiling_violations, quantile_violations, probabilities

            source_frontier, source_ceiling_violations, source_quantile_violations, source_probability = reliability_frontier(
                source_x, source_target, source_conditions, development_rows
            )
            p201_frontier, p201_ceiling_violations, p201_quantile_violations, p201_probability = reliability_frontier(
                p201_x, p201_target, p201_conditions
            )
            anchor = p201_frontier["0.90"]
            decision = config["decision"]
            checks = {
                "P201_q90_hard_risk": anchor["maximum_unsafe_selected_set_rate"] <= float(decision["maximum_P201_unsafe_selected_set_rate"]),
                "P201_q90_hard_coverage": anchor["mean_any_authority_coverage"] >= float(decision["minimum_P201_mean_any_authority_coverage"]),
                "P201_ceiling_monotonicity": p201_ceiling_violations == 0,
                "P201_requested_risk_monotonicity": p201_quantile_violations == 0,
            }
            verdict = config["verdict_on_pass"] if all(checks.values()) else config["verdict_on_failure"]
            torch.save({
            ...
            summary = {
                ...
                "P201_probability_range": [float(p201_probability.min()), float(p201_probability.max())],
                "monotonicity": {"source_ceiling": source_ceiling_violations, "source_requested_risk": source_quantile_violations,
                                 "P201_ceiling": p201_ceiling_violations, "P201_requested_risk": p201_quantile_violations},
                "decision_checks": checks,
            ...
            print(json.dumps({"run_dir": str(run_dir), **summary}, indent=2))
            return
        if horizon_decision_focused_differentiable_authority_training:
            frozen_lattice = torch.load(
                args.runs_root / config["frozen_lattice_authority"]["run"] / config["frozen_lattice_authority"]["artifact"], map_location="cuda"
            )
            frozen_anchor = torch.load(
                args.runs_root / config["frozen_local_anchor"]["run"] / config["frozen_local_anchor"]["artifact"], map_location="cuda"
            )
            frozen_curvature = torch.load(
                args.runs_root / config["frozen_risk_curvature"]["run"] / config["frozen_risk_curvature"]["artifact"], map_location="cuda"
            )
            frozen_warp = torch.load(
                args.runs_root / config["frozen_context_risk_warp"]["run"] / config["frozen_context_risk_warp"]["artifact"], map_location="cuda"
            )
            frozen_temperature = torch.load(
                args.runs_root / config["frozen_differentiable_authority"]["run"] / config["frozen_differentiable_authority"]["artifact"], map_location="cuda"
            )
            feature_mean = np.asarray(frozen_lattice["input_mean"], np.float32)
            feature_scale = np.asarray(frozen_lattice["input_scale"], np.float32)
            source_feature = ((source_feature - feature_mean) / feature_scale).astype(np.float32)
            ...
            lattice_model.eval()
            for parameter in lattice_model.parameters():
            ...
            anchor_model.eval()
            curvature_model = ContextPositiveRiskCurvature(
                int(frozen_curvature["input_width"]), frozen_curvature["hidden_dimensions"]
            ).cuda()
            curvature_model.load_state_dict(frozen_curvature["risk_slope_state_dict"])
            curvature_model.eval()
            warp_model = ContextAnchoredRiskWarp(
                int(frozen_warp["input_width"]), frozen_warp["hidden_dimensions"], float(frozen_warp["maximum_slope"])
            ).cuda()
            warp_model.load_state_dict(frozen_warp["risk_warp_state_dict"])
            warp_model.eval()
            temperature_model = DifferentiableAuthorityTemperature(
                int(frozen_temperature["input_width"]), frozen_temperature["hidden_dimensions"],
                float(frozen_temperature["temperature_range"][0]), float(frozen_temperature["temperature_range"][1]),
            ).cuda()
            temperature_model.load_state_dict(frozen_temperature["temperature_model_state_dict"])
            ...
            with torch.no_grad():
                source_anchor_scale = torch.stack(
                    [anchor_model(source_x, torch.full((len(source_x),), float(value), device="cuda")) for value in normalized_horizons], 2
                )
                p201_anchor_scale = torch.stack(
                    [anchor_model(p201_x, torch.full((len(p201_x),), float(value), device="cuda")) for value in normalized_horizons], 2
                )
                source_shape, p201_shape = curvature_model(source_x), curvature_model(p201_x)
                source_base_warp, p201_base_warp = warp_model(source_x), warp_model(p201_x)
            quantile_range = np.asarray(config["requested_quantile_range"], np.float32)

            def decision_score(features, normalized_horizon, requested_q, anchor_scale, shape, base_warp, multiplier):
                requested_delta = requested_q - anchor_q
                adjusted_warp = base_warp * multiplier
                effective_q = anchor_q + requested_delta * torch.where(
                    requested_delta < 0, adjusted_warp[:, 0], adjusted_warp[:, 1]
                )
                effective_q = torch.clamp(effective_q, float(quantile_range[0]), float(quantile_range[1]))
                effective_delta = effective_q - anchor_q
                deformation = effective_delta[:, None] * shape[:, :1] + torch.where(
                    effective_delta[:, None] < 0, -(effective_delta[:, None] ** 2) * shape[:, 1:2],
                    (effective_delta[:, None] ** 2) * shape[:, 2:3],
                )
                return lattice_model(features, normalized_horizon, effective_q) + anchor_threshold * anchor_scale + deformation

            split = config["decision_split"]
            modulus = int(split["scene_modulus"])
            unique_split_scenes, scene_rank = np.unique(source_example_scenes, return_inverse=True)
            row_fold = (np.arange(len(unique_split_scenes), dtype=np.int64) % modulus)[scene_rank]
            fit_rows = row_fold == int(split["fit_scene_remainder"])
            development_rows = row_fold == int(split["development_scene_remainder"])
            residual_config = config["warp_multiplier_model"]
            residual_model = DecisionFocusedWarpMultiplier(
                source_feature.shape[1] + 2, residual_config["hidden_dimensions"],
                float(residual_config["minimum_multiplier"]), float(residual_config["maximum_multiplier"]),
            ).cuda()
            optimizer = torch.optim.AdamW(
            ...
            def frontier(features, anchor_scale, shape, base_warp, target, conditions, row_mask=None):
                indices = np.arange(len(features)) if row_mask is None else np.flatnonzero(row_mask)
                index_tensor = torch.from_numpy(indices).cuda()
                local_x = features[index_tensor]
                local_scale = anchor_scale[index_tensor, :, heldout_horizon_index]
                local_shape, local_base_warp = shape[index_tensor], base_warp[index_tensor]
                local_h = torch.full((len(local_x),), float(normalized_horizons[heldout_horizon_index]), device="cuda")
                local_target = target[indices, :, heldout_horizon_index]
                local_conditions = conditions[indices]
                adjusted, baseline, multipliers = {}, {}, []
                with torch.no_grad():
                    for q_value in evaluation_quantiles:
                        q_tensor = torch.full((len(local_x),), float(q_value), device="cuda")
                        all_ceiling_scores = []
                        all_baseline_scores = []
                        for ceiling in ceilings:
                            ceiling_tensor_local = torch.full((len(local_x),), float(ceiling), device="cuda")
                            multiplier_input = torch.cat((local_x, local_h[:, None], ceiling_tensor_local[:, None]), 1)
                            multiplier = residual_model(multiplier_input)
                            multipliers.append(multiplier.cpu().numpy())
                            all_ceiling_scores.append(decision_score(local_x, local_h, q_tensor, local_scale, local_shape, local_base_warp, multiplier).cpu().numpy())
                            all_baseline_scores.append(decision_score(local_x, local_h, q_tensor, local_scale, local_shape, local_base_warp, torch.ones_like(multiplier)).cpu().numpy())
                        # The score is ceiling-conditioned in P340, so evaluate each ceiling with its matched score.
                        adjusted_by_ceiling, baseline_by_ceiling = {}, {}
                        adjusted_coverages, adjusted_unsafe, baseline_coverages, baseline_unsafe = [], [], [], []
                        for ceiling_index, ceiling in enumerate(ceilings):
                            adjusted_metric = _variable_set_metrics(
                                all_ceiling_scores[ceiling_index], local_target, local_conditions,
                                np.asarray([ceiling], np.float32), set_sizes,
                            )
                            baseline_metric = _variable_set_metrics(
                                all_baseline_scores[ceiling_index], local_target, local_conditions,
                                np.asarray([ceiling], np.float32), set_sizes,
                            )
                            adjusted_by_ceiling[f"{float(ceiling):.6f}"] = adjusted_metric["by_ceiling"][str(float(ceiling))]
                            baseline_by_ceiling[f"{float(ceiling):.6f}"] = baseline_metric["by_ceiling"][str(float(ceiling))]
                            adjusted_coverages.append(adjusted_metric["mean_any_authority_coverage"])
                            adjusted_unsafe.append(adjusted_metric["maximum_unsafe_selected_set_rate"])
                            baseline_coverages.append(baseline_metric["mean_any_authority_coverage"])
                            baseline_unsafe.append(baseline_metric["maximum_unsafe_selected_set_rate"])
                        adjusted[f"{float(q_value):.2f}"] = {
                            "mean_any_authority_coverage": float(np.mean(adjusted_coverages)),
                            "maximum_unsafe_selected_set_rate": float(np.max(adjusted_unsafe)),
                            "by_ceiling": adjusted_by_ceiling,
                        }
                        baseline[f"{float(q_value):.2f}"] = {
                            "mean_any_authority_coverage": float(np.mean(baseline_coverages)),
                            "maximum_unsafe_selected_set_rate": float(np.max(baseline_unsafe)),
                            "by_ceiling": baseline_by_ceiling,
                        }
                multiplier_values = np.concatenate(multipliers)
                return adjusted, baseline, {
                    "mean": float(np.mean(multiplier_values)),
                    "range": [float(np.min(multiplier_values)), float(np.max(multiplier_values))],
                }

            source_adjusted, source_baseline, source_multiplier = frontier(
                source_x, source_anchor_scale, source_shape, source_base_warp, source_target, source_conditions, development_rows
            )
            p201_adjusted, p201_baseline, p201_multiplier = frontier(
                p201_x, p201_anchor_scale, p201_shape, p201_base_warp, p201_target, p201_conditions
            )
            nonanchor_keys = [f"{float(q):.2f}" for q in evaluation_quantiles if abs(float(q) - anchor_q) > 1e-6]
            adjusted_mean_coverage = float(np.mean([p201_adjusted[key]["mean_any_authority_coverage"] for key in nonanchor_keys]))
            baseline_mean_coverage = float(np.mean([p201_baseline[key]["mean_any_authority_coverage"] for key in nonanchor_keys]))
            adjusted_risk_excess = float(np.max([
                p201_adjusted[key]["maximum_unsafe_selected_set_rate"] - (1.0 - float(key)) for key in nonanchor_keys
            ]))
            baseline_risk_excess = float(np.max([
                p201_baseline[key]["maximum_unsafe_selected_set_rate"] - (1.0 - float(key)) for key in nonanchor_keys
            ]))
            anchor_metrics = p201_adjusted[f"{anchor_q:.2f}"]
            decision = config["decision"]
            checks = {
                "P201_q90_hard_risk": anchor_metrics["maximum_unsafe_selected_set_rate"] <= float(decision["maximum_P201_unsafe_selected_set_rate"]),
                "P201_q90_hard_coverage": anchor_metrics["mean_any_authority_coverage"] >= float(decision["minimum_P201_mean_any_authority_coverage"]),
                "P201_nonanchor_mean_coverage_not_worse_than_P337": adjusted_mean_coverage >= baseline_mean_coverage,
                "P201_nonanchor_nominal_risk_excess_not_worse_than_P337": adjusted_risk_excess <= baseline_risk_excess,
            }
            verdict = config["verdict_on_pass"] if all(checks.values()) else config["verdict_on_failure"]
            torch.save({
                "warp_multiplier_state_dict": residual_model.state_dict(), "input_width": source_feature.shape[1] + 2,
            ...
            summary = {
                ...
                "P201_warp_multiplier": p201_multiplier,
                "P201_nonanchor_mean_coverage": adjusted_mean_coverage,
                "P201_P337_nonanchor_mean_coverage": baseline_mean_coverage,
                "P201_nonanchor_nominal_risk_excess": adjusted_risk_excess,
                "P201_P337_nonanchor_nominal_risk_excess": baseline_risk_excess,
                "decision_checks": checks,
                "resources": {"gpu": torch.cuda.get_device_name(0), "peak_gpu_memory_gib": torch.cuda.max_memory_allocated() / 2 ** 30,
                              "peak_rss_gib": resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 2 ** 20,
                              "wall_seconds": time.monotonic() - started},
            ...
            print(json.dumps({"run_dir": str(run_dir), **summary}, indent=2))
            return
        if horizon_differentiable_risk_ceiling_authority_training:
            frozen_lattice = torch.load(
                args.runs_root / config["frozen_lattice_authority"]["run"] / config["frozen_lattice_authority"]["artifact"], map_location="cuda"
            )
            frozen_anchor = torch.load(
            ...
            with torch.no_grad():
                ...
                source_shape, p201_shape = curvature_model(source_x), curvature_model(p201_x)
                source_warp, p201_warp = warp_model(source_x), warp_model(p201_x)

            quantile_range = np.asarray(config["requested_quantile_range"], np.float32)

            def compiled_score(features, normalized_horizon, requested_q, anchor_scale, shape, warp):
                requested_delta = requested_q - anchor_q
                effective_q = anchor_q + requested_delta * torch.where(
                    requested_delta < 0, warp[:, 0], warp[:, 1]
                )
                effective_q = torch.clamp(effective_q, float(quantile_range[0]), float(quantile_range[1]))
                effective_delta = effective_q - anchor_q
                deformation = effective_delta[:, None] * shape[:, :1] + torch.where(
            ...
            split = config["distillation_split"]
            modulus = int(split["scene_modulus"])
            unique_split_scenes, scene_rank = np.unique(source_example_scenes, return_inverse=True)
            row_fold = (np.arange(len(unique_split_scenes), dtype=np.int64) % modulus)[scene_rank]
            fit_rows = row_fold == int(split["fit_scene_remainder"])
            development_rows = row_fold == int(split["development_scene_remainder"])
            temperature_config = config["temperature_model"]
            temperature_model = DifferentiableAuthorityTemperature(
                source_feature.shape[1] + 3, temperature_config["hidden_dimensions"],
                float(temperature_config["minimum_temperature"]), float(temperature_config["maximum_temperature"]),
            ).cuda()
            optimizer = torch.optim.AdamW(
                temperature_model.parameters(), lr=float(temperature_config["learning_rate"]),
                weight_decay=float(temperature_config["weight_decay"]),
            )
            fit_index = torch.from_numpy(np.flatnonzero(fit_rows)).cuda()
            training_horizon_tensor = torch.from_numpy(training_horizon_indices).cuda()
            ceiling_tensor = torch.from_numpy(ceilings).cuda()
            last = 0.0
            for step in range(int(temperature_config["steps"])):
                row = fit_index[torch.randint(len(fit_index), (int(temperature_config["batch_size"]),), device="cuda")]
                local_horizon_index = training_horizon_tensor[
                    torch.randint(len(training_horizon_tensor), (len(row),), device="cuda")
                ]
                requested_q = torch.empty(len(row), device="cuda").uniform_(
                ...
                local_ceiling = ceiling_tensor[torch.randint(len(ceiling_tensor), (len(row),), device="cuda")]
                with torch.no_grad():
                    scores = compiled_score(
                        source_x[row], horizon_tensor[local_horizon_index], requested_q,
                        source_anchor_scale[row, :, local_horizon_index], source_shape[row], source_warp[row],
                    )
                    teacher_size = torch.sum(scores <= local_ceiling[:, None], 1)
                temperature_input = torch.cat((
                    source_x[row], horizon_tensor[local_horizon_index, None], requested_q[:, None], local_ceiling[:, None]
                ), 1)
                temperature = temperature_model(temperature_input)
                feasibility = torch.sigmoid((local_ceiling[:, None] - scores) / temperature[:, None])
                size_probability = torch.stack((
                    1.0 - feasibility[:, 0], feasibility[:, 0] - feasibility[:, 1],
                    feasibility[:, 1] - feasibility[:, 2], feasibility[:, 2],
                ), 1).clamp_min(1e-7)
                size_probability = size_probability / size_probability.sum(1, keepdim=True)
                loss = -torch.log(size_probability[torch.arange(len(row), device="cuda"), teacher_size]).mean()
                optimizer.zero_grad(set_to_none=True)
                loss.backward()
                optimizer.step()
                last = float(loss.detach())
                if step % 500 == 0:
                    print(f"{config['task_id']} temperature step={step + 1} loss={last:.7f}", flush=True)
            temperature_model.eval()

            evaluation_quantiles = np.asarray(config["evaluation_quantiles"], np.float32)

            def relaxation_metrics(features, anchor_scale, shape, warp, row_mask=None):
                indices = np.arange(len(features)) if row_mask is None else np.flatnonzero(row_mask)
                index_tensor = torch.from_numpy(indices).cuda()
                local_x = features[index_tensor]
                local_scale = anchor_scale[index_tensor, :, heldout_horizon_index]
                local_shape, local_warp = shape[index_tensor], warp[index_tensor]
                local_h = torch.full((len(local_x),), float(normalized_horizons[heldout_horizon_index]), device="cuda")
                by_request, absolute_errors, correct, temperatures = {}, [], [], []
                with torch.no_grad():
                    for q_value in evaluation_quantiles:
                        q_tensor = torch.full((len(local_x),), float(q_value), device="cuda")
                        scores = compiled_score(local_x, local_h, q_tensor, local_scale, local_shape, local_warp)
                        for ceiling in ceilings:
                            ceiling_value = torch.full((len(local_x),), float(ceiling), device="cuda")
                            teacher_size = torch.sum(scores <= ceiling_value[:, None], 1)
                            temperature_input = torch.cat((local_x, local_h[:, None], q_tensor[:, None], ceiling_value[:, None]), 1)
                            temperature = temperature_model(temperature_input)
                            feasibility = torch.sigmoid((ceiling_value[:, None] - scores) / temperature[:, None])
                            probability = torch.stack((
                                1.0 - feasibility[:, 0], feasibility[:, 0] - feasibility[:, 1],
                                feasibility[:, 1] - feasibility[:, 2], feasibility[:, 2],
                            ), 1).clamp_min(0.0)
                            probability = probability / probability.sum(1, keepdim=True).clamp_min(1e-7)
                            expected_size = torch.sum(probability * torch.arange(4, device="cuda")[None], 1)
                            prediction = torch.argmax(probability, 1)
                            error = torch.abs(expected_size - teacher_size)
                            absolute_errors.append(error.cpu().numpy())
                            correct.append((prediction == teacher_size).cpu().numpy())
                            temperatures.append(temperature.cpu().numpy())
                            by_request[f"q={float(q_value):.2f},ceiling={float(ceiling):.6f}"] = {
                                "expected_size_MAE": float(error.mean()),
                                "hard_class_accuracy": float((prediction == teacher_size).float().mean()),
                                "mean_temperature": float(temperature.mean()),
                            }
                return {
                    "example_count": int(len(indices)), "request_count": int(len(by_request)),
                    "expected_size_MAE": float(np.mean(np.concatenate(absolute_errors))),
                    "hard_class_accuracy": float(np.mean(np.concatenate(correct))),
                    "mean_temperature": float(np.mean(np.concatenate(temperatures))),
                    "temperature_range": [float(np.min(np.concatenate(temperatures))), float(np.max(np.concatenate(temperatures)))],
                    "by_request": by_request,
                }

            source_relaxation = relaxation_metrics(source_x, source_anchor_scale, source_shape, source_warp, development_rows)
            p201_relaxation = relaxation_metrics(p201_x, p201_anchor_scale, p201_shape, p201_warp)
            with torch.no_grad():
                p201_h = torch.full((len(p201_x),), float(normalized_horizons[heldout_horizon_index]), device="cuda")
                p201_q = torch.full((len(p201_x),), anchor_q, device="cuda")
                p201_q90_score = compiled_score(
                    p201_x, p201_h, p201_q, p201_anchor_scale[:, :, heldout_horizon_index], p201_shape, p201_warp
                ).cpu().numpy()
            p201_hard_metrics = _variable_set_metrics(
                p201_q90_score, p201_target[:, :, heldout_horizon_index], p201_conditions, ceilings, set_sizes
            )
            decision = config["decision"]
            checks = {
                "P201_soft_expected_size_fidelity": p201_relaxation["expected_size_MAE"] <= float(decision["maximum_P201_expected_size_MAE"]),
            ...
            torch.save({
                ...
                "hidden_dimensions": temperature_config["hidden_dimensions"],
                "temperature_range": [float(temperature_config["minimum_temperature"]), float(temperature_config["maximum_temperature"])],
                "frozen_context_risk_warp": config["frozen_context_risk_warp"],
            }, run_dir / config["model_artifact"])
            summary = {
                "schema_version": config["output_schema_version"], "task_id": config["task_id"],
                "hypothesis_id": config["hypothesis_id"], "status": "done", "verdict": verdict, "role": config["role"],
                "training": {"example_count": int(fit_rows.sum()), "steps": int(temperature_config["steps"]),
                             "final_distillation_loss": last},
                "source_differentiable_authority_development": source_relaxation,
                "P201_differentiable_authority": p201_relaxation,
                "P201_frozen_P337_hard_q90_authority": p201_hard_metrics,
                "decision_checks": checks,
                "resources": {"gpu": torch.cuda.get_device_name(0), "peak_gpu_memory_gib": torch.cuda.max_memory_allocated() / 2 ** 30,
                              "peak_rss_gib": resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 2 ** 20,
                              "wall_seconds": time.monotonic() - started},
            ...
            print(json.dumps({"run_dir": str(run_dir), **summary}, indent=2))
            return
        if horizon_anchor_preserving_context_risk_warp_training:
            frozen_lattice = torch.load(
            ...
            frozen_curvature = torch.load(
                args.runs_root / config["frozen_risk_curvature"]["run"] / config["frozen_risk_curvature"]["artifact"],
                map_location="cuda",
            )
            feature_mean = np.asarray(frozen_lattice["input_mean"], np.float32)
            feature_scale = np.asarray(frozen_lattice["input_scale"], np.float32)
            ...
            with torch.no_grad():
                ...
                source_shape = curvature_model(source_x)
            ...
            optimizer = torch.optim.AdamW(
                risk_warp.parameters(), lr=float(warp_config["learning_rate"]), weight_decay=float(warp_config["weight_decay"])
            )
            y = torch.from_numpy(source_target).cuda()
            fit_index = torch.from_numpy(np.flatnonzero(warp_fit_rows)).cuda()
            training_horizon_tensor = torch.from_numpy(training_horizon_indices).cuda()
            training_quantile_range = np.asarray(warp_config["training_quantile_range"], np.float32)
            last = 0.0
            for step in range(int(warp_config["steps"])):
                row = fit_index[torch.randint(len(fit_index), (int(warp_config["batch_size"]),), device="cuda")]
                local_horizon_index = training_horizon_tensor[
                    torch.randint(len(training_horizon_tensor), (len(row),), device="cuda")
                ]
                ...
                warp_slopes = risk_warp(source_x[row])
                requested_delta = requested_q - anchor_q
                effective_q = anchor_q + requested_delta * torch.where(
                    requested_delta < 0, warp_slopes[:, 0], warp_slopes[:, 1]
                )
                effective_q = torch.clamp(effective_q, float(training_quantile_range[0]), float(training_quantile_range[1]))
                effective_delta = effective_q - anchor_q
                local_shape = source_shape[row]
                deformation = effective_delta * local_shape[:, 0] + torch.where(
                    effective_delta < 0, -(effective_delta ** 2) * local_shape[:, 1],
                    (effective_delta ** 2) * local_shape[:, 2],
                )
                raw_prediction = lattice_model(source_x[row], horizon_tensor[local_horizon_index], effective_q)[
                    torch.arange(len(row), device="cuda"), local_size_index
                ]
                anchor_margin = anchor_threshold * source_anchor_scale[row, local_size_index, local_horizon_index]
                prediction = raw_prediction + anchor_margin + deformation
                error = y[row, local_size_index, local_horizon_index] - prediction
                loss = torch.maximum(requested_q * error, (requested_q - 1.0) * error).mean()
                optimizer.zero_grad(set_to_none=True)
                loss.backward()
                optimizer.step()
                last = float(loss.detach())
                if step % 500 == 0:
                    print(f"{config['task_id']} warp step={step + 1} loss={last:.7f}", flush=True)
            risk_warp.eval()
            evaluation_quantiles = np.asarray(config["evaluation_quantiles"], np.float32)
            source_frontier, p201_frontier = {}, {}
            source_score_stack, p201_score_stack, baseline_source_score_stack = [], [], []
            with torch.no_grad():
                source_warp_slopes = risk_warp(source_x)
                p201_warp_slopes = risk_warp(p201_x)
                ...
                for q_value in evaluation_quantiles:
                    requested_delta = float(q_value) - anchor_q
                    source_effective_q = anchor_q + requested_delta * source_warp_slopes[:, 0 if requested_delta < 0 else 1]
                    p201_effective_q = anchor_q + requested_delta * p201_warp_slopes[:, 0 if requested_delta < 0 else 1]
                    source_effective_q = torch.clamp(source_effective_q, float(training_quantile_range[0]), float(training_quantile_range[1]))
                    p201_effective_q = torch.clamp(p201_effective_q, float(training_quantile_range[0]), float(training_quantile_range[1]))
                    source_delta = source_effective_q - anchor_q
                    p201_delta = p201_effective_q - anchor_q
                    source_deformation = source_delta[:, None] * source_shape[:, :1] + torch.where(
                        source_delta[:, None] < 0, -(source_delta[:, None] ** 2) * source_shape[:, 1:2],
                        (source_delta[:, None] ** 2) * source_shape[:, 2:3],
                    )
                    p201_deformation = p201_delta[:, None] * p201_shape[:, :1] + torch.where(
                        p201_delta[:, None] < 0, -(p201_delta[:, None] ** 2) * p201_shape[:, 1:2],
                        (p201_delta[:, None] ** 2) * p201_shape[:, 2:3],
                    )
                    source_score = lattice_model(source_x, heldout_h, source_effective_q) + anchor_threshold * source_anchor_scale[:, :, heldout_horizon_index] + source_deformation
                    p201_score = lattice_model(p201_x, p201_heldout_h, p201_effective_q) + anchor_threshold * p201_anchor_scale[:, :, heldout_horizon_index] + p201_deformation
                    baseline_q = torch.full((len(source_x),), float(q_value), device="cuda")
                    baseline_delta = float(q_value) - anchor_q
                    baseline_deformation = baseline_delta * source_shape[:, :1] + (
                        -(baseline_delta ** 2) * source_shape[:, 1:2] if baseline_delta < 0
                        else (baseline_delta ** 2) * source_shape[:, 2:3]
                    )
                    baseline_source_score = lattice_model(source_x, heldout_h, baseline_q) + anchor_threshold * source_anchor_scale[:, :, heldout_horizon_index] + baseline_deformation
                    ...
                    baseline_source_score_stack.append(baseline_source_score.cpu().numpy())
                    source_frontier[f"{float(q_value):.2f}"] = _variable_set_metrics(
                        source_score_np[development_rows], source_target[development_rows, :, heldout_horizon_index],
                        source_conditions[development_rows], np.asarray(frozen_lattice["heldout_log_cost_ceilings"], np.float32), set_sizes,
                    )
            ...
            source_pinball, baseline_pinball = [], []
            source_local_target = source_target[development_rows, :, heldout_horizon_index]
            for quantile_index, q_value in enumerate(evaluation_quantiles):
                error = source_local_target - source_score_stack[quantile_index][development_rows]
                baseline_error = source_local_target - baseline_source_score_stack[quantile_index][development_rows]
                source_pinball.append(float(np.mean(np.maximum(float(q_value) * error, (float(q_value) - 1.0) * error))))
                baseline_pinball.append(float(np.mean(np.maximum(float(q_value) * baseline_error, (float(q_value) - 1.0) * baseline_error))))
            mean_source_pinball = float(np.mean(source_pinball))
            mean_baseline_pinball = float(np.mean(baseline_pinball))
            p201_metrics = p201_frontier[f"{anchor_q:.2f}"]
            decision = config["decision"]
            checks = {
                ...
                "source_context_warp_pinball_not_worse_than_P336": mean_source_pinball <= mean_baseline_pinball,
            }
            verdict = config["verdict_on_pass"] if all(checks.values()) else config["verdict_on_failure"]
            torch.save({
                "risk_warp_state_dict": risk_warp.state_dict(), "input_width": source_feature.shape[1],
                "hidden_dimensions": warp_config["hidden_dimensions"], "maximum_slope": float(warp_config["maximum_slope"]),
                "anchor_quantile_level": anchor_q, "training_quantile_range": training_quantile_range,
                "frozen_risk_curvature": config["frozen_risk_curvature"],
            }, run_dir / config["model_artifact"])
            summary = {
                "schema_version": config["output_schema_version"], "task_id": config["task_id"],
                "hypothesis_id": config["hypothesis_id"], "status": "done", "verdict": verdict, "role": config["role"],
                ...
                "P336_frozen_source_heldout_mean_quantile_pinball": mean_baseline_pinball,
                "decision_checks": checks,
                "resources": {"gpu": torch.cuda.get_device_name(0), "peak_gpu_memory_gib": torch.cuda.max_memory_allocated() / 2 ** 30,
                              "peak_rss_gib": resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 2 ** 20,
                              "wall_seconds": time.monotonic() - started},
