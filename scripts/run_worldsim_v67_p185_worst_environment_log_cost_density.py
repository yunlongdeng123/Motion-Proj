"""Train a log-cost density against the worst contiguous source-scene environment."""
...
def _evaluate(
    ...
    if not np.array_equal(scenes, cost_scenes):
        raise RuntimeError("P185 trajectory grouping is not aligned")
    ...
    return {
        ...
        "worst_environment_integrated_brier": brier, "p182_integrated_brier": p182_brier,
        "brier_change_vs_p182": float((brier - p182_brier) / max(p182_brier, 1e-12)),
        "worst_environment_mean_absolute_reliability_error": error,
...
def main() -> None:
    ...
    if not np.array_equal(source_scenes, cost_scenes):
        raise RuntimeError("P185 source grouping is not aligned")
    ...
    scene_groups = np.array_split(unique_scenes, int(model_config["environment_count"]))
    environment_pools = [torch.from_numpy(np.flatnonzero(
        np.isin(source_scenes, group)
    ).astype(np.int64)).cuda() for group in scene_groups]
    ...
    final_environment_nll = [0.0] * len(environment_pools)
    temperature = float(model_config["worst_environment_temperature"])
    torch.backends.cuda.matmul.allow_tf32 = True
    torch.cuda.reset_peak_memory_stats()
    for step in range(int(model_config["training_steps"])):
        losses = []
        for pool in environment_pools:
            pool_index = torch.randint(
                len(pool), (int(model_config["batch_size_per_environment"]),), device="cuda",
            )
        ...
        objective = temperature * torch.logsumexp(stacked / temperature, dim=0)
        optimizer.zero_grad(set_to_none=True)
        objective.backward()
        optimizer.step()
        final_environment_nll = [float(value.detach().cpu()) for value in losses]
        if step % 500 == 0 or step + 1 == int(model_config["training_steps"]):
            formatted = "/".join(f"{value:.5f}" for value in final_environment_nll)
            print(f"P185 worst-environment step={step + 1} nll={formatted}", flush=True)
    ...
    checks = {
        "brier_noninferior_to_p182_every_cohort": all(
            float(row["worst_environment_integrated_brier"])
    ...
    summary = {
        ...
        "training": {
            ...
            "environment_scene_counts": [int(len(group)) for group in scene_groups],
            "final_environment_nll": final_environment_nll,
        },
