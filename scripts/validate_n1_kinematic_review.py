#!/usr/bin/env python
"""校验并汇总第三版 N1 的用户人审 JSONL；不代填 verdict、不启动 N2。"""
...
def validate(run_dir: Path, review_file: Path) -> dict:
    ...
    for relative, expected in manifest["immutable_file_hashes"].items():
    ...
    for audit_id in sorted(template_by_id):
        evidence_path = audit_dir / "evidence" / f"{audit_id}.json"
        ...
        if not isinstance(codes, list) or any(code not in FAILURE_CODES for code in codes):
            raise ValueError(f"failure_codes 非法: {audit_id}")
        ...
        if overall == "TRUE_POSITIVE":
            ...
            if "UNCERTAIN" not in components or "INVALID" in components:
    ...
    checks = {
        "machine_research_support": bool(run_summary["machine_gate_passed"]),
    ...
    return {
        "schema_version": "n1-kinematic-human-review-validation-v1",
        "run_dir": str(run_dir),
        ...
        "recommended_research_verdict": (
            "n1_kinematic_human_audit_pass"
            if passed
            else "reject_n1_kinematic_after_human_audit"
        ),
        "decision_note": "仅聚合人工填写；最终 verdict 仍须由用户确认并另建 adjudication run。",
        "n2_authorized": False,
    }
...
def main() -> None:
    ...
    summary = validate(args.run_dir, args.review_file)
