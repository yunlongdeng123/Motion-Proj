...
RUN_DIR=${1:-"$RUN_ROOT/${STAMP}__a4-p2-mixed-precision-s0-r1"}

export PYTHONPATH="$PROJECT:$DRIVESTUDIO${PYTHONPATH:+:$PYTHONPATH}"
mkdir -p "$RUN_ROOT"

"$PYTHON" "$PROJECT/scripts/run_worldsim_v3_a4_p2_precision.py" \
  --run-dir "$RUN_DIR" --protocol "$PROTOCOL"
"$DRIVE_PYTHON" "$PROJECT/scripts/run_worldsim_v3_a4_p2_worker.py" \
  --run-dir "$RUN_DIR" --protocol "$PROTOCOL" --operation source-dtype-audit
"$DRIVE_PYTHON" "$PROJECT/scripts/run_worldsim_v3_a4_p2_worker.py" \
...
"$PYTHON" "$PROJECT/scripts/aggregate_worldsim_v3_a4_p2.py" \
  --run-dir "$RUN_DIR" --protocol "$PROTOCOL"
"$PYTHON" "$PROJECT/scripts/audit_worldsim_v3_a4_p2_resume.py" \
  --run-dir "$RUN_DIR" --protocol "$PROTOCOL"
"$PYTHON" "$PROJECT/scripts/finalize_worldsim_v3_a4_p2.py" \
  --run-dir "$RUN_DIR" --protocol "$PROTOCOL"
