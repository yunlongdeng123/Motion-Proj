"""Fine-tune anchor and child evidence through the deployed categorical return field."""
...
def _ray_values(
    ...
    occupied = torch.cat(
        [occupied_masses(anchor_logits), occupied_masses(child_logits)]
    )
...
def _losses(
    ...
    depths, target_depth, energy, anchor_logits, child_logits = _ray_values(
        anchor_authority, child_authority, actor, config
    )
...
def _train(
    ...
    for epoch in range(int(config["epochs"])):
        ...
        for start in range(0, len(permutation), batch_size):
            ...
            items = [
                _losses(
                    anchor_authority, child_authority, actors[index], config
                )
        ...
        history.append(row)
        print(json.dumps({"stage": "m40_joint_categorical_train", **row}), flush=True)
    return history
...
def _summarize(rows: list[Mapping[str, Any]]) -> dict[str, Any]:
    def stratum(selected: list[Mapping[str, Any]]) -> dict[str, Any]:
        ...
        for reference in ("baseline", "m39"):
            output[f"m40_vs_{reference}_early_delta"] = (
                output["m40_early_rate"] - output[f"{reference}_early_rate"]
            )
            output[f"m40_vs_{reference}_hit_delta"] = (
                output["m40_hit_rate"] - output[f"{reference}_hit_rate"]
            )
        return output
...
def run(config_path: Path, run_id: str) -> dict[str, Any]:
    ...
    if device.type != "cuda" or not torch.cuda.is_available():
        raise RuntimeError("M40 requires CUDA")
    torch.cuda.reset_peak_memory_stats(device)
    torch.manual_seed(int(config["model"]["seed"]))
    np.random.seed(int(config["model"]["seed"]))
    started = time.monotonic()
    try:
        actors = _load_actors(config, device)
        anchor_checkpoint = torch.load(
            Path(config["m35_run"]) / "MODEL.pt", map_location=device, weights_only=False
        )
        m39_anchor_reference = m38_runner._load_authority(anchor_checkpoint, device)
        m39_anchor_reference.eval().requires_grad_(False)
        ...
        m39_child_reference = m38_runner._load_authority(child_checkpoint, device)
        m39_child_reference.eval().requires_grad_(False)
        ...
        torch.save(
            {
                ...
                "initialized_anchor_from": str(Path(config["m35_run"]) / "MODEL.pt"),
                "initialized_child_from": str(Path(config["m38_run"]) / "CHILD_MODEL.pt"),
                "seed": int(config["model"]["seed"]),
                "states": ["FREE", "OCCUPIED", "UNKNOWN"],
                "deployment": "continuous-authority categorical Gaussian surface-return CDF",
            },
            run_dir / "MODEL.pt",
        )
        ...
        with torch.inference_mode():
            for index, actor in enumerate(holdout_actors):
                ...
                initial_anchor_logits = m39_anchor_reference(
                    actor["authority_anchor_features_t"]
                )
                initial_child_logits = m39_child_reference(
                    actor["authority_child_features_t"]
                )
                initial_occupied = torch.cat(
                    [
                        occupied_masses(initial_anchor_logits),
                        occupied_masses(initial_child_logits),
                    ]
                )
                m39_result = m39_runner._categorical_partition(
                    actor, initial_occupied, config["evaluation"]
                )
                anchor_logits = anchor_authority(actor["authority_anchor_features_t"])
                child_logits = child_authority(actor["authority_child_features_t"])
                anchor_masses = torch.softmax(anchor_logits, dim=1)
                child_masses = torch.softmax(child_logits, dim=1)
                m40_result = m39_runner._categorical_partition(
                    actor,
                    torch.cat([anchor_masses[:, 1], child_masses[:, 1]]),
                    config["evaluation"],
                )
                correlations["anchor_predicted"].append(anchor_masses[:, 1].cpu().numpy())
                correlations["anchor_target"].append(
                    actor["authority_target_masses_t"][:, 1].cpu().numpy()
                )
                correlations["child_predicted"].append(child_masses[:, 1].cpu().numpy())
                ...
                rows.append(
                    {
                        ...
                        "m40_early_count": int(np.count_nonzero(m40_result["early"])),
                        "m40_hit_count": int(np.count_nonzero(m40_result["hit"])),
                        "mean_anchor_masses": anchor_masses.mean(dim=0).cpu().tolist(),
                ...
                if (index + 1) % 10 == 0 or index + 1 == len(holdout_actors):
                    print(
                        json.dumps(
                            {"stage": "m40_holdout", "progress": f"{index + 1}/{len(holdout_actors)}"}
                        ),
                        flush=True,
                    )
        ...
        worst_stratum_delta = max(
            float(metrics["hazard"]["m40_vs_baseline_early_delta"]),
            float(metrics["clear"]["m40_vs_baseline_early_delta"]),
        )
        decisions = {
            "all_early_nonincrease": float(
                metrics["all"]["m40_vs_baseline_early_delta"]
            ...
            "all_hit_retained": float(metrics["all"]["m40_vs_baseline_hit_delta"])
        ...
        summary = {
            ...
            "status": "done",
            "verdict": (
                "m40_development_passed" if all(decisions.values())
                else "m40_development_rejected"
            ),
        ...
        m37_runner._write_jsonl(run_dir / "HOLDOUT_JOINT_AUTHORITY_ROWS.jsonl", rows)
        ...
        m37_runner._write_json(
            run_dir / "status.json",
            {"status": "failed", "phase": "m40", "error": f"{type(error).__name__}: {error}"},
        )
        raise
