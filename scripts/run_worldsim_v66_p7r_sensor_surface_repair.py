"""运行V6.6 P7R sensor-supported Actor local surface repair。"""
...
def run(config_path: Path, runs_root: Path, run_id: str) -> dict[str, object]:
    ...
    verdict = (
        "supported_consumed_legacy_sensor_surface_repair"
        if all(gates.values())
        else "rejected_consumed_legacy_sensor_surface_repair"
    )
    summary = {
        "schema_version": "worldsim_v66.p7r_sensor_surface_repair_summary.v1",
