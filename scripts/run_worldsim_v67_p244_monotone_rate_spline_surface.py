"""Distill a continuous monotone surface with analytically integrated rate splines."""
...
class MonotoneRateSplineSurface(nn.Module):
 ...
 def forward(self,feature,normalized_log_budget):
  context=self.encoder(feature);rates=functional.softplus(self.rate_knots(context).reshape(-1,4,self.knot_count));width=2./(self.knot_count-1);areas=.5*(rates[:,:,:-1]+rates[:,:,1:])*width;cumulative=torch.cat((torch.zeros_like(rates[:,:,:1]),torch.cumsum(areas,2)),2);position=((normalized_log_budget.reshape(-1,1)+1)/width).clamp(0,self.knot_count-1);index=torch.floor(position).long().clamp(max=self.knot_count-2);fraction=position-index;gather=index[:,None,:].expand(-1,4,-1);r0=torch.gather(rates,2,gather).squeeze(2);r1=torch.gather(rates,2,gather+1).squeeze(2);base=torch.gather(cumulative,2,gather).squeeze(2);integral=base+width*(r0*fraction+.5*(r1-r0)*fraction.square());units=torch.sigmoid(self.intercept(context)+integral);return torch.cumprod(units,1)
...
def _paired_dataset(path,common,feature_budgets,training_budgets,heldout_budgets):
 ...
 if not np.array_equal(scenes,training_scenes) or not np.array_equal(scenes,heldout_scenes):raise RuntimeError('P244 feature/training/heldout rows misaligned')
