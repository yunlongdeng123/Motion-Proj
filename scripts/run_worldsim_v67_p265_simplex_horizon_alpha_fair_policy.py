"""Compile alpha-fair budgets conditioned on a four-horizon preference simplex."""
...
class SimplexHorizonAlphaFairPolicy(nn.Module):
 def __init__(self,width,knot_count):
  super().__init__();self.knot_count=int(knot_count);self.encoder=nn.Sequential(nn.Linear(41,width),nn.SiLU(),nn.Linear(width,width),nn.SiLU());self.intercept=nn.Linear(width,1);self.rate_knots=nn.Linear(width,self.knot_count)
 def forward(self,feature,normalized_log_price,alpha,horizon_weights):
  context=self.encoder(torch.cat((feature,alpha[:,None],horizon_weights),1));rates=F.softplus(self.rate_knots(context));width=2./(self.knot_count-1);areas=.5*(rates[:,:-1]+rates[:,1:])*width;cumulative=torch.cat((torch.zeros_like(rates[:,:1]),torch.cumsum(areas,1)),1);position=((normalized_log_price+1)/width).clamp(0,self.knot_count-1);index=torch.floor(position).long().clamp(max=self.knot_count-2);fraction=position-index;r0=torch.gather(rates,1,index[:,None]).squeeze(1);r1=torch.gather(rates,1,(index+1)[:,None]).squeeze(1);base=torch.gather(cumulative,1,index[:,None]).squeeze(1);integral=base+width*(r0*fraction+.5*(r1-r0)*fraction.square());return torch.tanh(self.intercept(context).squeeze(1)-integral)


def _utility(probability,alpha,weights,epsilon):
 ...
 return np.sum(value*weights,axis=-1)
...
def _optimal_budget(model,feature,alphas,weight_vectors,prices,grid_z,epsilon,chunk):
 probability=_grid_probability(model,feature,grid_z,chunk);cost=(grid_z+1)/2;by_alpha=[]
 for alpha in alphas:
  by_weight=[]
  for weights in weight_vectors:
   utility=_utility(probability,alpha,weights,epsilon);by_weight.append(grid_z[np.argmax(utility[:,:,None]-prices[None,None,:]*cost[None,:,None],axis=1)])
  by_alpha.append(np.stack(by_weight,1))
 return np.stack(by_alpha,1)


def _evaluate(student,teacher,feature,target,alphas,weight_vectors,prices,price_z,epsilon):
 ...
 with torch.no_grad():
  for ai,alpha in enumerate(alphas):
   by_weight=[];by_cvalue=[];by_tvalue=[]
   for wi,weights in enumerate(weight_vectors):
    row=[];crow=[];trow=[];wv=torch.from_numpy(np.broadcast_to(weights,(len(x),4)).copy()).cuda()
    for pi,value in enumerate(price_z):
     av=torch.full((len(x),),float(alpha),device='cuda');prediction=student(x,torch.full((len(x),),float(value),device='cuda'),av,wv);truth=torch.from_numpy(target[:,ai,wi,pi]).cuda();cp=teacher(x,prediction).cpu().numpy();tp=teacher(x,truth).cpu().numpy();row.append(prediction.cpu().numpy());crow.append(_utility(cp,alpha,weights,epsilon)-prices[pi]*(prediction.cpu().numpy()+1)/2);trow.append(_utility(tp,alpha,weights,epsilon)-prices[pi]*(target[:,ai,wi,pi]+1)/2)
    by_weight.append(np.stack(row,1));by_cvalue.append(np.stack(crow,1));by_tvalue.append(np.stack(trow,1))
   candidate.append(np.stack(by_weight,1));candidate_value.append(np.stack(by_cvalue,1));target_value.append(np.stack(by_tvalue,1))
 torch.cuda.synchronize();forward=time.monotonic()-started;candidate=np.stack(candidate,1);candidate_value=np.stack(candidate_value,1);target_value=np.stack(target_value,1);return {'trajectory_count':int(len(feature)),'alpha_count':int(len(alphas)),'horizon_weight_count':int(len(weight_vectors)),'price_count':int(len(prices)),'normalized_log_budget_MAE':float(np.mean(np.abs(candidate-target))),'mean_frozen_simplex_alpha_fair_utility_regret':float(np.mean(target_value-candidate_value)),'price_monotonicity_violations':int(np.sum(np.diff(candidate,axis=3)>1e-7)),'student_forward_seconds':forward}
