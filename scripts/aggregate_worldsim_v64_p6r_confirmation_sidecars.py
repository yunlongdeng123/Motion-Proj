"""Aggregate per-scene P6R confirmation sidecars without copying arrays."""
...
def run(config_path: Path, task_root: Path, run_dir: Path) -> dict[str, object]:
    ...
    peak_gpu = 0.0
    for scene in scenes:
        source = task_root / f"20260826T144500Z__confirmation-native-{scene}-s0-r1"
        ...
        if not summary["passed"] or int(summary["target_count"]) != 12:
            raise RuntimeError(f"incomplete source: {source}")
...
def main() -> None:
    ...
    print(json.dumps(run(args.config.resolve(), args.task_root.resolve(), args.run_dir.resolve()), indent=2))
