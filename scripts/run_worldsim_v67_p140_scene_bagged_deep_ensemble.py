"""Train an Actor deep ensemble with one fixed source-scene bootstrap per member."""
...
from scripts.run_worldsim_v67_p139_scene_balanced_deep_ensemble import _actor_entries_with_scene


def _scene_bootstrap_pool(scene_ids: np.ndarray, seed: int) -> tuple[np.ndarray, int, int]:
    scenes = np.unique(scene_ids)
    sampled = np.random.default_rng(seed).choice(scenes, size=len(scenes), replace=True)
    pool = np.concatenate([np.flatnonzero(scene_ids == scene) for scene in sampled])
    return pool.astype(np.int64), int(len(np.unique(sampled))), int(len(scenes))
...
def main() -> None:
    ...
    for seed_value in config["member_seeds"]:
        ...
        pool_np, unique_count, scene_count = _scene_bootstrap_pool(scene_ids, seed_value)
        pool = torch.from_numpy(pool_np).cuda()
        bootstrap_stats[str(seed_value)] = {
            "drawn_scene_count": scene_count,
            "unique_scene_count": unique_count,
            "member_token_pool_size_with_duplicates": int(len(pool_np)),
        }
        ...
        for step in range(int(model_config["steps"])):
            position = torch.randint(
                len(pool), (int(model_config["batch_size"]),), device="cuda",
            )
            index = pool[position]
            ...
            if step % 500 == 0 or step + 1 == int(model_config["steps"]):
                print(
                    f"P140 scene-bagged seed={seed_value} step={step + 1} "
                    f"nll={final_loss:.6f} unique_scenes={unique_count}",
                    flush=True,
                )
    ...
    torch.save({
        ...
        "scene_bootstrap_stats": bootstrap_stats,
    ...
    for cohort in config["development_cohorts"]:
        ...
        if not np.array_equal(scenes, cost_scenes):
            raise RuntimeError("P140 trajectory grouping is not aligned")
        ...
        results[cohort["name"]] = {
            ...
            "scene_bagged_selected_mean_cost": float(actual_cost[selected].mean()),
            "p126_selected_mean_cost": float(reference["selected_cost"]),
            "scene_bagged_cost_spearman": model_spearman,
    ...
    decisions = {
        "no_selected_cost_regression": all(
            x["scene_bagged_selected_mean_cost"] <= x["p126_selected_mean_cost"]
    ...
    summary = {
        ...
        "role": config["role"],
        "training": {
            "actor_time_tokens": int(len(features)),
            "source_scenes": int(len(np.unique(scene_ids))),
            "member_scene_bootstrap": bootstrap_stats,
    ...
    print(json.dumps({
        "run_dir": str(run_dir), "verdict": verdict, "decision_checks": decisions,
    }, indent=2), flush=True)
