"""Train an Actor ensemble conditioned on normalized fraction and absolute future time."""
...
from motion_proj.worldsim_v67.actor_state_reliability import (
    ACTOR_FEATURE_NAMES, spearman_correlation,
)
...
def _absolute_time_actor_entries(
    ...
    absolute_time = horizons[:, None] * fractions[None, :]
...
def _trajectory_score(
    ...
    actor_features, _, inverse = _absolute_time_actor_entries(arrays)
...
def main() -> None:
    ...
    raw_features, raw_target, _ = _absolute_time_actor_entries(source)
    ...
    for seed_value in config["member_seeds"]:
        ...
        for step in range(int(model_config["steps"])):
            ...
            if step % 500 == 0 or step + 1 == int(model_config["steps"]):
                print(
                    f"P145 absolute-time seed={seed_value} step={step + 1} nll={final_loss:.6f}",
                    flush=True,
                )
    ...
    for cohort in config["development_cohorts"]:
        ...
        if not np.array_equal(scenes, cost_scenes):
            raise RuntimeError("P145 trajectory grouping is not aligned")
        ...
        results[cohort["name"]] = {
            ...
            "absolute_time_selected_mean_cost": float(actual_cost[selected].mean()),
            "p126_selected_mean_cost": float(reference["selected_cost"]),
            "absolute_time_cost_spearman": model_spearman,
    ...
    decisions = {
        "no_selected_cost_regression": all(
            x["absolute_time_selected_mean_cost"] <= x["p126_selected_mean_cost"]
