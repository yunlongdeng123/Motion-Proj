#!/usr/bin/env python3
"""Run frozen perception over logged and edited full-episode sensor arrays."""
...
def _load_model(root: Path) -> torch.nn.Module:
    ...
    model = deeplabv3_resnet50(weights=None, weights_backbone=None, num_classes=19, aux_loss=True)
...
def _load_rgb(path: Path, key: str) -> np.ndarray:
...
def main() -> int:
    ...
    for line in args.index.read_text(encoding="utf-8").splitlines():
        ...
        labels = _predict(model, _load_rgb(Path(item["render_path"]), item["rgb_key"]))
        ...
        rows.append({
            "case_id": item["case_id"], "frame_index": item["frame_index"],
            "variant": item["variant"], "rgb_key": item["rgb_key"],
            "repeat_index": item["repeat_index"], "label_path": relative,
            "label_array_sha256": _array_sha256(labels), "label_file_sha256": hashlib.sha256(output_path.read_bytes()).hexdigest(),
            "unique_label_count": int(np.unique(labels).size),
        })
    _write_jsonl(args.output_dir / "PERCEPTION_OUTPUTS.jsonl", rows)
    _write_json(args.output_dir / "WORKER_RESULT.json", {
        "schema_version": "worldsim_v6.r87_perception_worker.v1", "row_count": len(rows),
        "elapsed_seconds": time.monotonic() - started,
        "peak_gpu_memory_mib": int(torch.cuda.max_memory_allocated() / (1024 * 1024)),
        "sensor_rgb_keys": sorted({row["rgb_key"] for row in rows}),
    })
    return 0
