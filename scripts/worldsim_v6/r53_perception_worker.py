#!/usr/bin/env python3
"""隔离执行 R53 compiled sensor 的冻结语义感知。"""
...
def _load_compiled_rgb(path: Path) -> np.ndarray:
    with np.load(path, allow_pickle=False) as archive:
        image = np.asarray(archive["compiled_rgb"])
...
def main() -> int:
    ...
    for line in args.index.read_text(encoding="utf-8").splitlines():
        ...
        labels = _predict(model, _load_compiled_rgb(Path(item["render_path"])))
        ...
        rows.append({
            "case_id": item["case_id"], "repeat_index": item["repeat_index"], "label_path": relative,
    ...
    _write_json(args.output_dir / "WORKER_RESULT.json", {
        "schema_version": "worldsim_v6.r53_perception_worker.v1", "row_count": len(rows),
        "elapsed_seconds": time.monotonic() - started,
        "peak_gpu_memory_mib": int(torch.cuda.max_memory_allocated() / (1024 * 1024)),
        "sensor_rgb_key": "compiled_rgb",
    })
    return 0
