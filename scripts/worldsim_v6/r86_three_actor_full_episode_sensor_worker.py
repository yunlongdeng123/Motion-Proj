#!/usr/bin/env python3
"""Execute a three-actor scene patch for a full episode with logged baselines."""
...
def main() -> int:
    ...
    with torch.inference_mode():
        for frame_index in frames:
            ...
            logged_rgb = _numpy(logged["rgb"]).astype(np.float32)
            logged_depth = _numpy(logged["depth"]).astype(np.float32)
            logged_opacity = _numpy(logged["opacity"]).astype(np.float32)
            ...
            np.savez_compressed(
                output / sensor_name,
                logged_rgb=logged_rgb.astype(np.float16),
                logged_depth=logged_depth,
                logged_opacity=logged_opacity.astype(np.float16),
            ...
            rows.append(
                {
                    ...
                    "edited_vs_logged_rgb_changed_pixels": int(
                        (
                            np.mean(
                                np.abs(
                                    native_rgb.astype(np.float64)
                                    - logged_rgb.astype(np.float64)
                                ),
                                axis=-1,
                            )
                            > 1.0e-6
                    ...
                    "edited_vs_logged_rgb_mae": float(
                        np.mean(
                            np.abs(
                                native_rgb.astype(np.float64) - logged_rgb.astype(np.float64)
                            )
                        )
                    ),
    ...
    _write_json(
        ...
        {
            "schema_version": "worldsim_v6.r86_worker_audit.v1",
