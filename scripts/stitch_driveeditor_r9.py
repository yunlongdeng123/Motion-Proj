...
def assemble(row: dict, window_root: Path, output_root: Path, manifest_root: Path) -> dict:
    cid = row["case_id"]
    case = row["case"]
    ...
    for index in range(10):
        ...
        video = part_root / "counterfactual.mp4"
        if not record.is_file() or not video.is_file():
            return {"case_id": cid, "status": "partial", "ready_windows": len(parts)}
        outcome = json.loads(record.read_text(encoding="utf-8"))
        if outcome.get("status") != "generation_complete" or outcome.get("steps") != 25:
            return {"case_id": cid, "status": "partial", "ready_windows": len(parts)}
    ...
    if result_path.is_file():
        ...
        if prior.get("status") == "generation_complete" and video_info(case_root / "counterfactual.mp4")[:2] == (100, 10.0):
            return prior
    ...
    result = {"case_id": cid, "status": "generation_complete", "generated_frames": 100,
              "native_windows": 10, "native_window_duration_s": 1.0, "output_fps": 10,
...
def main() -> None:
    ...
    results = [assemble(row, args.window_root, args.output_root, args.manifest_root) for row in manifest["cases"]]
