...
def main():
    ...
    for row in rows:
        ...
        record = {"case_id": cid, "input": row, "generation": result, "automatic": evaluation, "input_audit": load(path / "input-audit.json", {}), "ai_preliminary": ai.get(cid), "human_verdict": None}
        items.append(record)
    ...
    body = [f'<h1>OmniDreams · 自建反事实 benchmark</h1><p class="muted">{completion} · {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")}</p>',
        ...
        '<section class="card" id="overview"><h2>逐 case 六维对照</h2><p class="note">0–4 为 AI 序数诊断，非官方论文分数。— 表示未评／弃权／不适用，不等于 0。每项分母不同；不合成总分。</p><div class="scroll"><table><thead><tr><th>Case</th><th>状态</th>'+''.join(f'<th>{d}</th>' for d in DIMS)+'<th>背景 MAE↓</th><th>事实 PSNR↑</th></tr></thead><tbody>']
    for item in items:
        cid = item["case_id"]
        scores = (item["ai_preliminary"] or {}).get("dimensions", {})
        summary = item["automatic"].get("summary", {})
        body.append(f'<tr><td><a href="#{cid}">{cid}</a></td><td>{"已生成" if item["generation"].get("status") == "generation_complete" else "运行中"}</td>'+''.join(f'<td>{fmt(scores.get(d, {}).get("pair"))}</td>' for d in DIMS)+f'<td>{fmt(summary.get("outside_edit_mae_01_mean"))}</td><td>{fmt(summary.get("factual_psnr_db_mean"))}</td></tr>')
    ...
    for item in items:
        ...
        body.append(f'<article id="{cid}" data-case="{cid}"><h2>{cid}</h2><p><span class="pill">{row["scene_name"]} · camera {row["camera_index"]}</span> {html.escape(json.dumps(row["case"]["intervention"], ensure_ascii=False))}</p>')
