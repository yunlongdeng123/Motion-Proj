#!/usr/bin/env python3
"""按冻结顺序执行 DR-M3 scene-0230 预处理与分级训练门禁。"""
...
SCENE = PROCESSED_ROOT / "scene-0230"
...
SOURCE_INPUTS = [
    ("runner.py", PROJECT / "scripts/run_dr_m3_scene0230.py"),
...
def initialize_run(run_dir):
    ...
    if not terminal.is_file():
        raise RuntimeError("M2 尚无 terminal.json，禁止启动 M3")
    m2_terminal = json.loads(terminal.read_text())
    if m2_terminal.get("status") != "done":
        raise RuntimeError("M2 未通过，禁止启动 M3")
    ...
    if existing_manifest.is_file():
        ...
        if existing.get("config_fingerprint") != fingerprint:
            raise RuntimeError("已有 M3 实例 fingerprint 与当前配置不一致")
        return
...
def update_run_summary(run_dir, requested_through, failure=None):
    ...
    summary = """# DR-M3-ADGS-0230-01 Summary
...
100/1,000 iterations 只用于工程画像；只有 60,000 iterations + official render/metrics 完整后，M3 才能标记 `done`。
""".format(
        status=status,
