...
parser=argparse.ArgumentParser();parser.add_argument('--summary',type=Path,required=True);parser.add_argument('--output',type=Path,required=True);args=parser.parse_args();args.output.mkdir(parents=True,exist_ok=True)
...
for i,ds in enumerate(['nuscenes','av2']):
    for method,stats in data['summary'][ds].items():
        ...
        q=stats['ready']['metrics_query'];hit=q['hit_rate']['mean'];free=q['mean_free_intrusion_m']['mean'];recall=q['positive_surface_recall_02']['mean']
        if None in [hit,free,recall]:continue
        marker='o' if method in ['WEX','RIF','DCS'] else '^'
        for ax,x,label in [(axes[i,0],free,'Mean free-space intrusion (m), lower is better'),(axes[i,1],recall*100,'Surface recall @ 0.2m (%), higher is better')]:
            ax.scatter(x,hit*100,c=colors[method],marker=marker,s=70,label=method);ax.annotate(method,(x,hit*100),xytext=(5,4),textcoords='offset points',fontsize=8)
            ax.set_xlabel(label);ax.set_ylabel('Held-out first-hit rate (%)');ax.grid(alpha=.2);ax.set_title(ds+' / own FIT, exposed DEV')
...
fig.savefig(args.output/'quality_tradeoffs.png');fig.savefig(args.output/'quality_tradeoffs.pdf');plt.close(fig)
...
box(2.65,2.6,2.4,.65,'A: shared-domain exchange','#d8e9f5');box(2.65,1.5,2.4,.65,'B: interval FEM field','#fce7d5');box(2.65,.4,2.4,.65,'C: demand-driven patches','#dcefe3')
...
for method,control in [('WEX','A3_milp'),('RIF','B2_regular'),('DCS','C1_residual')]:
    ...
    points=np.vstack([build,query]);lo=points.min(0);hi=points.max(0);center=(lo+hi)/2;radius=max((hi-lo).max()/2,.25)*1.15
    fig=plt.figure(figsize=(11,4.7));fig.suptitle(f'{method} vs {control}: largest negative nuScenes hit difference ({delta*100:+.2f} pp)\n{row["case_id"]}',fontsize=10)
    for i,r in enumerate([row,ref]):
        ax=fig.add_subplot(1,2,i+1,projection='3d')
        with np.load(r['surface_path']) as z:v=z['vertices_actor_m'];f=z['faces']
        if len(f):ax.add_collection3d(Poly3DCollection(v[f],facecolor='#6494b0',edgecolor='#39576b',linewidths=.15,alpha=.5))
        ax.scatter(*build.T,s=4,c='#222222',label='BUILD');ax.scatter(*query.T,s=5,c='#d44935',label='held-out returns')
        ...
        q=r['metrics_query'];ax.set_title(f'{r["method"]}: hit {q["hit_rate"]*100:.1f}%, miss {q["miss_rate"]*100:.1f}%');ax.legend(fontsize=7,loc='lower left')
