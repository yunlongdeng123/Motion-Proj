"""Train a task-conditioned ensemble for residual error projected on each query boundary."""
...
class ConditionalProjectedGaussian(nn.Module):
...
def _conditional_tokens(arrays: dict[str, np.ndarray]) -> tuple[np.ndarray, np.ndarray]:
    residual = np.asarray(
    ...
    target = np.sum(residual * normal, axis=2)
    ...
    with torch.no_grad():
        for start in range(0, len(raw_features), 262144):
            batch = torch.from_numpy(
                (raw_features[start:start + 262144] - feature_mean) / feature_scale,
            ).cuda()
...
def _trajectory_score(
    ...
    raw_features, _ = _conditional_tokens(arrays)
    point_count = int(arrays["actor_position_error_vector_ego_profile_m"].shape[1])
    member_means, member_scales = [], []
    for model in models:
        mean, scale = _predict(
            model, raw_features, feature_mean, feature_scale, target_mean, target_scale,
        )
    ...
    projected_mean = means.mean(axis=0)
    epistemic_variance = means.var(axis=0)
    aleatoric_variance = np.square(scales).mean(axis=0)
...
def main() -> None:
    ...
    raw_features, raw_target = _conditional_tokens(source)
    ...
    for seed_value in config["member_seeds"]:
        ...
        for step in range(int(model_config["steps"])):
            ...
            if step % 500 == 0 or step + 1 == int(model_config["steps"]):
                print(
                    f"P142 conditional-projection seed={seed_value} step={step + 1} "
    ...
    for cohort in config["development_cohorts"]:
        ...
        if not np.array_equal(scenes, cost_scenes):
            raise RuntimeError("P142 trajectory grouping is not aligned")
        ...
        results[cohort["name"]] = {
            ...
            "conditional_selected_mean_cost": float(actual_cost[selected].mean()),
            "p126_selected_mean_cost": float(reference["selected_cost"]),
            "conditional_cost_spearman": model_spearman,
    ...
    decisions = {
        "no_selected_cost_regression": all(
            x["conditional_selected_mean_cost"] <= x["p126_selected_mean_cost"]
