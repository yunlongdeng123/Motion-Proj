"""Score frozen global and stratum-conditional selective compiler arms."""
...
def _arm_summary(cases: list[dict[str, object]], arm: str) -> dict[str, object]:
    rows = [row["arms"][arm] for row in cases]
    failures = sum(bool(row["case_loss"]) for row in rows)
    strata = {}
    for stratum in sorted({str(row["stratum"]) for row in cases}):
        selected = [row["arms"][arm] for row in cases if row["stratum"] == stratum]
        stratum_failures = sum(bool(row["case_loss"]) for row in selected)
        strata[stratum] = {
            "case_count": len(selected),
            "failure_count": stratum_failures,
            "empirical_case_risk": float(stratum_failures / len(selected)),
            "mean_realized_coverage": float(np.mean([row["realized_coverage"] for row in selected])),
        }
    return {
        "case_count": len(rows),
...
def run(config_path: Path, runs_root: Path, run_id: str) -> dict[str, object]:
    ...
    for scene in scenes:
        for evidence_unit in _unit_dirs(evidence_root, scene):
            ...
            arms = {}
            for arm, coverage in (
                ("c0_global", global_coverage),
                ("m0_conditional", conditional_coverages[strata[scene]]),
            ):
    ...
    arms = {name: _arm_summary(cases, name) for name in ("c0_global", "m0_conditional")}
    coverage_uplift = float(
        arms["m0_conditional"]["mean_realized_coverage"]
        - arms["c0_global"]["mean_realized_coverage"]
    )
    gates_config = config["gates"]
    gates = {
        "minimum_conditional_coverage_uplift": coverage_uplift
        >= float(gates_config["minimum_conditional_coverage_uplift"]),
        "maximum_conditional_failures": int(arms["m0_conditional"]["failure_count"])
        <= int(gates_config["maximum_conditional_failures"]),
        "maximum_conditional_failures_per_stratum": all(
            int(row["failure_count"])
            <= int(gates_config["maximum_conditional_failures_per_stratum"])
            for row in arms["m0_conditional"]["strata"].values()
        ),
    }
    if bool(gates_config.get("require_not_more_failures_than_global", False)):
        gates["not_more_failures_than_global"] = (
            int(arms["m0_conditional"]["failure_count"])
            <= int(arms["c0_global"]["failure_count"])
        )
    verdict = (
    ...
    summary = {
        ...
        "arms": arms,
        "conditional_coverage_uplift": coverage_uplift,
        "gate_results": gates,
        "model_refit": False,
    ...
    return {"run_dir": str(run_dir), "verdict": verdict, "coverage_uplift": coverage_uplift, "gate_results": gates}
