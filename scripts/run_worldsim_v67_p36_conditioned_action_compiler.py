"""Train a condition-aware differentiable top-k action compiler."""
...
def run(config_path: Path, runs_root: Path, run_id: str) -> dict[str, object]:
    ...
    }, run_dir / "CONDITIONED_ACTION_COMPILER.pt")
    ...
    conditioned_scores = score_conditioned_action_compiler(model, selection, fraction, [horizon], mean, scale)
    ...
    zeros = np.zeros(len(cases["target_offset"]), dtype=np.float32)
    scene_to_group = {int(k): int(v) for k, v in config["compiler"]["scene_to_group"].items()}
    shared = (
        fraction, int(config["compiler"]["maximum_actions_per_case"]),
        float(config["compiler"]["minimum_case_coverage"]), scene_to_group,
        float(config["compiler"]["minimum_group_case_coverage"]),
    )
    conditioned = group_coverage_constrained_selection(selection, conditioned_scores, zeros, *shared)
    ...
    gates = {
        ...
        "minimum_group_coverage": conditioned["minimum_group_case_coverage"] >= float(gates_cfg["minimum_group_case_coverage"]),
        "improves_frozen_joint_compiler": improvement["delta_over_frozen_joint_compiler"] >= float(gates_cfg["minimum_delta_over_joint_compiler"]),
        "minimum_scene_support": conditioned["scene_nonincreasing_count"] >= int(gates_cfg["minimum_nonincreasing_scene_support"]),
    }
