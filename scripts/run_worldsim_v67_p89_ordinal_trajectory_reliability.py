"""Train a monotone multi-threshold reliability curve for visited Actor sets."""
...
class OrdinalDeepSetRisk(torch.nn.Module):
    def __init__(
        self, feature_count: int, threshold_count: int,
        element_dimensions: Sequence[int], decoder_dimensions: Sequence[int],
    ) -> None:
        super().__init__()
        ...
        self.ordinal_head = torch.nn.Linear(decoder_width, threshold_count)
    ...
    def forward(self, features: torch.Tensor, mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        mean = (encoded * expanded_mask).sum(1) / expanded_mask.sum(1).clamp(min=1)
        maximum = encoded.masked_fill(~expanded_mask, -torch.inf).max(1).values
        decoded = self.decoder(torch.cat((mean, maximum), dim=1))
        raw = self.ordinal_head(decoded)
        logits = [raw[:, 0]]
        for index in range(1, raw.shape[1]):
            logits.append(logits[-1] - torch.nn.functional.softplus(raw[:, index]))
        return torch.stack(logits, dim=1), torch.nn.functional.softplus(
...
@torch.no_grad()
def _predict(
    model: OrdinalDeepSetRisk, features: torch.Tensor, mask: torch.Tensor,
    threshold_index: int, batch_size: int = 4096,
) -> np.ndarray:
    outputs = []
    for start in range(0, len(features), batch_size):
        logits, _ = model(features[start:start + batch_size], mask[start:start + batch_size])
        outputs.append(logits[:, threshold_index].cpu().numpy())
...
def main() -> None:
    ...
    thresholds = np.asarray(model_config["ordinal_error_thresholds_m"], dtype=np.float32)
    threshold_count = len(thresholds)
    pair_groups = []
    for threshold_index, threshold in enumerate(thresholds):
        labels = source["max_error"] > threshold
        for horizon in sorted(np.unique(source["horizon_seconds"]).tolist()):
            members = source["horizon_seconds"] == horizon
            positive, negative = np.flatnonzero(members & labels), np.flatnonzero(members & ~labels)
            if len(positive) and len(negative):
                pair_groups.append((threshold_index, torch.from_numpy(positive).long().cuda(),
                    torch.from_numpy(negative).long().cuda()))
    query_model = OrdinalDeepSetRisk(len(FEATURE_NAMES), threshold_count,
        model_config["element_dimensions"], model_config["decoder_dimensions"]).cuda()
    actor_model = OrdinalDeepSetRisk(len(ACTOR_FEATURE_NAMES), threshold_count,
        model_config["element_dimensions"], model_config["decoder_dimensions"]).cuda()
    ...
    pair_per_group = max(1, int(model_config["pair_batch_size"]) // len(pair_groups))
    final = {}
    ...
    for epoch in range(int(model_config["epochs"])):
        ...
        negative_parts = []
        threshold_parts = []
        for threshold_index, positive, negative in pair_groups:
            positive_parts.append(positive[torch.randint(len(positive), (pair_per_group,), device="cuda")])
            negative_parts.append(negative[torch.randint(len(negative), (pair_per_group,), device="cuda")])
            threshold_parts.append(torch.full((pair_per_group,), threshold_index, dtype=torch.long, device="cuda"))
        positive = torch.cat(positive_parts); negative = torch.cat(negative_parts); threshold_ids = torch.cat(threshold_parts)
        query_positive, _ = query_model(query_sets[positive], mask[positive])
        query_negative, _ = query_model(query_sets[negative], mask[negative])
        actor_positive, _ = actor_model(actor_sets[positive], mask[positive])
        actor_negative, _ = actor_model(actor_sets[negative], mask[negative])
        row_index = torch.arange(len(positive), device="cuda")
        query_pair = torch.nn.functional.softplus(float(model_config["pair_margin"])
            - query_positive[row_index, threshold_ids] + query_negative[row_index, threshold_ids]).mean()
        actor_pair = torch.nn.functional.softplus(float(model_config["pair_margin"])
            - actor_positive[row_index, threshold_ids] + actor_negative[row_index, threshold_ids]).mean()
        ...
        if epoch % 250 == 0 or epoch + 1 == int(model_config["epochs"]):
            print(f"ordinal-trajectory epoch={epoch + 1} " + " ".join(f"{k}={v:.6f}" for k, v in final.items()), flush=True)
    torch.save({"query_feature_mean": query_mean, "query_feature_scale": query_scale,
        ...
        "maximum_visited_actors": model_config["maximum_visited_actors"], "thresholds_m": thresholds,
        "selection_threshold_index": model_config["selection_threshold_index"],
        ...
        run_dir / "ORDINAL_TRAJECTORY_RELIABILITY.pt")
    ...
    selection_threshold_index = int(model_config["selection_threshold_index"])
    query_score = _predict(query_model.eval(), evaluation_query, evaluation_mask, selection_threshold_index)
    actor_score = _predict(actor_model.eval(), evaluation_actor, evaluation_mask, selection_threshold_index)
    ...
    summary = {"schema_version": config["output_schema_version"], "task_id": config["task_id"], "hypothesis_id": config["hypothesis_id"],
        ...
        "training": {"source_trajectory_count": int(len(source["max_error"])), "ordinal_thresholds_m": thresholds.tolist(),
            "threshold_horizon_pair_groups": int(len(pair_groups)), "final_losses": final},
