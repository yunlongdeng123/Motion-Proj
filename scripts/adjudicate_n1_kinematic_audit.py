#!/usr/bin/env python
"""不可变登记第三次 N1 kinematics-first 人工评测的用户 reject。

本工具只校验、复制并聚合用户填写的 verdict，不修改父 run，不代填人工判断，
也不授权或启动 N2。
"""
...
from scripts.validate_n1_kinematic_review import (
    COMPONENT_FIELDS,
    validate,
)
...
def _unique_terminal(parent: Path) -> str:
    ...
    if markers != ["AWAITING_HUMAN_REVIEW"]:
        raise RuntimeError(
            f"父 run 终态不唯一或不是 AWAITING_HUMAN_REVIEW: {markers}"
        )
    return markers[0]
...
def adjudicate(config_path: Path, output_root: Path | None = None) -> Path:
    ...
    if validation["recommended_research_verdict"] != config["decision"][
        "research_verdict"
    ]:
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/resim/n1_kinematic_audit_rejection_v1.yaml"),
    )
