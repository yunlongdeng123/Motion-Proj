"""Train a low-rank-plus-diagonal conditional copula after the P206 static ablation."""
...
class LowRankConditionalCopula(nn.Module):
...
def main():
 ...
 h=np.asarray(c['horizons_seconds'],np.float32);scores,clearances,costs,scenes=_align(_trajectory_payload(arrays,members,ens,float(c['boundary_state_cost']['clearance_floor_m'])),h);raw=np.concatenate((scores,clearances),1);features=torch.from_numpy(((raw-np.asarray(p199['feature_mean']))/np.asarray(p199['feature_scale'])).astype(np.float32)).cuda();norms=tuple(fd['norms']);clip=float(c['evaluation']['probability_clip']);u=np.stack([_variable_cdf(density,scores[:,i],np.full(len(scores),h[i],np.float32),clearances[:,i],costs[:,i],norms) for i in range(4)],1);normal=torch.distributions.Normal(torch.tensor(0.),torch.tensor(1.));z=normal.icdf(torch.from_numpy(np.clip(u,clip,1-clip))).float().cuda();split=c['split'];dev=scenes%int(split['development_scene_modulus'])==int(split['development_scene_remainder']);train=~dev;indices=torch.from_numpy(np.flatnonzero(train)).cuda();m=c['model'];model=LowRankConditionalCopula(8,m['hidden_dimensions'],4,int(m['rank'])).cuda();opt=torch.optim.AdamW(model.parameters(),lr=float(m['learning_rate']),weight_decay=float(m['weight_decay']));last=0.
 for step in range(int(m['steps'])):
  idx=indices[torch.randint(len(indices),(int(m['batch_size']),),device='cuda')];chol=model.correlation_cholesky(features[idx]);target=z[idx];solved=torch.cholesky_solve(target[:,:,None],chol).squeeze(2);loss=.5*(2*torch.log(torch.diagonal(chol,dim1=1,dim2=2)).sum(1)+(target*solved).sum(1)).mean();opt.zero_grad();loss.backward();opt.step();last=float(loss.detach())
  if step%500==0:print(f'P207 low-rank conditional copula step={step+1} nll={last:.6f}',flush=True)
 budgets=np.asarray(c['reliability_budgets'],np.float32);marginal=np.stack([_predict_cdf(density,scores[:,i],np.full(len(scores),h[i],np.float32),clearances[:,i],budgets,norms) for i in range(4)],1);dev_idx=torch.from_numpy(np.flatnonzero(dev)).cuda();marginal_tensor=torch.from_numpy(marginal[dev].astype(np.float32)).cuda();mc=int(c['evaluation']['monte_carlo_samples']);candidate=_joint_probabilities(model,features[dev_idx],marginal_tensor,mc,int(c['seed']));baseline=_joint_probabilities(control,features[dev_idx],marginal_tensor,mc,int(c['seed']));truth=np.all(costs[dev,:,None]<=budgets[None,None,:],1);cb=float(np.mean((candidate-truth)**2));pb=float(np.mean((baseline-truth)**2));ce=float(np.mean(np.abs(candidate.mean(0)-truth.mean(0))));pe=float(np.mean(np.abs(baseline.mean(0)-truth.mean(0))));br=(pb-cb)/pb;cr=(pe-ce)/max(pe,1e-12);checks={'integrated_Brier_strictly_better_than_P199':cb<pb,'calibration_error_noninferior_to_P199':cr>=float(c['decision']['minimum_calibration_error_reduction_vs_P199'])};verdict=c['verdict_on_pass'] if all(checks.values()) else c['verdict_on_failure'];torch.save({'model_state_dict':model.state_dict(),'rank':m['rank'],'hidden_dimensions':m['hidden_dimensions'],'feature_mean':p199['feature_mean'],'feature_scale':p199['feature_scale'],'base_marginal':c['frozen_p182']},d/c['model_artifact']);summary={'schema_version':c['output_schema_version'],'task_id':c['task_id'],'hypothesis_id':c['hypothesis_id'],'status':'done','verdict':verdict,'role':c['role'],'training':{'final_copula_nll_without_marginal_constant':last},'development':{'low_rank_integrated_brier':cb,'P199_integrated_brier':pb,'Brier_reduction_vs_P199':br,'low_rank_mean_absolute_reliability_error':ce,'P199_mean_absolute_reliability_error':pe,'calibration_error_reduction_vs_P199':cr},'decision_checks':checks,'resources':{'gpu':torch.cuda.get_device_name(0),'peak_gpu_memory_gib':torch.cuda.max_memory_allocated()/2**30,'peak_rss_gib':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/2**20,'wall_seconds':time.monotonic()-started},'claim_boundary':c['claim_boundary']};(d/'summary.json').write_text(json.dumps(summary,indent=2)+'\n');(d/'status.json').write_text(json.dumps({'status':'done','completed_at_utc':datetime.now(timezone.utc).isoformat()},indent=2)+'\n');print(json.dumps({'run_dir':str(d),**summary},indent=2))
