"""在V6.7任务未读cohort上迁移冻结的Actor-local geometry head。"""
...
def run(config_path: Path, runs_root: Path, run_id: str) -> dict[str, object]:
    ...
    checkpoint_path = runs_root / str(config["model"]["run"]) / str(
        config["model"]["artifact"]
    )
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    if tuple(checkpoint["feature_names"]) != FEATURE_NAMES:
        raise RuntimeError("V6.7 feature contract differs from the frozen V6.6 checkpoint")
    ...
    verdict = (
        ...
        if all(gates.values())
        else "rejected_task_untouched_legacy_geometry_transfer"
    )
    (run_dir / "TRANSFER_SCORES.jsonl").write_text(
        "".join(
            json.dumps(
                {
    ...
    summary = {
        "schema_version": "worldsim_v67.p1_geometry_transfer_summary.v1",
