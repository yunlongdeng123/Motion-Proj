"""Extend the supported P126 ensemble from three to five independently trained members."""
...
def main() -> None:
    ...
    for seed_value in config["new_member_seeds"]:
        ...
        for step in range(int(model_config["steps"])):
            ...
            if step % 500 == 0 or step + 1 == int(model_config["steps"]):
                print(
                    f"P141 five-member seed={seed_value} step={step + 1} nll={final_loss:.6f}",
                    flush=True,
                )
    ...
    torch.save({
        ...
        + [int(x) for x in config["new_member_seeds"]],
    ...
    for cohort in config["development_cohorts"]:
        ...
        if not np.array_equal(scenes, cost_scenes):
            raise RuntimeError("P141 trajectory grouping is not aligned")
        ...
        results[cohort["name"]] = {
            ...
            "five_member_selected_mean_cost": float(actual_cost[selected].mean()),
            "three_member_selected_mean_cost": float(reference["selected_cost"]),
            "five_member_cost_spearman": model_spearman,
            "three_member_cost_spearman": float(reference["spearman"]),
            "spearman_gain_over_three_member": float(
                model_spearman - float(reference["spearman"])
            ),
        }
        print(json.dumps({cohort["name"]: results[cohort["name"]]}, indent=2), flush=True)
    gains = [x["spearman_gain_over_three_member"] for x in results.values()]
    decisions = {
        "no_selected_cost_regression": all(
            x["five_member_selected_mean_cost"] <= x["three_member_selected_mean_cost"]
    ...
    summary = {
        ...
        "training": {
            ...
            "reused_member_count": int(len(frozen["member_state_dicts"])),
            "new_member_final_nll": final_losses,
            "ensemble_member_count": int(len(models)),
        },
