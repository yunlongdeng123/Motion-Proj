"""Fine-tune M8 with GT-supervised bidirectional tail surface tubes."""
...
def _tail_mean(values: torch.Tensor, fraction: float) -> torch.Tensor:
    ...
    return torch.topk(values, k=count, largest=True, sorted=False).values.mean()


def _surface_tail(
    surface: torch.Tensor, targets: torch.Tensor, fraction: float
) -> tuple[torch.Tensor, torch.Tensor]:
    distances = torch.cdist(targets, surface)
    target_to_surface = _tail_mean(distances.min(dim=1).values, fraction)
    surface_to_target = _tail_mean(distances.min(dim=0).values, fraction)
    return target_to_surface, surface_to_target


def _prepare_tail_actor(
    ...
    actor["m8_frame_reference_t"] = _frame_coverage(surface, targets, groups).clamp_min(
        1.0e-3
    ).detach()

    maximum = int(model_config["maximum_tail_rays_per_frame"])
    tail_indices = []
    for indices in groups:
        if len(indices) > maximum:
            positions = torch.linspace(
                0, len(indices) - 1, steps=maximum, device=device
            ).round().long()
            indices = indices.index_select(0, positions)
        tail_indices.append(indices)
    actor["tail_targets_t"] = targets.index_select(0, torch.cat(tail_indices))
    with torch.no_grad():
        target_tail, surface_tail = _surface_tail(
            surface,
            actor["tail_targets_t"],
            float(model_config["tail_fraction"]),
        )
    actor["m8_target_tail_reference_t"] = target_tail.clamp_min(1.0e-3).detach()
    actor["m8_surface_tail_reference_t"] = surface_tail.clamp_min(1.0e-3).detach()


def _geometry_with_tail(
    ...
    ) / actor["m8_frame_reference_t"]
    target_tail, surface_tail = _surface_tail(
        surface, actor["tail_targets_t"], float(config["tail_fraction"])
    )
    target_tail = target_tail / actor["m8_target_tail_reference_t"]
    surface_tail = surface_tail / actor["m8_surface_tail_reference_t"]
    tail = 0.5 * (target_tail + surface_tail)
    losses["temporal_frame"] = temporal
    losses["target_tail"] = target_tail
    losses["surface_tail"] = surface_tail
    losses["tail"] = tail
    losses["geometry_tail"] = (
        losses["geometry"]
        + float(config["temporal_frame_weight"]) * temporal
        + float(config["tail_weight"]) * tail
    )
    return losses
...
def _train(
    ...
    for epoch in range(int(config["fine_tune_epochs"])):
        ...
        for start in range(0, len(permutation), batch_size):
            ...
            geometry_items = [
                _geometry_with_tail(model, actors[index], config) for index in indices
            ]
            physics_items = [
            ...
            conflict, cosine = m6_runner._pcgrad_step(
                model, optimizer, means["geometry_tail"], means["physics"]
            )
        ...
        history.append(row)
        print(
            json.dumps({"stage": "m29_gt_tail_finetune", "actors": len(actors), **row}),
            flush=True,
        )
    return history
...
def _surface_tail_metrics(
    surface: torch.Tensor, actor: Mapping[str, Any], fraction: float
) -> tuple[float, float]:
    target_tail, surface_tail = _surface_tail(
        surface, actor["tail_targets_t"], fraction
    )
    return float(target_tail), float(surface_tail)
...
def _frame_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    def stratum(selected: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        current = np.asarray(
            [row["m29_frame_mean_target_distance_m"] for row in selected], dtype=np.float64
        )
        return {
            "actor_count": len(selected),
            "m8_frame_mean_target_distance_m": float(reference.mean()),
            "m29_frame_mean_target_distance_m": float(current.mean()),
            ...
            "m29_mean_worst_frame_distance_m": float(
                np.mean([row["m29_worst_frame_target_distance_m"] for row in selected])
            ),
        }
...
def _tail_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    def stratum(selected: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        def mean(name: str) -> float:
            return float(np.mean([row[name] for row in selected]))

        m8_target = mean("m8_target_tail_distance_m")
        m29_target = mean("m29_target_tail_distance_m")
        m8_surface = mean("m8_surface_tail_distance_m")
        m29_surface = mean("m29_surface_tail_distance_m")
        return {
            "actor_count": len(selected),
            "m8_target_tail_distance_m": m8_target,
            "m29_target_tail_distance_m": m29_target,
            "target_tail_delta_m": m29_target - m8_target,
            "m8_surface_tail_distance_m": m8_surface,
            "m29_surface_tail_distance_m": m29_surface,
            "surface_tail_delta_m": m29_surface - m8_surface,
        }
...
def _decisions(
    ...
    m8_metrics: Mapping[str, Any],
    tails: Mapping[str, Any],
    config: Mapping[str, Any],
) -> dict[str, bool]:
    minimum = float(config["minimum_tail_improvement_m"])
    return {
        "target_tail_improvement": float(tails["all"]["target_tail_delta_m"])
        < -minimum,
        "surface_tail_improvement": float(tails["all"]["surface_tail_delta_m"])
        < -minimum,
        "hazard_early_non_degradation": float(metrics["hazard"]["output_early_rate"])
        <= float(m8_metrics["hazard"]["output_early_rate"])
        + float(config["maximum_hazard_early_rate_delta"]),
        "chamfer_non_degradation": float(metrics["output_mean_chamfer_m"])
        <= float(m8_metrics["output_mean_chamfer_m"])
        + float(config["maximum_chamfer_delta_m"]),
        "hit_recall_non_degradation": float(metrics["output_hit_recall"])
        >= float(m8_metrics["output_hit_recall"])
...
def run(config_path: Path, run_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        for actor in actors:
            _prepare_tail_actor(actor, config["model"], device)
        ...
        with torch.inference_mode():
            for index, actor in enumerate(holdout_actors):
                ...
                m8_surface_t = torch.as_tensor(
                    m8_surface, dtype=torch.float32, device=device
                )
                output_t = torch.as_tensor(output, dtype=torch.float32, device=device)
                m8_frame = _surface_frame_metrics(m8_surface_t, actor)
                m29_frame = _surface_frame_metrics(
                    output_t, actor
                )
                m8_tail = _surface_tail_metrics(
                    m8_surface_t, actor, float(config["model"]["tail_fraction"])
                )
                m29_tail = _surface_tail_metrics(
                    output_t, actor, float(config["model"]["tail_fraction"])
                )
                del m8_surface_t, output_t
                ...
                row.update(
                    {
                        **identity,
                        ...
                        "m29_frame_mean_target_distance_m": m29_frame[0],
                        "m29_worst_frame_target_distance_m": m29_frame[1],
                        "m8_target_tail_distance_m": m8_tail[0],
                        "m8_surface_tail_distance_m": m8_tail[1],
                        "m29_target_tail_distance_m": m29_tail[0],
                        "m29_surface_tail_distance_m": m29_tail[1],
                    }
                )
        ...
        tails = _tail_summary(rows)
        decisions = _decisions(metrics, m8_metrics, tails, config["decision"])
        passed = all(decisions.values())
        m6_runner._write_jsonl(run_dir / "TRAIN_HOLDOUT_TAIL.jsonl", rows)
        summary = {
            "schema_version": "worldsim_v71.m29_gt_tail_surface_tube.v1",
            "task_id": config["task_id"],
            "hypothesis_id": config["hypothesis_id"],
            "status": "done",
            "verdict": "m29_development_supported" if passed else "m29_development_rejected",
        ...
        m6_runner._write_json(
            run_dir / "status.json",
            {"status": "failed", "phase": "m29", "error": f"{type(error).__name__}: {error}"},
        )
        raise
