...
readonly COHORT=/root/autodl-tmp/motion_proj/configs/worldsim_v7/av2_evicomp_fresh_cohort_v1.json
readonly DATA_ROOT=/root/autodl-tmp/data/av2/sensor/val
readonly STATE_ROOT=/root/autodl-tmp/data/av2/v7_p16_download_state
readonly DOWNLOAD_LOG=/root/autodl-tmp/data/av2/v7_p16_download.log
readonly MINIMUM_FREE_BYTES=80530636800

mkdir -p "$DATA_ROOT" "$STATE_ROOT"
exec 9>"$STATE_ROOT/download.lock"
if ! flock -n 9; then
  printf '%s another P16 downloader already owns the lock\n' "$(date --iso-8601=seconds)" >>"$DOWNLOAD_LOG"
  exit 3
fi
...
if [[ "${#log_ids[@]}" -ne 10 ]]; then
  printf '%s invalid P16 cohort count=%s\n' "$(date --iso-8601=seconds)" "${#log_ids[@]}" >>"$DOWNLOAD_LOG"
  exit 4
fi
...
for index in "${!log_ids[@]}"; do
  ...
  while true; do
    ...
    if "$S5CMD" --numworkers 8 --log error --no-sign-request sync \
