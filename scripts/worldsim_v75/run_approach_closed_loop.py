...
def main():
    ...
    conditioning=args.conditioning_dir or OUT/'conditioning'; conditioning.mkdir(exist_ok=True)
    ...
    if args.arm!='gt_clean':
        ...
        supplement_path=OUT/'reconstruction/reference_supplement_result.json'
        if supplement_path.exists():
            supplement=json.loads(supplement_path.read_text())
            assert supplement['status']=='complete' and supplement['generation_admitted'] and supplement['target']==source['target']
            assert supplement['source_readout']==str(OUT/'reconstruction/readout_ray_control_result.json')
            # 原4点拒绝保留；只替换明确补证的参照，不改DVGT或全局尺度读出。
            read['readouts']['reference_lidar']=supplement['reference_readout']; read['generation_admitted']=True
            error_protocol={'source_readout':supplement['source_readout'],'reference_supplement':str(supplement_path),
                            'supplement_protocol':str(supplement_path.with_name('reference_supplement_protocol.json')),
                            'change':'reference arm has two additional pre-cutoff LiDAR scans; DVGT and global-scale readouts unchanged',
                            'arms':['dvgt_metric','dvgt_lidar_scaled','reference_lidar'],'seed':42,'frames':117,'human_verdict':None}
            path=OUT/'error_arm_protocol.json'
            if path.exists(): assert json.loads(path.read_text())==error_protocol
            else: save(path,error_protocol)
